...
func (r *Repository) RecordStabilization(ctx context.Context, c billingmigration.RecordStabilizationCommand) (billingmigration.StabilizationObservation, bool, error) {
	...
	COALESCE((SELECT sum(error_count) FROM billing_migration_access_api_signal_windows WHERE program_id=$1 AND project_id=$2 AND window_ended_at>=GREATEST($3,$7::timestamptz-($8*interval '1 second'))),$5+1),
	EXISTS(SELECT 1 FROM billing_migration_access_api_signal_windows WHERE program_id=$1 AND project_id=$2 AND request_count>0 AND window_ended_at>=GREATEST($3,$7::timestamptz-($8*interval '1 second'))),
	...
	if breaches == nil {
		// A healthy observation breaches nothing, and a nil slice encodes as
		// NULL against a text[] NOT NULL column — so the healthy path, the
		// one that matters most, could never be persisted.
		breaches = []string{}
	}
