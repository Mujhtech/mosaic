...
export function TransactionFactDetailPage({
  ...
  const replay = useMutation(createReplayJobMutationOptions(projectId, environmentId, queryClient))
  ...
  const factsByAttemptId = new Map<string, TransactionFact>(
    (siblingFacts.data?.items ?? []).flatMap((item) =>
      item.validationAttemptId ? [[item.validationAttemptId, item] as const] : [],
    ),
  )
        ...
        {record ? (
          <>
            <div className="flex flex-wrap items-center gap-2">
              ...
              />
              <StatusPill label={factKindLabel(record.factKind)} tone="neutral" />
            ...
            >
              <div className="mb-4">
                <DualTimestamps occurredAt={record.occurredAt} recordedAt={record.recordedAt} />
              </div>
              <dl>
                ...
                />
                <DefinitionRow label="Fact kind" value={factKindLabel(record.factKind)} />
                <DefinitionRow
                ...
                />
                <DefinitionRow label="Revoked" value={record.revokedAt ?? "—"} />
                <DefinitionRow label="Refunded" value={record.refundedAt ?? "—"} />
              </dl>
              <p className="text-muted-foreground mt-4 text-xs leading-5">
                The period above is what the store stated about this transaction. Mosaic does not
                interpret it, extend it, or use it to decide what anyone may do in your app.
              </p>
            </WorkflowPanel>
            ...
            <ReplayPanel
              ...
              onReplay={() =>
                replay.mutate({
                  kind: "revalidation",
                  ...(record.sourceRawInputId ? { rawInputId: record.sourceRawInputId } : {}),
                })
              }
              quarantineHref={`${billingBase}/quarantine`}
