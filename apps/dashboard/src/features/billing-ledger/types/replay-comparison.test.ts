...
describe("replay comparison", () => {
  it("keeps both attempts, whichever order the caller supplies", () => {
    const earlier = attempt({ attemptNumber: 4, id: "attempt_4", outcome: "quarantined" })
    const later = attempt({ attemptNumber: 5, id: "attempt_5", outcome: "validated" })

    const forwards = compareReplayAttempts({ attempt: earlier }, { attempt: later })
    const backwards = compareReplayAttempts({ attempt: later }, { attempt: earlier })
  ...
  it("flags a conflict when the outcome or the resolved Product changes", () => {
    const earlier = attempt({ attemptNumber: 1, id: "attempt_1", outcome: "validated" })
    const later = attempt({ attemptNumber: 2, id: "attempt_2", outcome: "quarantined" })

    const outcomeConflict = compareReplayAttempts({ attempt: earlier }, { attempt: later })
    ...
    const productConflict = compareReplayAttempts(
      { attempt: earlier, fact: { mosaicProductId: "prod_a" } as TransactionFact },
      {
  ...
  it("reports no conflict, and marks fields unchanged, when the replay reproduces the result", () => {
    const fact = { mosaicProductId: "prod_a", resolutionState: "active_mapping" } as TransactionFact
    ...
    expect(comparison.rows.find((row) => row.label === "Outcome")?.changed).toBe(false)
    expect(comparison.rows.find((row) => row.label === "Resolved Mosaic Product")?.changed).toBe(
      false,
    )
  })
  ...
  it("compares the newest attempt against the one before it and leaves older ones intact", () => {
    ...
    expect(attempts.map((item) => item.id)).toEqual(["attempt_1", "attempt_3", "attempt_2"])
  })
