...
export function parseTransactionFilters(search: Record<string, unknown>): TransactionFilters {
  ...
  const orderedRange = from && to && Date.parse(from) > Date.parse(to) ? {} : { from, to }
  const limit = TRANSACTION_PAGE_SIZES.find((size) => size === Number(search.limit))

  return {
    applicationId: safeString(search.applicationId, IDENTIFIER),
    cursor: safeString(search.cursor, /^[\x20-\x7E]{1,512}$/),
    ...orderedRange,
...
export function serializeTransactionFilters(filters: TransactionFilters): TransactionFilters {
  const entries = Object.entries(filters).filter(([, value]) => value !== undefined && value !== "")
  return { ...(Object.fromEntries(entries) as TransactionFilters), limit: filters.limit }
}
...
export function clientAppliedFilterCount(filters: TransactionFilters) {
  return CLIENT_APPLIED_FILTER_KEYS.filter((key) => filters[key] !== undefined).length
}
...
export function applyClientTransactionFilters(
  ...
  return items.filter((item) => {
    if (filters.applicationId && item.applicationId !== filters.applicationId) return false
    if (filters.productId && item.mosaicProductId !== filters.productId) return false
    if (filters.storeEnvironment && item.storeEnvironment !== filters.storeEnvironment) return false
    if (filters.resolutionState && item.resolutionState !== filters.resolutionState) return false
    if (reference) {
      const haystack = [
