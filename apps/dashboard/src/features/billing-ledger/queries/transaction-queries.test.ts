...
const { transactionKeys, validationAttemptsQueryOptions } = await import(
  "@/features/billing-ledger/queries/transaction-queries"
);
...
describe("validation attempt scoping", () => {
  ...
  it("asks the API for one input's attempts rather than filtering a page in the browser", async () => {
    const options = validationAttemptsQueryOptions(
      "proj_1",
      "env_1",
      "rawin_42"
    );
  ...
  it("caches each input's history separately, so one record cannot answer for another", () => {
    ...
    expect(
      transactionKeys
        .attempts("proj_1", "env_1", "rawin_42")
        .slice(0, scope.length)
    ).toEqual([...scope]);
  });
});
