...
export const transactionKeys = {
  ...
  facts: (projectId: string, environmentId: string, filters: Record<string, unknown>) =>
    ...
    queryFn: async ({ signal }): Promise<TransactionFact | null> => {
      ...
      for (let page = 0; page < FACT_LOOKUP_MAX_PAGES; page += 1) {
        const result = await listTransactionFacts({
          client: generatedDashboardClient,
          path: { environmentId, projectId },
          query: { limit: FACT_LOOKUP_PAGE_SIZE, ...(cursor ? { cursor } : {}) },
          signal,
          throwOnError: true,
        })
        const match = result.data.data?.items?.find((item) => item.id === factId)
...
export function billingLedgerQueryOptions(projectId: string, environmentId: string) {
  return queryOptions({
