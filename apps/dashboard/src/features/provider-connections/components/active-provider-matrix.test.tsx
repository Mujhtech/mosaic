...
describe("ActiveProviderMatrix", () => {
  it("lets Members review impact and directs them to an Owner or Admin instead of mutating", () => {
    ...
    expect(
      screen.getByText("0 affected Products · 0 active Paywalls")
    ).toBeVisible();
    expect(
      screen.queryByRole("button", { name: "Confirm clear" })
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "Ask an Owner or Admin" })
  ...
  it("renders StoreKit as a credential-free native assignment", () => {
    const queryClient = new QueryClient();
    queryClient.setQueryData(
      providerConnectionKeys.nativeProfile("app_store", "ios"),
      {
        adapterVersion: "2.0",
        capabilities: [
    ...
    expect(
      screen.getByText(/Built in · no server credentials · adapter 2.0/)
    ).toBeVisible();
    expect(
      screen.getByText("1 conditional or unsupported capability")
    ).toBeVisible();
  });
  ...
  it("renders a persisted assignment with explicit Environment, Application, and platform", () => {
    ...
    expect(
      screen.getByRole("row", { name: /Example iOS.*IOS.*RevenueCat sandbox/i })
    ).toBeVisible();
  ...
  it("describes persisted custom SDK-only metadata without implying hosted authorization", () => {
    ...
    expect(
      screen.getByText("Custom provider · SDK-only · sandbox")
    ).toBeVisible();
    expect(screen.getByText("Never synchronized")).toBeVisible();
    expect(
      screen.getByText(/host app supplies this custom provider at runtime/i)
    ).toBeVisible();
  ...
  it("gates clearing behind the affected Product and active Paywall impact review", () => {
    ...
    expect(
      screen.getByText("1 affected Product · 1 active Paywall")
    ).toBeVisible();
    ...
    expect(
      screen.getByText(
        /new publishing will fail readiness and SDK configuration cannot resolve/i
      )
    ).toBeVisible();
