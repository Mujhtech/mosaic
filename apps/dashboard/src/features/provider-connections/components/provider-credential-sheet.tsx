...
export function ProviderCredentialSheet({
  ...
  const title = action === "rotate" ? "Rotate credential" : "Reconnect RevenueCat"
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        setSubmitError(error instanceof Error ? error.message : `${title} failed.`)
      } finally {
  ...
  return (
    <Sheet
      ...
      open={open}
    >
      <SheetTrigger render={<Button type="button" variant="outline" />}>{title}</SheetTrigger>
      <SheetContent className="w-full sm:max-w-md">
        <SheetHeader className="border-b p-5">
          <SheetTitle>{title}</SheetTitle>
          <SheetDescription>
            The replacement secret is validated before it replaces the current encrypted credential.
          </SheetDescription>
        </SheetHeader>
        <form
            ...
            >
              {(field) => (
                <Field data-invalid={field.state.meta.errors.length > 0}>
                  ...
                  />
                  <FieldDescription>
                    Cleared after this attempt and never returned or shown again.
                  </FieldDescription>
