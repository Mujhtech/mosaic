...
describe("active provider assignment cache impact", () => {
  it("invalidates only the assignment scope and affected Project catalog", async () => {
    ...
    expect(queryClient.getQueryState(projectCatalogKey)?.isInvalidated).toBe(true)
    expect(queryClient.getQueryState(unrelatedAssignmentKey)?.isInvalidated).toBe(false)
    expect(queryClient.getQueryState(unrelatedCatalogKey)?.isInvalidated).toBe(false)
  })
})
