...
export class ProviderConnectionCreatedButTestFailed extends Error {
  constructor(readonly connection: ProviderConnection) {
    super(
...
async function invalidateConnection(
  ...
  await Promise.all([
    queryClient.invalidateQueries({ queryKey: providerConnectionKeys.list(projectId) }),
    queryClient.invalidateQueries({ queryKey: providerConnectionKeys.detail(connectionId) }),
    queryClient.invalidateQueries({ queryKey: providerConnectionKeys.health(connectionId) }),
    queryClient.invalidateQueries({ queryKey: providerConnectionKeys.capabilities(connectionId) }),
    queryClient.invalidateQueries({ queryKey: providerConnectionKeys.diagnostics(connectionId) }),
  ])
}
  ...
  return mutationOptions({
    mutationFn: async (input: CreateRevenueCatConnectionInput) => {
      ...
      try {
        ...
        throw new ProviderConnectionCreatedButTestFailed(connection)
      }
    },
    onSettled: async (result, error) => {
      const connection =
        result?.connection ??
        (error instanceof ProviderConnectionCreatedButTestFailed ? error.connection : undefined)
      if (connection) await invalidateConnection(queryClient, connection.id, projectId)
    },
  })
}
...
export function testProviderConnectionMutationOptions(
  ...
  return mutationOptions({
    ...
    onSettled: async () => invalidateConnection(queryClient, connectionId, projectId),
  })
}
...
export function replaceProviderCredentialMutationOptions(
  ...
  return mutationOptions({
    mutationFn: async (input: ReplaceProviderCredentialInput) => {
      const request = action === "rotate" ? rotateProviderCredential : reconnectProviderConnection
  ...
  return mutationOptions({
    ...
    onSettled: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: providerConnectionKeys.syncRuns(connectionId) }),
      ])
    },
  })
}
...
export function setActiveProviderMutationOptions(
  ...
  return mutationOptions({
    ...
    onSuccess: async () =>
      invalidateActiveProviderImpact(applicationId, environmentId, projectId, queryClient),
  })
}
