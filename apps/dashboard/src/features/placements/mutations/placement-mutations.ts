...
export function createPlacementAndBindMutationOptions(
  ...
  return mutationOptions({
    ...
    onError: async (error) => {
      if (!(error instanceof PlacementCreatedWithoutBindingError)) return
      ...
      await queryClient.invalidateQueries({ queryKey: placementKeys.list(scope) })
    },
      ...
      queryClient.setQueryData<readonly HostedPlacement[]>(
        ...
        (current) =>
          current?.some((item) => item.id === placement.id)
            ? current.map((item) => (item.id === placement.id ? placement : item))
            : [...(current ?? []), placement],
      )
      await queryClient.invalidateQueries({ queryKey: placementKeys.list(scope) })
