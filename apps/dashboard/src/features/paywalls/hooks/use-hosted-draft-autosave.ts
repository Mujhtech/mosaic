...
export function useHostedDraftAutosave({
  ...
  const [status, setStatus] =
    useState<DraftAutosaveController["status"]>("idle");
      ...
      while (pendingRef.current && !conflictRef.current) {
        ...
        try {
          ...
          if (error instanceof HostedDraftConflictError) {
            ...
            setStatus(
              error instanceof HostedDraftOfflineError ? "offline" : "failed"
            );
          }
          return false;
        }
  ...
  const retry = useCallback(() => {
    if (!enabled || conflictRef.current || !pendingRef.current) {
      return;
    }
    attemptSave();
  }, [attemptSave, enabled]);
