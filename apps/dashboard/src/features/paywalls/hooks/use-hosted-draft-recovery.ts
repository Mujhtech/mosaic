...
export function useHostedDraftRecovery({
  ...
  const persist = useCallback(
    (reason = recoveryReason(status)) => {
      ...
      const didPersist = writeHostedDraftRecovery(scope, { document, expectedRevision, reason })
      if (didPersist) {
  ...
  useEffect(() => {
    ...
    const beforeUnload = (event: BeforeUnloadEvent) => {
      if (status === "idle" || status === "saved" || persist()) return
  ...
  return {
    clear,
    persist,
    prepareNavigation: () => status === "idle" || status === "saved" || persist(),
    record,
  }
}
