...
export async function createPaywallWithDraft(
  ...
  try {
    ...
    // biome-ignore lint/style/useErrorCause: PaywallCreatedWithoutDraftError chains the cause through its constructor
...
export function createHostedDraftMutationOptions(
  ...
  return mutationOptions({
    mutationFn: (document: MosaicDocument) =>
      adapter.createDraft({ ...input, document }),
    onSuccess: (draft) => {
      queryClient.setQueryData(
        paywallKeys.draft(
          {
...
export function createPaywallWithDraftMutationOptions(
  ...
  return mutationOptions({
    ...
    onError: async (error) => {
      if (!(error instanceof PaywallCreatedWithoutDraftError)) {
        return;
      }
      queryClient.setQueryData(
        paywallKeys.detail(
          { paywallId: error.paywall.id, projectId: input.projectId },
          adapter
        ),
        error.paywall
      );
      await queryClient.invalidateQueries({
        queryKey: paywallKeys.all(input.projectId),
      });
    },
    onSuccess: async ({ draft, paywall }) => {
      queryClient.setQueryData(
        paywallKeys.detail(
          { paywallId: paywall.id, projectId: input.projectId },
          adapter
        ),
