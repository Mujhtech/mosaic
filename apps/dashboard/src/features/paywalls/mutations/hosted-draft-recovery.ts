...
function isRecoveryRecord(value: unknown): value is HostedDraftRecoveryRecord {
  if (!isRecord(value) || value.version !== HOSTED_DRAFT_RECOVERY_VERSION) return false
  return (
    ...
    (value.reason === "conflict" || value.reason === "offline" || value.reason === "unsaved") &&
...
export function writeHostedDraftRecovery(
  ...
  try {
    ...
    if (new Blob([serialized]).size > MAX_RECOVERY_BYTES) return false
...
export function listHostedDraftRecoveries(input: {
  ...
  for (let index = 0; index < window.localStorage.length; index += 1) {
    ...
    if (!key?.startsWith(HOSTED_DRAFT_RECOVERY_PREFIX)) continue
    try {
  ...
  return recoveries.toSorted((left, right) => right.savedAt.localeCompare(left.savedAt))
}
...
export function serializeHostedRecoveryDocument(record: HostedDraftRecoveryRecord) {
