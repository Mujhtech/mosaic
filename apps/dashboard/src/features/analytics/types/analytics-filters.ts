...
export function parseAnalyticsFilters(
  ...
  const from =
    typeof search.from === "string" && ISO_DATE.test(search.from)
      ? search.from
      : defaults.from;
  const to =
    typeof search.to === "string" && ISO_DATE.test(search.to)
      ? search.to
      : defaults.to;
  return {
    ...
    platform:
      search.platform === "ios" || search.platform === "android"
        ...
        : undefined,
    locale:
      typeof search.locale === "string" && LOCALE.test(search.locale)
        ? search.locale
        : undefined,
    applicationVersion:
      typeof search.applicationVersion === "string" &&
      search.applicationVersion.length <= 64
