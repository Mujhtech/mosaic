...
export function useAnalyticsRole(organizationId: string): AnalyticsRole | undefined {
  const session = useQuery(sessionQueryOptions())
  const members = useQuery(membersQueryOptions(organizationId))
  return members.data?.items.find((membership) => membership.actorId === session.data?.id)?.role
}
