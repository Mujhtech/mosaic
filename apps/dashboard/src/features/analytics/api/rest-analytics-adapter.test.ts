...
describe("privacy REST requests", () => {
  it("keeps identity values in POST bodies and out of URLs", async () => {
    ...
    const adapter = createGeneratedAnalyticsAdapter(createGeneratedDashboardClient(fetchMock))

    await adapter.previewIdentity(
      { organizationId: "org_01", projectId: "project_01", environmentId: "environment_01" },
      { scope: "application_user", value: identity },
    )

    const call = fetchMock.mock.calls[0]
    expect(call).toBeDefined()
    if (!call) throw new Error("Expected one privacy request")
    const request = call[0] as Request
    expect(request.url).not.toContain(identity)
    expect(request.method).toBe("POST")
    expect(await request.clone().text()).toContain(identity)
    expect(JSON.stringify(analyticsKeysForInspection())).not.toContain(identity)
  })

  it("requires a fresh preview digest and explicit deletion confirmation without leaking identity", async () => {
    const identity = { scope: "installation" as const, value: "private-installation-value" }
    const fetchMock = vi.fn<(input: RequestInfo | URL, init?: RequestInit) => Promise<Response>>(
      async () =>
        new Response(
          JSON.stringify({
    ...
    const call = fetchMock.mock.calls[0]
    if (!call) throw new Error("Expected one deletion request")
    const request = call[0] as Request
...
describe("analytics dimension mapping", () => {
  ...
  it("maps backend dimension keys into Paywall comparison and issue rows", async () => {
      ...
      async (input) => {
        ...
        const metrics = url.includes("paywall-version-comparison")
          ...
          : url.includes("provider-errors")
            ...
            : []
        return new Response(JSON.stringify({ data: { metrics, freshness: {} } }), {
          status: 200,
    ...
    await expect(adapter.getPaywallComparison(scope, filters)).resolves.toMatchObject([
              ...
              metrics: [
                {
                  ...
                  dimensions: isPlatform ? { platform: "ios" } : { locale: "en-NG" },
                },
              ],
              freshness: {},
            },
