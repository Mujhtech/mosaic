...
export function EventExportAction({
  ...
  const downloadMutation = useMutation({
    mutationFn: async () => {
      if (!jobId) throw new Error("The export job is unavailable.")
      return adapter.downloadJob(scope, jobId)
    },
    onSuccess: (blob) => downloadBlob(blob, "mosaic-event-export.ndjson"),
  })
  const job = useQuery({ ...jobQueryOptions(scope, jobId ?? "", adapter), enabled: Boolean(jobId) })
  ...
  return (
    <div className="flex items-center gap-2">
      {job.data?.state === "completed" && job.data.downloadAvailable ? (
        ...
        <Button
          disabled={
            mutation.isPending || job.data?.state === "queued" || job.data?.state === "leased"
          }
      ...
      {job.data?.state === "failed" ? (
        <Button onClick={() => mutation.mutate()} type="button" variant="outline">
          Retry export
        </Button>
      ) : null}
      {mutation.error ? (
