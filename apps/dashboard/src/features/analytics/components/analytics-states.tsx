    ...
    >
      ...
      <div>
        ...
        <p className="mt-0.5 text-muted-foreground text-xs">
          ...
          latest event received {formatTimestamp(freshness.latestReceivedAt)}.{" "}
          {freshness.lateEventPolicy}
        </p>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <p>
        ...
        {sampleSize === 1 ? "presentation" : "presentations"}. Mosaic does not
        declare a winner below 100 presentations.
      </p>
    </div>
  );
}
...
export function WarningList({ warnings }: { warnings: string[] }) {
  ...
  return (
    <ul className="space-y-1 rounded border border-border bg-muted/20 p-3 text-sm">
