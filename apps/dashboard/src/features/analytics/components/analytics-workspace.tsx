...
export function AnalyticsWorkspace({
  ...
  return (
    <WorkspacePage
      actions={
        surface === "data-privacy" ? undefined : (
          <EventExportAction
        ...
        <div className="rounded border border-border bg-muted/20 p-3">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <span className="flex items-center gap-2 text-muted-foreground text-xs">
              <ChartLineUpIcon aria-hidden size={16} /> Single-Environment
              reporting · event-count basis
            </span>
          </div>
          <nav
            aria-label="Analytics"
            className="mt-3 flex flex-wrap gap-1 border-t pt-3"
          >
            {tabs.map((tab) => (
              <Link
                className={cn(
                  "rounded px-3 py-2 font-medium text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                  tab.surface === surface
    ...
    return (
      <div className="space-y-4">
        <div
          ...
          role="status"
        >
          <strong>Authority stays separate.</strong> Client-observed completions
          include purchased outcomes only; already-entitled is separate.
          Provider-confirmed completion is unavailable until a trusted server or
          provider source is configured—it is never displayed as zero.
        </div>
        <FunnelPanel
          adapter={restAnalyticsAdapter}
          filters={filters}
