...
export function CreateMigrationProgramForm({
        ...
        const program = await onCreate({
          body: {
            applications: value.applicationIds.map((applicationId) => {
              ...
              if (!application) throw new Error("invalid_scope")
  ...
  return (
    ...
    >
      <form.Field
        name="environmentId"
        validators={{ onSubmit: ({ value }) => (value ? undefined : "Choose an Environment.") }}
      >
        {(field) => (
      ...
      <form.Field
        ...
        validators={{
          onSubmit: ({ value }) => (value.trim() ? undefined : "Enter the RevenueCat Project ID."),
        }}
      >
        {(field) => (
          <Field data-invalid={field.state.meta.errors.length > 0}>
            <FieldLabel htmlFor="migration-source-project">RevenueCat Project ID</FieldLabel>
            <Input
      ...
      <form.Field
        ...
        validators={{
          onSubmit: ({ value }) => (value ? undefined : "Enter a least-privilege migration key."),
        }}
      >
        {(field) => (
          <Field className="md:col-span-2" data-invalid={field.state.meta.errors.length > 0}>
            <FieldLabel htmlFor="migration-source-key">RevenueCat migration API key</FieldLabel>
            <Input
              autoComplete="off"
              ...
              value={field.state.value}
            />
            <FieldDescription>Cleared after every submission and never read back.</FieldDescription>
            <FieldError errors={field.state.meta.errors.map((message) => ({ message }))} />
          </Field>
        )}
      </form.Field>
      <form.Field
        ...
        validators={{
          onSubmit: ({ value }) =>
            value.length ? undefined : "Select at least one Application/platform scope.",
        }}
      >
        {(field) => (
          <Field className="md:col-span-2" data-invalid={field.state.meta.errors.length > 0}>
            ...
            <FieldDescription>
              Choose every Application and platform explicitly. Mosaic does not infer or wildcard
              this scope.
            </FieldDescription>
            <div className="grid gap-2 rounded border p-3 sm:grid-cols-2">
              {applications.map((application) => (
                <label className="flex items-center gap-2 text-sm" key={application.id}>
                  <input
