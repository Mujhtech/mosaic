...
export function MigrationProgramsPage({
  ...
  const create = useMutation(createMigrationProgramMutationOptions(projectId, queryClient))
  ...
  const error =
    project.error ?? access.error ?? programs.error ?? environments.error ?? applications.error
  const state = resolveHostedQueryState({
    ...
    isPending:
      project.isPending ||
      access.isPending ||
      (scopeReady && (programs.isPending || environments.isPending || applications.isPending)),
    loadingDescription: "Loading Project migration programs and their available scope.",
    onRetry: () => {
      void programs.refetch()
      void environments.refetch()
      void applications.refetch()
    },
    permissionDescription: "Project membership is required to view Migration Programs.",
  ...
  const createDialog = (
    <Dialog
      ...
      open={createOpen}
    >
      <DialogTrigger render={<Button size="sm" />}>Create program</DialogTrigger>
        ...
        <div className="px-4 pb-4">
          <CreateMigrationProgramForm
            ...
            onCreated={(program) => {
              setCreateOpen(false)
              void navigate({
                params: (prev) => ({ ...prev, programId: program.programId }),
          ...
          <WorkflowPanel title={`${items.length} Migration Program(s)`}>
            <ul className="divide-y">
              {items.map((detail) => {
                ...
                return (
                  <li className="flex items-center gap-3 py-4" key={program.programId}>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground mt-1 text-xs">
                        {program.scope.applications.length} explicit Application/platform scope(s) ·
                        state version {program.stateVersion}
                      </p>
                    </div>
                    <StatusPill
                      label={program.state.replaceAll("_", " ")}
                      tone={
                        program.state === "failed" || program.state === "cancelled"
                          ? "negative"
                          : "neutral"
                      }
                    />
