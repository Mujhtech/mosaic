...
export function FreezeMappingAction({
  ...
  return (
    <div className="ml-auto max-w-xl rounded border border-dashed p-3">
      <p className="font-semibold text-sm">Freeze version {mapping.version}</p>
      <p className="mt-1 text-muted-foreground text-xs">
        Impact: mapping set <code>{mapping.mappingSetId}</code> with digest{" "}
        <code className="break-all">{mapping.mappingDigest}</code> becomes
        immutable and the Program advances to Import.
      </p>
