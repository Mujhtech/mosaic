...
describe("GuidedMappingForm", () => {
  it("validates structured rows and presents an exact pre-submit review", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Review mapping set" })
    ).toBeDisabled();
    expect(
      screen.getByText("Enter the exact source identifier.")
    ).toBeInTheDocument();
    fireEvent.change(
    ...
    fireEvent.change(
      screen.getByRole("textbox", { name: /Mosaic target ID/ }),
      {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Create reviewed draft" })
    );
