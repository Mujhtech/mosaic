...
describe("billing migration operational safety", () => {
  ...
  it("fails closed and uses only server-returned command capabilities", () => {
    ...
    expect(
      detail(["manage-mappings", "unknown", "run-import"]).commandCapabilities
    ).toEqual(new Set(["manage-mappings", "run-import"]));
  });
  ...
  it("states that dry and shadow runs confer no billing authority", () => {
    expect(dryRunAuthorityNotice("dry_run")).toContain(
      "does not change billing authority"
    );
    expect(dryRunAuthorityNotice("shadow")).toContain(
      "does not change billing authority"
    );
  });
  ...
  it("protects the critical journey by state and server capability", () => {
    expect(
      migrationCommandJourney("mapping", detail(["manage-mappings"]))
    ).toMatchObject({
    ...
    expect(
      migrationCommandJourney("importing", detail(["run-import"]))
    ).toMatchObject({
      canCreateMapping: false,
      canImport: true,
      canQueueDryRun: true,
    });
    expect(migrationCommandJourney("shadowing", detail([])).canAssess).toBe(
      false
    );
    expect(
      migrationCommandJourney("shadowing", detail(["assess-readiness"]))
        .canAssess
    ).toBe(true);
  });
  ...
  it("fails completion closed and exposes server blockers", () => {
    expect(migrationCompletionBlockers(undefined)).toEqual([
