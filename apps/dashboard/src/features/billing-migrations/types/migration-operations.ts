...
export type MigrationCommandCapability = (typeof migrationCommandCapabilities)[number]
...
export function normalizeMigrationProgramDetail(payload: BillingMigrationProgramDetail) {
...
export function migrationRequiresDistinctApprover(command: "cutover" | "rollback" | "legal_hold") {
  return command === "cutover" || command === "rollback" || command === "legal_hold"
}

export function migrationCompletionBlockers(
  prerequisites: BillingMigrationCompletionPrerequisites | undefined,
): string[] {
  if (!prerequisites) return ["Completion evidence is unavailable."]
  if (prerequisites.eligible) return []
  const blockers: string[] = []
  if (!prerequisites.credentialRemoved) blockers.push("migration credential is still active")
  if (prerequisites.unresolvedCriticalBlocking > 0)
    blockers.push(`${prerequisites.unresolvedCriticalBlocking} critical or blocking cases remain`)
  if (!prerequisites.authorityStable) blockers.push("authority is not stable")
  if (!prerequisites.webhookReady) blockers.push("webhook delivery is not ready")
  ...
  return blockers.length ? blockers : ["Server-derived completion prerequisites are not satisfied."]
}

const TERMINAL_RUN_STATUSES = new Set(["completed", "failed", "cancelled"])

export function migrationRunIsTerminal(run: Pick<BillingMigrationRunJob, "status"> | undefined) {
...
export function migrationRunPollingInterval(
  ...
  return run && !migrationRunIsTerminal(run) && updateCount < MAX_RUN_POLL_UPDATES ? 4000 : false
}
...
export function migrationCommandJourney(
  ...
  return {
    canAssess: state === "shadowing" && canRunMigrationCommand(detail, "assess-readiness"),
    canCreateMapping: state === "mapping" && canRunMigrationCommand(detail, "manage-mappings"),
    canFreezeMapping: state === "mapping" && canRunMigrationCommand(detail, "manage-mappings"),
    canImport: state === "importing" && canRunMigrationCommand(detail, "run-import"),
    canQueueDryRun:
      ["importing", "dry_run"].includes(state) && canRunMigrationCommand(detail, "run-import"),
    canQueueShadow:
      ["dry_run", "shadowing"].includes(state) && canRunMigrationCommand(detail, "run-import"),
  }
}
...
export function dryRunAuthorityNotice(runKind: BillingMigrationRunJob["runKind"]) {
...
export function laterLifecycleNotice(program: Pick<BillingMigrationProgram, "state">) {
  return ["ready", "cutover_pending", "stabilizing", "completed", "rolled_back"].includes(
    program.state,
  )
...
export function migrationErrorCopy(
  error: unknown,
  command: "create" | "mapping" | "freeze" | "import" | "run" | "readiness" | "lifecycle",
) {
  ...
  return {
    ...
    import: "Mosaic could not queue this import batch. Review its manifest, mapping set, and size.",
    mapping: "Mosaic could not create this mapping set. Review every source and target.",
    readiness: "Mosaic could not create a readiness assessment from the current evidence.",
