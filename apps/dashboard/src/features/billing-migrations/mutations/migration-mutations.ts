...
type LifecycleCommand =
  ...
  | { kind: "approve_proposal"; proposalId: string; body: BillingMigrationStateVersionRequest }
  ...
  | { kind: "observe_stabilization"; body: BillingMigrationObserveStabilizationRequest }
  ...
  | { kind: "remove_credential"; body: BillingMigrationCredentialRemovalRequest }
  | { kind: "propose_legal_hold"; body: BillingMigrationLegalHoldProposalRequest }
  | {
    ...
    mutationFn: async ({
      ...
      switch (command.kind) {
        case "propose_cutover":
          return (await proposeBillingMigrationCutover({ ...common, body: command.body })).data.data
            .payload
        ...
        case "create_checkpoint":
          return (await createBillingMigrationCheckpoint({ ...common, body: command.body })).data
            .data.payload
        case "execute_cutover":
          return (await executeBillingMigrationCutover({ ...common, body: command.body })).data.data
            .payload
        case "propose_rollback":
          return (await proposeBillingMigrationRollback({ ...common, body: command.body })).data
            .data.payload
        case "execute_rollback":
          return (await executeBillingMigrationRollback({ ...common, body: command.body })).data
            .data.payload
        case "observe_stabilization":
          return (await observeBillingMigrationStabilization({ ...common, body: command.body }))
            .data.data.payload
        case "preview_repair":
          return (await previewBillingMigrationRepair({ ...common, body: command.body })).data.data
            .payload
        case "execute_repair":
          return (await executeBillingMigrationRepair({ ...common, body: command.body })).data.data
            .payload
        case "redeliver_webhook":
          return (await redeliverBillingMigrationWebhook({ ...common, body: command.body })).data
            .data.payload
        case "remove_credential":
          return (await removeBillingMigrationCredential({ ...common, body: command.body })).data
            .data.payload
        case "propose_legal_hold":
          return (await proposeBillingMigrationLegalHold({ ...common, body: command.body })).data
            .data.payload
        ...
        case "complete":
          return (await completeBillingMigration({ ...common, body: command.body })).data.data
            .payload
      }
    },
    onSettled: async () => {
...
async function refreshProgram(queryClient: QueryClient, projectId: string, programId?: string) {
  await queryClient.invalidateQueries({ queryKey: migrationKeys.project(projectId) })
  if (programId) {
    await queryClient.invalidateQueries({ queryKey: migrationKeys.program(projectId, programId) })
  }
}

export function createMigrationProgramMutationOptions(projectId: string, queryClient: QueryClient) {
  return mutationOptions({
    gcTime: 0,
    mutationFn: async ({
      body,
...
export function queueMigrationRunMutationOptions(
  ...
  return mutationOptions({
    mutationFn: async ({
      ...
      const operation =
        kind === "dry_run" ? queueBillingMigrationDryRun : queueBillingMigrationShadowRun
