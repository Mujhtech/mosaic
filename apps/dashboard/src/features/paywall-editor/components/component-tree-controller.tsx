...
export function useComponentTreeModel({
  ...
  const [collapsedScreenIds, setCollapsedScreenIds] = useState<
    ReadonlySet<string>
  >(() => new Set());
  ...
  const [catalogDropPreview, setCatalogDropPreview] =
    useState<CatalogDropPreview | null>(null);
  ...
  const rows = useMemo(
    () =>
      document
        ? visibleRows(document, expandedTreeNodes, collapsedScreenIds)
        : [],
  ...
  const issueSummaries = useMemo(() => {
    const documentSummary: LayerIssueSummary = {
      errorCount: 0,
      warningCount: 0,
    };
  ...
  const canMoveUp =
    structuralReason === null && boundaries?.canMovePrevious === true;
  const canMoveDown =
    structuralReason === null && boundaries?.canMoveNext === true;
  const canIndent =
    structuralReason === null && boundaries?.canIndentIntoPrevious === true;
  const canOutdent =
    structuralReason === null && boundaries?.canOutdent === true;
  ...
  function toggleScroll(screenId: string) {
    if (collapsedScreenIds.has(screenId)) {
      expandScroll(screenId);
    } else {
      collapseScroll(screenId);
    }
  }
  ...
  function addDestination(presentation: "screen" | "sheet") {
    ...
    const result = appendScreen(activeDocument, {
      presentation,
      sourceScreenId,
    });
  ...
  function reportOperation(result: TreeOperationResult, success: string) {
    if (result.status === "accepted") {
      setNotice({
        tone: "success",
        title: success,
        detail: "The change can be undone.",
      });
      return;
    }
    setNotice({
    ...
    setRenameDraft(
      layerMetadata.labels[id] ??
        componentLabel(
          node,
          new Set(
            activeDocument.screens.map((screen) => screen.layout.content.id)
          ),
          {}
        )
    );
  ...
  function commitRename() {
    ...
    if (nextLabel) {
      workspace.setLayerLabel(renameId, nextLabel);
    } else {
      workspace.removeLayerLabel(renameId);
    }
    setRenameId(null);
  }
  ...
  function dragOverTarget(
    event: DragEvent<HTMLDivElement>,
    key: string,
    target: TreeMoveTarget
  ) {
    const sourceId =
      dragSourceIdRef.current || event.dataTransfer.getData(LAYER_DRAG_TYPE);
    if (!sourceId) {
      return;
    }
    ...
    event.dataTransfer.dropEffect =
      result.status === "accepted" ? "move" : "none";
  }
  ...
  function dropOnTarget(
    ...
    const preview =
      dropPreview?.key === key
        ? dropPreview.result
        : previewDrop(key, sourceId, target);
  ...
  function catalogDropLocation(row: TreeRow): TreeInsertionLocation | null {
    ...
    return parent
      ? { parentId: parent.parent.id, index: parent.index + 1 }
      : null;
  }
  ...
  function layerDropTargetForRow(
    ...
    const position =
      bounds.height > 0 ? (clientY - bounds.top) / bounds.height : 0;
    if (
      (row.node.type === "stack" ||
        row.node.type === "button" ||
  ...
  function dragOverRow(event: DragEvent<HTMLDivElement>, row: TreeRow) {
    ...
    if (!hasLayerPayload(event)) {
      return;
    }
    const target = layerDropTargetForRow(
      event.clientY,
      row,
      event.currentTarget
    );
  ...
  function beginPointerLayerDrag(
    ...
    const startedFromControl =
      ...
      origin.closest("button, input, [role='menuitem']") !== null;
    if (!draggable || event.button !== 0 || startedFromControl) {
      return;
    }
  ...
  function movePointerLayerDrag(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const targetElement =
      hit?.closest<HTMLElement>("[data-layer-row-id]") ?? null;
    const targetRow = targetElement
      ? rowById.get(targetElement.dataset.layerRowId ?? "")
      : null;
  ...
  function finishPointerLayerDrag(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (preview?.result.status === "accepted") {
      reportOperation(
        editor.moveComponent(drag.sourceId, preview.target),
        "Layer moved"
      );
  ...
  function catalogDropBlock(
    ...
    if (isDocumentTransactionActive) {
      return {
        ...
        detail:
          "Commit or cancel the active text or property edit, then drop the component.",
      };
    }
    if (isMarked(activeDocument, location.parentId, lockedIds)) {
      return {
        ...
        detail:
          "Unlock the destination Stack in Layers, then try the drop again.",
      };
    }
    if (
      validatePayload &&
      !COMPONENT_CATALOG_BY_TYPE.has(type as InsertableBlockType)
    ) {
      return {
    ...
    if (
      validatePayload &&
      type === "countdown" &&
      !isValidCountdownInstant(countdownEndsAt)
    ) {
      return {
  ...
  function dragCatalogOverRow(event: DragEvent<HTMLDivElement>, row: TreeRow) {
    ...
    const targetLabel =
      row.kind === "scroll"
        ? "Scroll Container"
        : rowLabel(row, layerMetadata.labels);
    ...
    if (block) {
      setNotice({ tone: "danger", title: block.title, detail: block.detail });
    }
  }
  ...
  function dropCatalogOnRow(event: DragEvent<HTMLDivElement>, row: TreeRow) {
    ...
    if (
      block ||
      !location ||
      !COMPONENT_CATALOG_BY_TYPE.has(rawType as InsertableBlockType)
    ) {
      const failure = block ?? {
        title: "That component type is not supported.",
        detail: "Drag a Protocol 0.2 component from Add content.",
  ...
  function handleTreeKeyDown(
    ...
    const rowLocked =
      row.kind === "component" && isMarked(activeDocument, row.id, lockedIds);
    const rowParent =
      row.kind === "component" ? findParent(activeDocument, row.id) : null;
    ...
    switch (event.key) {
      ...
      case "ArrowRight":
        if (expandable && !expanded) {
          if (row.kind === "scroll") {
            expandScroll(row.screenId);
          } else {
            editor.toggleTreeNode(row.id);
          }
...
export type ComponentTreeModel = NonNullable<
  ReturnType<typeof useComponentTreeModel>
>;
