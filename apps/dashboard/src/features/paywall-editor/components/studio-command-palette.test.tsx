...
function PaletteHarness({
  ...
  const buttons = flattenDocument(document).filter(
    (entry) => entry.node.type === "button"
  );
  const countdowns = flattenDocument(document).filter(
    (entry) => entry.node.type === "countdown"
  );
  ...
  return (
    <>
      ...
      <output data-testid="button-actions">
        {buttons
          .map((entry) =>
            entry.node.type === "button" ? entry.node.action.type : ""
          )
          .join("|")}
      </output>
...
describe("StudioCommandPalette", () => {
  it("exposes accessible slugged groups, every supported component, and discoverable hints", () => {
    ...
    for (const entry of COMPONENT_CATALOG) {
      expect(
        screen.getByRole("button", { name: new RegExp(`Add ${entry.label}`) })
      ).toBeVisible();
    }
    ...
    expect(
      document.querySelector('[id="studio-command-group-Add content"]')
    ).toBeNull();
  });
  ...
  it("runs feature-owned navigation, workspace, appearance, import, and export actions", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Reopen command palette" })
    );
    fireEvent.click(
      screen.getByRole("button", { name: /Use dark canvas appearance/ })
    );
