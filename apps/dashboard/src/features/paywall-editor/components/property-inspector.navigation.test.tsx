...
describe("property inspector safety", () => {
  it("allows the former start destination to become a Sheet", async () => {
    ...
    const presentation = await screen.findByRole("combobox", {
      name: "Presentation",
    });
  ...
  it("exposes Image sizing, appearance, visibility, and accessibility", async () => {
    ...
    await waitFor(() =>
      expect(getInspectorSection("Content")).toBeInTheDocument()
    );
    ...
    expect(
      screen.getByRole("spinbutton", { name: "Aspect ratio" })
    ...
    expect(
      screen.getByRole("combobox", { name: "Width behaviour" })
    ).toBeVisible();
  });
  ...
  it("exposes production Protocol 0.2 text controls progressively", async () => {
    ...
    expect(
      screen.getByRole("group", { name: "Outer spacing" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Add accessibility label" })
    ).toBeInTheDocument();
  ...
  it("shows a contextual icon and ancestry breadcrumb without leaking machine IDs", async () => {
    ...
    const path = await screen.findByRole("navigation", {
      name: "Selected layer path",
    });
    expect(path).toHaveTextContent("Scroll Container/Content Stack/Text");
    expect(
      document.querySelector('[data-layer-type-icon="text"]')
    ).toBeInTheDocument();
    textRender.unmount();

    const stackRender = renderInspector("paywall-content");
    await waitFor(() =>
      expect(getInspectorSection("Layout")).toBeInTheDocument()
    );
    expect(
      document.querySelector('[data-inspector-section="Children"]')
    ).not.toBeInTheDocument();
    stackRender.unmount();

    const productRender = renderInspector("monthly-card");
    const productInspector = await screen.findByRole("region", {
      name: "Properties",
    });
    expect(
      within(productInspector).queryByText("monthly-plan")
    ).not.toBeInTheDocument();
    expect(
      within(productInspector).queryByText("yearly-plan")
    ).not.toBeInTheDocument();
    expect(
      within(productInspector).getByLabelText("Product")
  ...
  it("keeps structure in Layers while using group semantics for product sets", async () => {
    ...
    expect(
      screen.queryByRole("group", { name: "Child summary" })
    ).not.toBeInTheDocument();
    stackRender.unmount();

    renderInspector("plans");
    expect(
      await screen.findByRole("button", { name: "Add Product Card" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Edit Monthly Product Card" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "Edit Yearly Product Card (initial selection)",
      })
    ).toBeInTheDocument();
  });
  ...
  it("focuses canonical localized-default diagnostics on the editable content field", async () => {
    ...
    const issue = collectEditorValidation(invalidDocument).issues.find(
      (candidate) => candidate.documentPath.endsWith("/value/default")
    );
    if (!issue) {
      throw new Error("Missing canonical default diagnostic");
    }
  ...
  it("opens Advanced for canonical localization-key and feature-item-ID diagnostics", async () => {
    ...
    const keyIssue = collectEditorValidation(invalidTextDocument).issues.find(
      (candidate) =>
        candidate.code === "schema.pattern" &&
        candidate.componentId === "headline"
    );
    if (!keyIssue) {
      throw new Error("Missing canonical localization-key diagnostic");
    }
    ...
    await waitFor(() =>
      expect(getInspectorSection("Advanced")).toBeInTheDocument()
    );
    ...
    const itemIssue = collectEditorValidation(
      invalidFeatureDocument
    ).issues.find((candidate) => candidate.property === "items.InvalidItem.id");
    if (!itemIssue) {
      throw new Error("Missing canonical feature-item diagnostic");
    }
  ...
  it("focuses exact nested padding, accessibility, action, fallback, and feature fields", async () => {
    ...
    (plans.unavailableFallback as unknown as { selection: string }).selection =
      "invalid";
    await expectValidationIssueFocus({
  ...
  it("names a locked ancestor without exposing its component ID", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Unlock Content Stack" })
    );
    expect(screen.getByRole("textbox", { name: "Text" })).toBeEnabled();
  });
});
