...
export function ComponentTreeView({ model }: { model: ComponentTreeModel }) {
  const { addDestination, notice, rows } = model;
  ...
  return (
    <section aria-labelledby="component-tree-title">
      <div className="mb-3 flex items-start justify-between gap-2">
        <div>
          <h2 className="font-semibold text-sm" id="component-tree-title">
            Layers
          </h2>
      ...
      >
        {notice ? (
          <>
            ...
            <p className="mt-0.5 text-muted-foreground">{notice.detail}</p>
          </>
        ) : null}
      </StatusMessage>

      <div
        aria-label="Paywall component tree"
        className="space-y-0.5"
        role="tree"
      >
        {rows.map((row) => (
          <LayerTreeRow key={row.id} model={model} row={row} />
        ))}
      </div>
    </section>
  );
...
function LayerTreeRow({
  ...
  const expanded = (() => {
    ...
    if (isStack || isButton || isCarousel || isTabs || isProductContainer) {
  ...
  const incompatibleClientCount = capabilityRequirement
    ? previewClients.filter(
        (client) => !supportsCapability(client, capabilityRequirement)
      ).length
    : 0;
  ...
  const rowBoundaries = node
    ? getSiblingBoundaries(activeDocument, row.id)
    : null;
  const sourceRequiresChild =
    sourceParent?.collection !== "inProgressChildren" &&
    (Boolean(
      sourceParent && isRootContentId(activeDocument, sourceParent.parent.id)
    ) ||
  ...
  const layerActions: LayerActionItemsProps | null = node
    ? {
        availability: {
          delete: rowCanDelete,
          duplicate:
            !(immutable || effectivelyLocked) && node.type !== "productBadge",
          indent: rowCanIndent,
          moveDown:
            !(immutable || effectivelyLocked) &&
            rowBoundaries?.canMoveNext === true,
          moveUp:
            !(immutable || effectivelyLocked) &&
            rowBoundaries?.canMovePrevious === true,
  ...
  const dropAccepted =
    activeRowDrop && dropPreview.result.status === "accepted";
  const dropPlacement = activeRowDrop
    ? dropPreview?.target.placement
    : undefined;
    ...
    <div role="none">
      ...
      >
        <LayerRowTrigger
          ...
          expanded={expanded}
          hovered={hovered}
          incompatibleClientCount={incompatibleClientCount}
      ...
      {catalogDropPreview?.rowId === row.id ? (
        <div
          aria-live="polite"
          className={`my-1 rounded border px-2 py-1 font-medium text-[10px] ${
...
function LayerRowTrigger({
  ...
  activeRowDrop: boolean;
  capabilityRequirement: RequiredCapability | undefined;
  draggable: boolean;
  dropAccepted: boolean;
  dropPlacement: TreeMoveTarget["placement"] | undefined;
  effectivelyHidden: boolean;
  effectivelyLocked: boolean;
  expandable: boolean;
  expanded: boolean | undefined;
  hovered: boolean;
  incompatibleClientCount: number;
  issueSummary: LayerIssueSummary;
  label: string;
  layerActions: LayerActionItemsProps | null;
  model: ComponentTreeModel;
  ...
  return (
    <ContextMenuPrimitive.Trigger
      ...
      onDragLeave={(event) => {
        ...
        if (catalogDropPreview?.rowId === row.id) {
          setCatalogDropPreview(null);
        }
      }}
    ...
    >
      ...
      {expandable ? (
        <button
          ...
          className="grid size-6 shrink-0 place-items-center rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          onClick={(event) => {
            event.stopPropagation();
            if (isScroll) {
              toggleScroll(row.screenId);
            } else {
      ...
      <span className="grid size-5 shrink-0 place-items-center text-muted-foreground">
        <LayerTypeIcon type={node ? node.type : "scrollContainer"} />
      </span>
      {renameId === row.id && node ? (
        <input
          aria-label={`Rename ${label}`}
          className="min-w-0 flex-1 rounded border border-input bg-background px-1.5 py-1 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
