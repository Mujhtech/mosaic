      ...
      >
        {rows.map((row) => {
          ...
          const effectivelyLocked =
            !isScroll && isMarked(document, row.id, lockedIds);
          const directlyHidden = hiddenIds.has(row.id);
          const effectivelyHidden =
            !isScroll && isMarked(document, row.id, hiddenIds);
          ...
          const sourceRequiresChild =
            ...
            (Boolean(
              sourceParent &&
                isRootContentId(activeDocument, sourceParent.parent.id)
            ) ||
          ...
          const layerActions: LayerActionItemsProps | null = node
            ? {
                ...
                onIndent: () => {
                  editor.selectComponent(row.id);
                  reportOperation(
                    editor.indentSelectedComponent(),
                    "Layer indented"
                  );
                },
                onMoveDown: () => {
                ...
                onOutdent: () => {
                  editor.selectComponent(row.id);
                  reportOperation(
                    editor.outdentSelectedComponent(),
                    "Layer outdented"
                  );
                },
                onRename: () => {
                ...
                <ContextMenuPrimitive.Trigger
                  ...
                  onDragStart={(event) => {
                    ...
                    const startedFromControl =
                      ...
                      origin.closest("button, input") !== null;
                  ...
                  onPointerDown={(event) =>
                    beginPointerLayerDrag(event, row, draggable)
                  }
