...
function compatibleClient(): PreviewClient {
  return {
    ...
    supportedCapabilities: document.compatibility.requiredCapabilities.map(
      (capability) => ({
        ...capability,
      })
    ),
...
describe("preview compatibility", () => {
  it("requires exact schema and capability versions before presenting compatibility", () => {
    ...
    expect(
      compatibilityWarnings(document, [wrongDocumentVersion])
    ).toContainEqual(expect.stringContaining("cannot show every part"));
    ...
    expect(
      compatibilityWarnings(document, [wrongPreviewVersion])
    ).toContainEqual(
