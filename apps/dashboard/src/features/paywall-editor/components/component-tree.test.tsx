  ...
  return (
    <div>
      ...
      <output data-testid="document-nodes">
        {entries
          .map((entry) => `${entry.node.id}:${entry.node.type}`)
          .join("|")}
      </output>
      <output data-testid="document-revision">
      ...
      <output data-testid="tree-layout">
        {entries
          .map(
            (entry) =>
              `${entry.node.id}@${entry.parentId ?? "root"}:${entry.index}`
          )
          .join("|")}
      </output>
      ...
      <output data-testid="recent-insertions">
        {workspace.recentInsertions.join("|")}
      </output>
...
function createDataTransfer() {
  ...
  return {
    transfer: {
      ...
      getData: (type: string) =>
        payloadProtected ? "" : (values.get(type) ?? ""),
...
describe("ComponentTree", () => {
  it("adds a second screen from Layers and selects its editable starter content", async () => {
    renderTree();
    const addDestination = await screen.findByRole("button", {
      name: "Add screen or sheet",
    });
    const baselineUndo = Number(screen.getByTestId("undo-count").textContent);

    fireEvent.click(addDestination);
    fireEvent.click(
      await screen.findByRole("menuitem", { name: "Add screen" })
    );

    expect(await screen.findByTitle("Screen · screen-2")).toHaveAttribute(
      "aria-level",
      "1"
    );
    expect(
      document.querySelector('[data-layer-row-id="screen-2-title"]')
    ).toHaveAttribute("aria-level", "3");
    expect(screen.getByTestId("selected-id")).toHaveTextContent(
      "screen-2-title"
    );
    expect(screen.getByTestId("document-nodes")).toHaveTextContent(
      "screen-2-title:text"
    );
  ...
  it("adds a sheet from the focused screen without replacing the existing screen", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Add screen or sheet" })
    );
  ...
  it("keeps valid layers quiet and reserves status icons for exceptions", async () => {
    renderTree({
      previewClients: [previewClient("ios"), previewClient("android")],
    });
    const headline = await screen.findByTitle("Hero headline");

    expect(
      headline.querySelector('[data-slot="layer-status"]')
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText(/capability declared/i)
    ).not.toBeInTheDocument();
  });
  ...
  it("uses an error icon when a layer is invalid", async () => {
    ...
    expect(
      within(headline).getByLabelText(/validation errors?/)
  ...
  it("shows compatibility only where connected platform support differs", async () => {
    renderTree({
      previewClients: [
        previewClient("ios"),
        previewClient("android", "component.text"),
      ],
    });
    const headline = await screen.findByTitle("Hero headline");

    expect(
      within(headline).getByLabelText(
        "component.text support differs on 1 connected preview"
      )
    ).toHaveAttribute("data-slot", "layer-status");
    expect(
      screen
        .getByTitle("Product Selector")
        .querySelector('[data-slot="layer-status"]')
    ).not.toBeInTheDocument();
  });
  ...
  it("renders the Scroll root and nested content as an accessible, revealable ARIA tree", async () => {
    renderTree();
    await waitFor(() =>
      expect(screen.getByTitle("Hero headline")).toBeVisible()
    );
    ...
    expect(screen.getByTitle("Hero headline")).toHaveAttribute(
      "draggable",
      "true"
    );
    ...
    expect(
      screen.getByText("Drag anywhere on a layer row to reorder.")
    ).toBeVisible();
    expect(
      screen.queryByRole("toolbar", { name: "Selected layer actions" })
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /Move selected layer/ })
    ).not.toBeInTheDocument();
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Collapse Offer group" })
    );
  ...
  it("exposes Button in-progress children as selectable structural layers", async () => {
    ...
    fireEvent.click(
      within(purchase).getByRole("button", { name: "Expand Purchase" })
    );
  ...
  it("uses authored Product Cards and Badges as reorderable structural layers", async () => {
    ...
    fireEvent.click(
      within(selector).getByRole("button", { name: "Expand Product Selector" })
    );

    await waitFor(() =>
      expect(
        document.querySelector('[data-layer-row-id="monthly-card"]')
      ).toBeVisible()
    );
    const monthly = document.querySelector<HTMLElement>(
      '[data-layer-row-id="monthly-card"]'
    );
    const yearly = document.querySelector<HTMLElement>(
      '[data-layer-row-id="yearly-card"]'
    );
    expect(monthly).toHaveAttribute("aria-level", "4");
    expect(yearly).toHaveAttribute("aria-level", "4");
    if (!(monthly && yearly)) {
      throw new Error("Missing authored Product Card layers");
    }

    fireEvent.click(
      within(yearly).getByRole("button", { name: "Expand Product Card" })
    );
    expect(
      document.querySelector('[data-layer-row-id="yearly-badge"]')
  ...
  it("keeps hover synchronized and keyboard delete scoped to the focused row", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByTitle("Terms")).not.toBeInTheDocument()
    );
    expect(screen.getByTestId("document-nodes")).toHaveTextContent(
      "headline:text"
    );
    expect(screen.getByTestId("document-nodes")).not.toHaveTextContent(
      "legal:text"
    );
  });
  ...
  it("keeps duplicate and delete contextual while showing nest controls only when valid", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Actions for Hero headline" })
    );
    expect(
    ...
    expect(
      screen.queryByRole("menuitem", { name: "Indent" })
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("menuitem", { name: "Outdent" })
    ).not.toBeInTheDocument();
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Actions for Product Selector" })
    );
    expect(
      await screen.findByRole("menuitem", { name: "Indent" })
    ).toBeVisible();
    expect(
      screen.queryByRole("menuitem", { name: "Outdent" })
    ).not.toBeInTheDocument();
    ...
    expect(
      await screen.findByRole("menuitem", { name: "Outdent" })
    ).toBeVisible();
    expect(
      screen.queryByRole("menuitem", { name: "Indent" })
    ).not.toBeInTheDocument();
  ...
  it("opens the same layer actions from a native right click", async () => {
    ...
    const overflowActions = (await screen.findAllByRole("menuitem")).map(
    ...
    const contextActions = (await screen.findAllByRole("menuitem")).map(
      (item) => item.textContent?.trim()
    );
  ...
  it("keeps rename, lock, and hide as workspace-only metadata", async () => {
    ...
    const baselineRevision =
      screen.getByTestId("document-revision").textContent;
    const baselineUndo = screen.getByTestId("undo-count").textContent;
    const baselinePortable =
      screen.getByTestId("portable-document").textContent;
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Actions for Primary headline" })
    );
    fireEvent.click(
      await screen.findByRole("menuitem", { name: "Lock canvas layer" })
    );
    await waitFor(() =>
      expect(screen.getByTestId("locked-ids")).toHaveTextContent("headline")
    );
    ...
    fireEvent.click(
      await screen.findByRole("menuitem", { name: "Hide on canvas" })
    );
    await waitFor(() =>
      expect(screen.getByTestId("hidden-ids")).toHaveTextContent("headline")
    );
    expect(screen.getByTestId("document-revision")).toHaveTextContent(
      baselineRevision ?? ""
    );
    expect(screen.getByTestId("undo-count")).toHaveTextContent(
      baselineUndo ?? ""
    );
    expect(screen.getByTestId("portable-document").textContent).toBe(
      baselinePortable
    );
    expect(screen.getByTestId("portable-document")).not.toHaveTextContent(
      "Primary headline"
    );
    expect(screen.getByTestId("portable-document")).not.toHaveTextContent(
      "layerMetadata"
  ...
  it("shows the store rejection reason and recovery for an invalid native tree drop", async () => {
    ...
    expect(
      screen.getByText(
        "A component cannot be moved relative to or inside itself."
      )
    ).toBeVisible();
    expect(
      screen.getByText("Choose a different component or Stack as the target.")
    ).toBeVisible();
  });
  ...
  it("accepts a MIME-only generic Button payload on drop", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTitle("Content Stack")).toBeVisible()
    );
    const transfer = createDataTransfer();
    fireEvent.dragStart(screen.getByRole("button", { name: /^Button\b/i }), {
      dataTransfer: transfer.transfer,
    });
    expect(transfer.transfer.getData(COMPONENT_LIBRARY_DRAG_TYPE)).toBe(
      "button"
    );

    transfer.protect();
    fireEvent.dragOver(screen.getByTitle("Content Stack"), {
      dataTransfer: transfer.transfer,
    });
    expect(screen.getByText("Drop inside Content Stack")).toBeVisible();

    transfer.reveal();
    fireEvent.drop(screen.getByTitle("Content Stack"), {
      dataTransfer: transfer.transfer,
    });
    ...
    expect(
      screen.getByTestId("document-nodes").textContent?.match(/:button/g)
    ).toHaveLength(4);
  });
  ...
  it("carries an explicit Countdown deadline through a whole-row Layers drop", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: /^Countdown\b/i })
    ...
    expect(
      transfer.transfer.getData(COMPONENT_LIBRARY_COUNTDOWN_ENDS_AT_DRAG_TYPE)
    ).toBe("2029-05-06T07:08:00Z");
    ...
    expect(screen.getByTestId("portable-document")).toHaveTextContent(
