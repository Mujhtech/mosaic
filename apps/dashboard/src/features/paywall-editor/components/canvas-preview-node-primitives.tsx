...
import {
  fillAxisIsBounded,
  fixedSizingClipsOverflow,
} from "@/features/paywall-editor/utils/sizing";
import { interpolateProductText } from "@/lib/mosaic-protocol";

const PRICE_TOKEN = /\{\{\s*product\.price\s*\}\}/;
...
export function frameStyle(
  ...
  const badgePlacement =
    node.type === "productBadge" ? node.placement : undefined;
  return {
    ...
    ...(badgePlacement?.mode === "overlay"
      ? {
          ...
          insetBlockStart: badgePlacement.anchor.startsWith("top")
            ? badgePlacement.inset
            : undefined,
          insetInlineEnd: badgePlacement.anchor.endsWith("End")
            ? badgePlacement.inset
            : undefined,
...
export function resolveProductTemplate(
  value: string,
  product: PreviewProductContext | undefined
) {
  if (!product) {
    return value;
  }
...
export function productCardRequiresPrice(
  ...
  const usesPrice = (value: Parameters<typeof resolveLocalizedText>[1]) =>
    PRICE_TOKEN.test(resolveLocalizedText(document, value, locale));
  if (card.accessibility && usesPrice(card.accessibility.label)) {
    return true;
  }

  function nodeRequiresPrice(node: ProtocolNode): boolean {
    if (node.type === "text") {
      return usesPrice(node.value);
    }
...
interface PreviewAppearance {
  background?: NonNullable<
    Extract<ProtocolNode, { type: "text" }>["appearance"]
  >["background"];
  border?: NonNullable<
    Extract<ProtocolNode, { type: "text" }>["appearance"]
  >["border"];
...
export function subtreeIncludesId(
  node: ProtocolNode,
  id: string | null
): boolean {
  if (!id) {
  ...
  if (node.type === "button") {
    return [...node.children, ...(node.inProgressChildren ?? [])].some(
      (child) => subtreeIncludesId(child, id)
    );
  }
...
export function headingElement(
  ...
  return `h${Math.min(6, Math.max(1, level))}` as
    | "h1"
    | "h2"
    | "h3"
    | "h4"
...
export function countdownText(
  ...
  const first = COUNTDOWN_UNITS.findIndex(
    ([unit]) => unit === node.largestUnit
  );
  const last = COUNTDOWN_UNITS.findIndex(
    ([unit]) => unit === node.smallestUnit
  );
