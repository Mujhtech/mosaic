...
function InitializePreview({ mode }: { mode: MetadataMode }) {
  ...
  useLayoutEffect(() => {
    ...
    if (mode === "hide-headline") {
      workspace.setLayerCanvasHidden("headline", true);
    }
    if (mode === "hide-root") {
...
function PreviewProbe() {
  ...
  return (
    <div>
      <output data-testid="preview-hovered">
        {hoveredComponentId ?? "none"}
      </output>
      <output data-testid="preview-selected">
      ...
      <output data-testid="preview-document-json">
        {document && JSON.stringify(document).includes("countdownPreviewAt")
          ? "leaked"
          : "clean"}
      </output>
...
describe("PreviewCanvas layer metadata", () => {
  it("blocks canvas selection and inline editing for locked layers while preserving hover sync", async () => {
    ...
    const headline = view.container.querySelector<HTMLElement>(
      '[data-component-id="headline"]'
    );
    expect(headline).not.toBeNull();
    expect(headline).toHaveAttribute("aria-disabled", "true");

    fireEvent.click(heading);
    expect(screen.getByTestId("preview-selected")).toHaveTextContent(
      "subtitle"
    );
  ...
  it("uses protocol-appropriate HTML and keeps structural editing out of the canvas", async () => {
    ...
    const body = screen.getByText(
      /Edit once and preview the same native document/
    );
    const productSelector = screen.getByRole("group", {
      name: "Choose a plan",
    });
    ...
    const imageFrame = view.container.querySelector<HTMLElement>(
      '[data-preview-node-type="image"]'
    );
    ...
    expect(
      purchase.closest('[data-component-id="purchase"]')
    ).toHaveTextContent("Continue");
    expect(purchase.querySelector("div, textarea")).toBeNull();
    expect(
      screen.getByRole("button", { name: "Restore previous purchases" })
    ).toBeVisible();
    ...
    const previewFrames = view.container.querySelectorAll<HTMLElement>(
    ...
    expect(screen.getByTestId("preview-undo")).toHaveTextContent(
      undoCount ?? ""
    );
  ...
  it("keeps name-only Product Cards available when the provider price is blank", async () => {
    ...
    expect(
      screen.queryByRole("radio", { name: /Yearly/ })
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Continue with selected plan" })
    ).toBeEnabled();
  });
  ...
  it("previews a Product Card state without changing runtime selection or document history", async () => {
    ...
    expect(screen.getByTestId("preview-product-state")).toHaveTextContent(
  ...
  it("previews Button in-progress children when their layer is selected", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Preview purchase progress" })
    );
  ...
  it("mirrors directional Icon glyphs in RTL preview", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Open preview settings" })
    );
  ...
  it("removes hidden layers and explains a hidden root Stack without mutating the document", async () => {
    ...
    expect(
      screen.getByText(/Edit once and preview the same native document/).tagName
    ).toBe("P");
    expect(screen.getByTestId("preview-undo")).toHaveTextContent("0");
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Content Stack is hidden on the canvas/)
      ).toBeVisible()
    );
    expect(
      screen.queryByRole("heading", {
  ...
  it("accepts a generic Button drop after MIME-only dragover", async () => {
    ...
    transfer.protect();
    fireEvent.dragOver(
      screen.getByRole("region", { name: "Browser editing preview" }),
      {
        dataTransfer: transfer.transfer,
      }
    );
    expect(
      screen.getByText("Drop to insert at the current Layers selection.")
    ).toBeVisible();
    ...
    await waitFor(() =>
      expect(screen.getByTestId("preview-buttons")).toHaveTextContent("4")
    );
  ...
  it("keeps workspace device, orientation, zoom, context, and appearance outside document history", async () => {
    ...
    expect(
      device?.querySelector('[data-device-sensor="dynamic-island"]')
    ).not.toBeNull();
    expect(device?.querySelector("[data-device-camera-lens]")).not.toBeNull();
    expect(
      device?.querySelector('[data-device-drag-surface="status-bar"]')
    ).toBeNull();
    expect(screen.getByTestId("device-name-label")).toHaveTextContent(
      "iPhone 17 Pro"
    );
    expect(screen.getByTestId("device-size-label")).toHaveTextContent(
      '6.3" · 402 × 874 pt'
    );
    expect(
      device?.querySelector('[data-system-icon="ios-cellular"]')
    ).not.toBeNull();
    expect(
      device?.querySelector('[data-system-icon="ios-wifi"]')
    ).not.toBeNull();
    expect(
      device?.querySelector('[data-system-icon="ios-battery"]')
    ).toHaveAttribute("data-battery-percent", "82");
    ...
    expect(screen.getByTestId("device-status-bar")).toHaveStyle({
      paddingInline: "35px",
    });
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Use landscape orientation" })
    );
    ...
    expect(
      device?.querySelector('[data-device-sensor="punch-hole"]')
    ).not.toBeNull();
    expect(
      device?.querySelector('[data-system-icon="ios-cellular"]')
    ).toBeNull();
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Fit device to canvas" })
    );
    ...
    await chooseSelectOption(
      screen.getByRole("combobox", { name: "Preview locale" }),
      /^ar · /
    );
    await waitFor(() =>
      expect(screen.getByTestId("preview-locale")).toHaveTextContent("ar")
    );
    fireEvent.change(
      screen.getByRole("slider", { name: /Preview text scale/i }),
      {
        target: { value: "1.5" },
      }
    );
    await waitFor(() =>
      expect(screen.getByTestId("preview-text-scale")).toHaveTextContent("1.5")
    );
  ...
  it("restores a screen frame position from workspace preferences without document history", async () => {
    ...
    await waitFor(() =>
      expect(frame.style.transform).toContain("translate(123.5px,-45.25px)")
    );
    expect(screen.getByTestId("preview-undo")).toHaveTextContent("0");
    expect(screen.getByTestId("preview-document-json")).toHaveTextContent(
      "clean"
    );
  });
  ...
  it("previews Countdown from frozen workspace time and advances to completion without mutation", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Advance Countdown preview by 1 day" })
    );
  ...
  it("uses a double-click transaction for inline edits and cancels without history", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Final inline headline" })
    ).toBeVisible();
    expect(screen.getByTestId("preview-undo")).toHaveTextContent("1");
    ...
    expect(
      screen.getByRole("heading", { name: "Build a paywall people understand" })
    ).toBeVisible();
  });
  ...
  it("keeps the device and preview frames non-draggable and navigates layers by keyboard", async () => {
    ...
    const device =
      view.container.querySelector<HTMLElement>("[data-device-id]");
    ...
    expect(screen.getByTestId("preview-selected")).not.toHaveTextContent(
      "none"
    );
  });
});
...
describe("Canvas geometry", () => {
  it("keeps device and orientation independent and clamps fit to the 20–200% workspace range", () => {
    expect(
      resolveCanvasDeviceGeometry("ipad-pro-11", "portrait")
    ).toMatchObject({
      height: 1210,
      width: 834,
    });
    expect(
      resolveCanvasDeviceGeometry("ipad-pro-11", "landscape")
    ).toMatchObject({
      height: 834,
      width: 1210,
    });
