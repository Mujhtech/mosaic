...
export function ComponentLibrary() {
  ...
  const { layerMetadata, recentInsertions } = useStudioWorkspaceSelector(
    selectLibraryWorkspace
  );
  ...
  const countdownEndsAt = countdownInstantFromLocalInput(
    countdownDeadlineInput
  );
  ...
  function insert(type: InsertableBlockType) {
    ...
    reportResult(
      editor.insertComponentAt(
        ...
        insertionLocation,
        type === "countdown"
          ? { countdownEndsAt: countdownEndsAt ?? undefined }
          : undefined
      ),
      type,
      label
  ...
  return (
    <section aria-labelledby="component-library-title" className="space-y-4">
      <div>
        <h2 className="font-semibold text-sm" id="component-library-title">
          Add content
        </h2>
        <p className="mt-0.5 text-muted-foreground text-xs leading-5">
          Insert only protocol-supported blocks into the selected Stack
          position.
        </p>
      </div>
      <div>
        ...
        <div className="relative">
          ...
          />
          <input
            className="w-full rounded border border-input bg-background py-2 pr-3 pl-8 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            id="component-search"
      ...
      {recentInsertions.length > 0 ? (
        <div>
          <h3 className="mb-1.5 font-semibold text-muted-foreground text-xs">
            Recent
          </h3>
      ...
      {filteredCategories.length > 0 ? (
        <div className="space-y-4">
          {filteredCategories.map((category) => (
            <section
              aria-labelledby={`component-category-${category.label}`}
              key={category.label}
            >
              <h3
                className="mb-1.5 font-semibold text-muted-foreground text-xs"
        ...
        <div className="rounded border border-border border-dashed p-4 text-center">
          <p className="font-medium text-sm">No supported components match</p>
      ...
      <div className="rounded border border-border bg-muted/35 p-3">
        <p className="font-semibold text-xs">Insertion target</p>
        <p className="mt-1 text-muted-foreground text-xs leading-4">
          {insertionPreview(
            document,
            selectedComponentId,
            layerMetadata.labels
          )}
        </p>
      </div>
      ...
      {selectedType === "countdown" ? (
        <div className="space-y-1.5">
          ...
          <input
            ...
            className="w-full rounded border border-input bg-background px-2 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          ...
          >
            Required before insertion. Expired deadlines are valid and preview
            the completed state.
          </p>
        </div>
      ) : null}
      ...
      <p className="text-[11px] text-muted-foreground leading-4">
        Double-click a component or focus it and press Enter to insert.
        Component drags carry the same protocol type and configured Countdown
        deadline.
      </p>
    </section>
  );
