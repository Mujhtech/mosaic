...
describe("Studio automated workflow", () => {
  ...
  it("completes the first local editing journey and restores document plus workspace state", async () => {
    ...
    fireEvent.change(inlineEditor, {
      target: { value: "Workflow-ready headline" },
    });
    fireEvent.keyDown(inlineEditor, { key: "Enter" });
    expect(
      screen.getByRole("heading", { name: "Workflow-ready headline" })
    ).toBeVisible();
    ...
    fireEvent.click(
      within(componentPanel).getByRole("button", { name: "Text" })
    );
    fireEvent.click(
      within(componentPanel).getByRole("button", { name: "Insert Text" })
    );
    expect(within(componentPanel).getByText("Text inserted")).toBeVisible();

    fireEvent.click(screen.getByRole("button", { name: "Layers" }));
    const layersPanel = await screen.findByRole("complementary", {
      name: "Layers",
    });
    ...
    fireEvent.click(
      within(canvas).getByRole("group", { name: "Choose a plan" })
    );
    fireEvent.click(
      screen.getByRole("button", { name: "Edit Monthly Product Card" })
    );
    const initialProduct = screen.getByRole("checkbox", {
    ...
    const device = canvas.querySelector<HTMLElement>(
      '[data-device-id="pixel-10-pro"]'
    );
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      "ar is missing paywall.headline"
    );
    ...
    await waitFor(
      () =>
        expect(
          window.localStorage.getItem(LOCAL_PROJECT_STORAGE_KEY)
        ).toContain("عنوان جاهز للنشر"),
      { timeout: 1500 }
    );
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Resume autosave" })
    );
    await screen.findByTestId("studio-editor-shell");

    expect(
      screen.getByRole("combobox", { name: "Preview device" })
    ).toHaveTextContent("Pixel 10 Pro");
    fireEvent.click(
      screen.getByRole("button", { name: "Open preview settings" })
    );
    expect(
    ...
    const restoredCanvas = screen.getByRole("region", {
      name: "Browser editing preview",
    });
    expect(
