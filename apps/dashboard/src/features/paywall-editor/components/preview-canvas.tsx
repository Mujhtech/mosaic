...
const DevicePreviewFlowNode = memo(function DevicePreviewFlowNode({
  ...
  const zoom =
    data.canvas.fitMode === "manual" ? data.canvas.zoom : viewport.zoom;
  return (
    <>
      <Handle
...
function safeFramePosition(
  position: StudioCanvasFramePosition
): StudioCanvasFramePosition | null {
...
export function PreviewCanvas({
  ...
  const [transientFramePositions, setTransientFramePositions] =
    useState<TransientFramePositions>(() => ({
      workspaceBase: framePositions,
      positions: {},
    }));
  ...
  const preset = useMemo(
    () => getCanvasDevicePreset(canvas.device),
    [canvas.device]
  );
  ...
  const syncViewport = useCallback(
    (flow: ReactFlowInstance<CanvasDeviceFlowNode>, duration: number) => {
      if (canvas.fitMode === "fit") {
        flow.fitView(fitViewOptions(duration));
        return;
      }
      flow.zoomTo(canvas.zoom, { duration });
    },
  ...
  const activeScreen =
    screenContainingNode(document, selectedComponentId) ??
  ...
  function handleCatalogDragOver(event: DragEvent<HTMLElement>) {
    if (!hasPayload(event, COMPONENT_LIBRARY_DRAG_TYPE)) {
      return;
    }
  ...
  function handleCatalogDrop(event: DragEvent<HTMLElement>) {
    ...
    if (type === "countdown" && !isValidCountdownInstant(countdownEndsAt)) {
      setDropNotice({
        tone: "danger",
        message:
          "Countdown needs an explicit valid UTC deadline from Add content.",
      });
      return;
    }
  ...
  function beginInlineEdit(node: ProtocolNode) {
    if (!getEditableCanvasText(node) || editingComponentId === node.id) {
      return;
    }
  ...
  function handleCanvasKeyboard(event: KeyboardEvent<HTMLElement>) {
    if (event.target !== event.currentTarget || isFormControl(event.target)) {
      return;
    }
    ...
    event.preventDefault();
    const currentIndex = visibleSelectableIds.indexOf(
      selectedComponentId ?? ""
    );
  ...
  function handleNodeChanges(changes: NodeChange<CanvasDeviceFlowNode>[]) {
    const positionChanges = changes.filter(
      (
        ...
        NodeChange<CanvasDeviceFlowNode>,
        { type: "position" }
    ...
    setTransientFramePositions((current) => {
      ...
      positionChanges.forEach((change) => {
        ...
        const position = change.position
          ? safeFramePosition(change.position)
          : null;
  ...
  function handleMoveEnd(
    event: globalThis.MouseEvent | TouchEvent | null,
    viewport: Viewport
  ) {
    if (!event) {
      return;
    ...
    workspace.setCanvasPreference(
      ...
      Math.min(
        ...
        Math.max(
          ...
          Number(viewport.zoom.toFixed(2))
        )
      )
    );
  }
  ...
  const sharedNodeProps = {
    ...
    onCarouselPageChange: (id: string, index: number) =>
      setRuntime((current) => {
        ...
        return {
          ...base,
          carouselPages: { ...base.carouselPages, [id]: index },
        };
      }),
    ...
    onProductSelect: (id: string, productCardId: string) =>
      setRuntime((current) => {
        ...
        return {
          ...base,
          selectedProducts: { ...base.selectedProducts, [id]: productCardId },
        };
      }),
  ...
  const selectedScreen =
    screenContainingNode(document, selectedComponentId) ?? activeScreen;
  const columnGap = nodeGeometry.width + 140;
  const rowGap = nodeGeometry.height + 120;
  const columnCount = Math.max(
    1,
    Math.ceil(Math.sqrt(document.screens.length))
  );
  ...
  const deviceNodes: CanvasDeviceFlowNode[] = document.screens.map(
    (screen, index) => {
      ...
      return {
        ...
        type: "device-preview",
        position:
          devicePositions[screen.id] ??
          framePositions[screen.id] ??
          fallbackPosition,
        data: {
          ...
          onRootClick: (event) => {
            if (
              ...
              !lockedIds.has(screen.layout.content.id)
            ) {
  ...
  const navigationEdges: Edge[] = flattenDocument(document).flatMap(
    ({ node }) => {
      if (node.type !== "button" || node.action.type !== "navigateTo") {
        return [];
      }
      const sourceScreen = screenContainingNode(document, node.id);
      const source = sourceScreen
        ? deviceNodeIdByScreenId.get(sourceScreen.id)
        : undefined;
      ...
      >
        <ReactFlow<CanvasDeviceFlowNode>
          ...
          onNodeDragStop={(_, node) => {
            ...
            if (screenId && position) {
              workspace.setFramePosition(screenId, position);
            }
          }}
        ...
        >
          ...
          {dropNotice ? (
            <Panel position="top-center">
              <div
                aria-live={
                  dropNotice.tone === "danger" ? "assertive" : "polite"
                }
                className={`rounded-full border px-3 py-1.5 font-medium text-xs shadow-sm backdrop-blur ${
