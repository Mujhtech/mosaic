...
function previewClient(
  ...
  return {
    ...
    supportedCapabilities: capabilities
      ? document.compatibility.requiredCapabilities.map((capability) => ({
          ...capability,
        }))
      : [],
...
describe("PreviewConnectionPanel", () => {
  it("keeps the empty state actionable and reports all three native platforms", () => {
    ...
    expect(
      within(panel).getByText(/npm run dev:studio/, { selector: "code" })
    ).toBeVisible();
    ...
    expect(
      within(panel).getByRole("heading", { name: "Capabilities" })
    ).toBeVisible();
    expect(
      within(panel).getByRole("heading", { name: "Last diagnostic" })
    ).toBeVisible();
    expect(
      within(panel).getByText("No preview diagnostics yet.")
    ).toBeVisible();
    expect(within(panel).getAllByText("Reconnecting").length).toBeGreaterThan(
      0
    );
  });
  ...
  it("separates connected, handshaking, and disconnected platform status", () => {
    renderPanel({
      clients: [
        previewClient("flutter"),
        previewClient("ios", { capabilities: false }),
      ],
    });
    ...
    if (!(flutterRow && iosRow && androidRow)) {
      throw new Error("Expected all platform rows");
    }
    ...
    expect(
      screen.getByText(/Flutter preview · \d+ protocol · \d+ preview/)
    ).toBeVisible();
  ...
  it("shows the latest diagnostic and exposes reconnect as a recovery action", () => {
    ...
    expect(
      screen.getByText("Flutter used the component fallback.")
    ).toBeVisible();
    expect(
      screen.getByText(
        "Replace the unsupported component, then resend the draft."
      )
    ).toBeVisible();
