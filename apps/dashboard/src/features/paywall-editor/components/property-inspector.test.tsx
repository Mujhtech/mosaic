...
describe("property inspector safety", () => {
  it("keeps product binding contextual to the selected Product Card", async () => {
    ...
    expect(
      await screen.findByRole("option", { name: "Yearly" })
  ...
  it("edits Product Card Default and Selected states with explicit inheritance", async () => {
    ...
    fireEvent.click(
      card.getByRole("button", { name: "Reset selected appearance" })
    );
    ...
    fireEvent.click(
      card.getByRole("button", { name: "Reset fill to Default" })
    );
    expect(card.getByLabelText("Fill")).toHaveValue("surface.default");
    expect(screen.getByTestId("selected-card-style")).toHaveTextContent("{}");
  ...
  it("applies a colour token from the palette as one undoable editor change", async () => {
    ...
    await waitFor(() =>
      expect(getInspectorSection("Background")).toBeInTheDocument()
    );
    openInspectorSection("Background");

    expect(screen.getByLabelText("Background")).toHaveValue("transparent");
    fireEvent.click(
      screen.getByRole("button", { name: "Open Background colour picker" })
    );
    const palette = await screen.findByRole("dialog", {
      name: "Background colour",
    });
    fireEvent.click(
      within(palette).getByRole("button", { name: "Action primary" })
    );
  ...
  it("exposes Product Badge content alongside placement, layout, and state styling", async () => {
    ...
    await waitFor(() =>
      expect(getInspectorSection("Badge")).toBeInTheDocument()
    );
    ...
    expect(
      content.getByRole("button", { name: "Edit Text in Product Badge" })
    ).toBeVisible();
    expect(
      content.getByText(/product variables work inside Text/)
    ).toBeVisible();
  });
  ...
  it("authors Product Card sizing, including fixed width", async () => {
    ...
    await chooseSelectOption(
      size.getByRole("combobox", { name: "Width behaviour" }),
      "Fixed"
    );

    expect(size.getByRole("spinbutton", { name: "Fixed width" })).toHaveValue(
      320
    );
    ...
    expect(
      size.getByRole("combobox", { name: "Width behaviour" })
    ).toHaveAccessibleName("Width behaviour");
  ...
  it("keeps media background selection valid with inline asset recovery", () => {
    ...
    expect(
      screen.getByRole("group", { name: "Background type" })
    ).toBeVisible();
