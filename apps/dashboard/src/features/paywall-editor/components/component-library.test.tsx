...
function LibraryProbe() {
  ...
  const countdowns = nodes.flatMap((node) =>
    node.type === "countdown" ? [node.endsAt] : []
  );

  return (
    <div>
      <output data-testid="node-types">
        {nodes.map((node) => node.type).join("|")}
      </output>
      <output data-testid="buttons">
        {buttons.map((node) => node.id).join("|")}
      </output>
      <output data-testid="countdowns">{countdowns.join("|")}</output>
      <output data-testid="recent-insertions">
        {recentInsertions.join("|")}
      </output>
...
describe("ComponentLibrary", () => {
  it("inserts the keyboard-targeted type and records that exact type as recent", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("recent-insertions")).toHaveTextContent("image")
    );
  ...
  it("requires a valid explicit Countdown deadline before insertion or dragging", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Insert Countdown" })
    ).toBeDisabled();
    ...
    expect(
      transfer.getData(COMPONENT_LIBRARY_COUNTDOWN_ENDS_AT_DRAG_TYPE)
    ).toBe("2000-01-01T00:00:00Z");

    fireEvent.click(screen.getByRole("button", { name: "Insert Countdown" }));
    await waitFor(() =>
      expect(screen.getByTestId("countdowns")).toHaveTextContent(
  ...
  it("emits exact protocol component types in drag payloads", async () => {
    ...
    fireEvent.dragStart(catalogCard("Button"), {
      dataTransfer: buttonTransfer,
    });
