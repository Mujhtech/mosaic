...
describe("StudioResizableWorkspace", () => {
  ...
  it("uses explicit unit props and imports the resizable implementation only through its wrapper", () => {
    ...
    expect(screen.getByTestId("studio-left-panel")).toHaveAttribute(
      "data-default-size",
      "300px"
    );
  ...
  it("uses the activity rail to collapse, expand, and select another tool", async () => {
    ...
    await waitFor(() =>
      expect(layers).toHaveAttribute(
        "title",
        "Layers — expand tool panel (G then L)"
      )
    );
    expect(JSON.parse(memory.read() ?? "null").panels.left.collapsed).toBe(
      true
    );
  ...
  it("keeps stable slot identities from rerendering during workspace-only updates", async () => {
    const recordRender =
      vi.fn<(name: "left" | "canvas" | "properties" | "diagnostics") => void>();

    function Slot({
      name,
    }: {
      name: "left" | "canvas" | "properties" | "diagnostics";
    }) {
      recordRender(name);
  ...
  it("keeps properties outside the compact group while retaining diagnostics persistence", () => {
    ...
    expect(
      screen.queryByTestId("studio-properties-panel")
  ...
  it("persists canonical upstream double-click results without intercepting the separator event", async () => {
    ...
    const adapterStore = createStudioWorkspaceStore({
      storage: adapterMemory.storage,
    });
    ...
    fireEvent.doubleClick(
      screen.getByRole("separator", { name: "Resize Studio tools and canvas" })
    );
    await waitFor(() =>
      expect(memory.storage.setItem).toHaveBeenCalledTimes(1)
    );
  ...
  it("provides compact properties through a Sheet and keeps the desktop-required safe state", async () => {
    ...
    expect(
      within(
        screen.getByRole("region", { name: "Studio requires a larger screen" })
      ).getByRole("button", { name: "Export local draft" })
    ).toBeVisible();
