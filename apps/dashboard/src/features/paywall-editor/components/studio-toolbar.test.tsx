...
function toolbarProps(
  overrides: Partial<StudioToolbarProps> = {}
): StudioToolbarProps {
  return {
...
describe("StudioToolbar", () => {
  it("humanizes a read-only document identity and exposes the dedicated Studio actions", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Focused offer v2" })
    ).toBeVisible();
    expect(screen.getByText("Saved locally")).toBeVisible();
    expect(screen.getByText(/Preview clients/)).toHaveTextContent(
      "Preview clients · 2"
    );
    expect(
      screen.getByRole("button", { name: "Open connected previews" })
    ).toHaveAttribute("aria-controls", "connected-preview-panel");
    expect(
      screen.getByRole("button", { name: "Open connected previews" })
    ).toHaveAttribute("title", "2 of 2 previews updated");
    ...
    expect(
      screen.queryByRole("button", { name: "Toggle diagnostics" })
    ).not.toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByRole("link", { name: "Connect to hosted" })
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("button", { name: "Open Studio commands" })
    ).not.toBeInTheDocument();
  });
  ...
  it("preserves local work before opening the optional hosted connection flow", () => {
    ...
    const blockedEvent = new MouseEvent("click", {
      bubbles: true,
      cancelable: true,
    });
  ...
  it("uses stable fallback text for blank identities", () => {
    const { rerender } = render(
      <StudioToolbar {...toolbarProps({ documentIdentity: "   " })} />
    );
    expect(
      screen.getByRole("heading", { name: "Untitled paywall" })
    ).toBeVisible();

    rerender(
      <StudioToolbar
        {...toolbarProps({ documentIdentity: "benefitsFirst-paywall" })}
      />
    );
    expect(
      screen.getByRole("heading", { name: "Benefits First paywall" })
    ).toBeVisible();
  });
