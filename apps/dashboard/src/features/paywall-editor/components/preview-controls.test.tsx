...
function Harness() {
  ...
  return (
    <>
      ...
      <output data-testid="preview-document-revision">
...
describe("preview controls", () => {
  ...
  it("changes the portable default locale in one history step without corrupting defaults", async () => {
    ...
    await chooseSelectOption(
      screen.getByRole("combobox", { name: "Default locale" }),
      "de"
    );
    expect(screen.getByTestId("document-default-locale")).toHaveTextContent(
      "de"
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Undo localization change" })
    );
    expect(screen.getByTestId("document-default-locale")).toHaveTextContent(
      "en"
    );
  ...
  it("keeps a frozen Countdown instant in workspace state and advances it without document history", () => {
    ...
    const revision = screen.getByTestId(
    ...
    expect(screen.getByTestId("preview-document-revision")).toHaveTextContent(
      revision ?? ""
    );
    expect(screen.getByTestId("preview-undo-count")).toHaveTextContent(
      undoCount ?? ""
    );
