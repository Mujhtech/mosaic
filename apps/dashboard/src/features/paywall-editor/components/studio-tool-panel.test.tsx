...
function ToolPanelProbe() {
  ...
  const featureCount = document
    ? flattenDocument(document).filter(
  ...
  return (
    <div>
      <button onClick={editor.undo} type="button">
      ...
      <output data-testid="tool-document-json">
...
function renderTool(
  tool: StudioTool,
  viewportMode: StudioViewportMode = "large"
) {
...
describe("StudioToolPanel", () => {
  it("keeps structure in Layers and the insertion path in Components", async () => {
    ...
    expect(
      screen.getByRole("tree", { name: "Paywall component tree" })
    ).toBeVisible();
    ...
    expect(
      screen.queryByRole("button", { name: "Insert Text" })
  ...
  it("presents the exact searchable Add content catalog", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "Insert Text" })).toBeEnabled()
    );
    expect(screen.getByRole("heading", { name: "Add content" })).toBeVisible();

    const commerce = screen
      .getByRole("heading", { name: "Commerce" })
      .closest("section");
  ...
  it("authors a reusable design-system colour as an undoable document change", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Add colours style" })
    );

    expect(
      screen.getByRole("textbox", { name: "Name for Colour 1" })
    ).toHaveValue("Colour 1");
  ...
  it("keeps one compact design-system editor open and authors valid gradient stops", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Add backgrounds style" })
    );

    expect(
      screen.queryByRole("textbox", { name: "Name for Colour 1" })
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: "Name for Background 1" })
    ).toBeVisible();
    ...
    expect(screen.getByRole("spinbutton", { name: "Angle" })).toHaveAttribute(
      "min",
      "0"
    );
  });
  ...
  it("recovers a media design style by creating and selecting a valid asset", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Add backgrounds style" })
    );
    expect(
      screen.getByRole("combobox", { name: "Background kind" })
    ).toHaveAccessibleName("Background kind");
  ...
  it("confirms and applies a template in place as one undoable document edit", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Use Benefits first" })
      ).toBeEnabled()
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Undo template replacement" })
    );
    expect(screen.getByTestId("tool-document-id")).toHaveTextContent(
      "focused-offer"
    );
  ...
  it("routes workspace settings through the accepted controller", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Reset workspace layout" })
      ).toBeEnabled()
    );
  ...
  it("points compact users to the sheet trigger instead of exposing a dead panel command", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Toggle properties" })
    ).not.toBeInTheDocument();
  });
});
