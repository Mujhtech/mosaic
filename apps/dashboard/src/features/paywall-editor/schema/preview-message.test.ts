...
import sessionFlowFixture from "../../../../../../protocol/fixtures/local-preview/v0.4/session-flow.messages.json";
import canonicalDocumentFixture from "../../../../../../protocol/fixtures/v0.4/complete-paywall.json";
...
describe("preview message adapter", () => {
  it("matches the canonical fixture flow including the intentional invalid draft", () => {
    ...
    if (!canonicalDocument.ok) {
      throw new Error("Canonical fixture must validate");
    }
    ...
    expect(canonicalDocument.value.schemaVersion).toBe("0.4");
  });
  ...
  it("emits exact canonical draft, commerce, and heartbeat envelopes", () => {
    ...
    expect(PREVIEW_WEBSOCKET_SUBPROTOCOL).toBe("mosaic.local-preview.v0.4");
    ...
    expect(
      validatePreviewMessage(commerce, { document: canonicalDocument.value }).ok
    ).toBe(true);
  ...
  it("drops messages with unknown envelope fields", () => {
    ...
    expect(
      parsePreviewMessage(JSON.stringify({ ...heartbeat, unexpected: true }))
    ).toBeNull();
  });
});
