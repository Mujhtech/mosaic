...
function appearanceOptions(node: ProtocolNode): AppearanceOption[] {
  ...
  return [
    { background: appearance?.background, opacity: appearance?.opacity ?? 1 },
  ];
}
...
function backgroundSamples(
  ...
  const layers = [
    ...findAncestorNodes(document, node.id),
    ...(includeSelf ? [node] : []),
  ];
  for (const layer of layers) {
    samples = samples.flatMap((sample) =>
      appearanceOptions(layer).map((appearance) =>
        applyAppearance(document, sample, appearance)
      )
    );
  }
      ...
      return [
        {
          ...
          threshold:
            ...
            (node.typography.fontSize >= 18.66 &&
              (node.typography.weight === "semibold" ||
                node.typography.weight === "bold"))
              ? 3
              : 4.5,
        },
      ];
...
function boundaryFields(node: ProtocolNode): ContrastField[] {
  ...
  const appearance = "appearance" in node ? node.appearance : undefined;
  if (!(appearance && "border" in appearance && appearance.border?.width)) {
    return [];
  }
  return [
    {
...
function evaluateContrast(
  ...
  for (const sample of samples) {
    ...
    const alpha =
      foreground.alpha * sample.contentOpacity * (field.opacity ?? 1);
    const composedForeground = composite(
      { ...foreground, alpha },
      sample.color
    );
    if (contrastRatio(composedForeground, sample.color) < field.threshold) {
      return "low";
    }
  }
...
function expandedRuntimeText(value: string) {
  return value
    .replaceAll(
      /\{\{\s*product\.name\s*\}\}/g,
      "A very long subscription product name"
    )
...
function estimatedTextWidthAtScale(
  ...
  return (
    Math.max(
      ...localizedValues(document, node.value).map(
        (value) => expandedRuntimeText(value).length
      )
    ) *
...
function fixedWidth(node: ProtocolNode): number | null {
  ...
  return width && typeof width === "object" && width.mode === "fixed"
    ? width.value
    : null;
}
...
function horizontalChildren(
  ...
  if (node.type === "productSelector" && node.direction === "horizontal") {
    return node.cards;
  }
  return null;
}

function childMinimumWidthAtScale(
  document: MosaicDocument,
  node: ProtocolNode,
  scale: number
) {
  ...
  if (node.type === "text") {
    return estimatedTextWidthAtScale(document, node, scale);
  }
  if (node.type === "icon") {
    return node.size;
  }
  if (node.type === "countdown") {
    return node.typography.fontSize * 18 * 0.55 * scale;
  }
  return 0;
}
...
function localizedEntries(
  node: ProtocolNode
): Array<{ property: string; value: LocalizedText }> {
  switch (node.type) {
    case "text":
    ...
    case "productSelector":
      return [
        { property: "accessibility", value: node.accessibility.label },
        {
          property: "unavailableFallback",
          value: node.unavailableFallback.message,
        },
      ];
    case "productCard":
...
export function validateEditorDocument(
  ...
  const selectorIds = new Set(
    entries.flatMap((entry) =>
      entry.node.type === "productSelector" ? entry.node.id : []
    )
  );
