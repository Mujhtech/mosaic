...
describe("Studio workspace store", () => {
  ...
  it("persists every real action exactly once", () => {
    ...
    expectOneWrite(store, memory.storage, () =>
      store.commitPanelSize("left", 340)
    );
    expectOneWrite(store, memory.storage, () =>
      store.setPanelCollapsed("left", true)
    );
    expectOneWrite(store, memory.storage, () =>
      store.setSelectedTool("components")
    );
    ...
    expectOneWrite(store, memory.storage, () =>
      store.setCanvasPreferences(canvas)
    );
    expectOneWrite(store, memory.storage, () =>
      store.setCanvasPreference("zoom", 1.75)
    );
    expectOneWrite(store, memory.storage, () =>
    ...
    expectOneWrite(store, memory.storage, () =>
      store.removeLayerLabel("headline")
    );
    expectOneWrite(store, memory.storage, () =>
      store.setLayerLocked("headline", true)
    );
    expectOneWrite(store, memory.storage, () =>
      store.toggleLayerLocked("headline")
    );
    expectOneWrite(store, memory.storage, () =>
      store.toggleLayerLocked("headline")
    );
    expectOneWrite(store, memory.storage, () =>
      store.setLayerCanvasHidden("hero-image", true)
    );
    expectOneWrite(store, memory.storage, () =>
      store.toggleLayerCanvasHidden("hero-image")
    );
    expectOneWrite(store, memory.storage, () =>
      store.toggleLayerCanvasHidden("hero-image")
    );
    expectOneWrite(store, memory.storage, () =>
      store.recordRecentInsertion("stack")
    );

    expect(JSON.parse(memory.read() ?? "null")).toEqual(
      store.getSnapshot().preferences
    );
  });
  ...
  it("rejects invalid and unchanged actions without writing or changing persistence metadata", () => {
    ...
    expectNoWrite(memory.storage, () =>
      store.commitPanelSize("diagnostics", 1201)
    );
    expectNoWrite(memory.storage, () =>
      store.commitPanelSize("left", Number.NaN)
    );
    ...
    expectNoWrite(memory.storage, () =>
      store.setSelectedTool(
        "future-tool" as StudioWorkspacePreferencesV1["selectedTool"]
      )
    );
    expectNoWrite(memory.storage, () => store.setCanvasPreference("zoom", 1.5));
    expectNoWrite(memory.storage, () =>
      store.setCanvasPreference("zoom", Number.POSITIVE_INFINITY)
    );
    expectNoWrite(memory.storage, () =>
      store.setFramePosition("main", { x: 120.25, y: -48.5 })
    );
    expectNoWrite(memory.storage, () =>
      store.setFramePosition("main", { x: Number.NaN, y: 0 })
    );
    expectNoWrite(memory.storage, () =>
      store.setFramePosition("Bad screen ID", { x: 0, y: 0 })
    );
    expectNoWrite(memory.storage, () =>
    ...
    expectNoWrite(memory.storage, () =>
      store.recordRecentInsertion(
        "scrollContainer" as StudioRecentInsertableType
      )
    );
  ...
  it("sets, removes, and toggles Studio-only layer metadata", () => {
    ...
    expect(
      store.getSnapshot().preferences.layerMetadata.canvasHiddenIds
    ).toEqual(["hero-image"]);
    expect(store.toggleLayerCanvasHidden("hero-image")).toBe(true);
    expect(
      store.getSnapshot().preferences.layerMetadata.canvasHiddenIds
  ...
  it("persists per-screen canvas frame positions as workspace-only state", () => {
    ...
    expect(store.setFramePosition("main", { x: 120.25, y: -48.5 })).toBe(true);
    expect(store.setFramePosition("upgrade-sheet", { x: 720, y: 64 })).toBe(
      true
    );
  ...
  it("keeps usable in-memory mutations when writes fail or storage is unavailable", () => {
    ...
    expect(unavailableStore.getSnapshot().preferences.selectedTool).toBe(
      "settings"
    );
  ...
  it("restores complete defaults even when persisted-key removal fails", () => {
    ...
    expect(store.getSnapshot().preferences).toBe(
      DEFAULT_STUDIO_WORKSPACE_PREFERENCES
    );
    expect(store.getSnapshot().latestPersistence).toEqual({
      operation: "reset",
  ...
  it("never mutates editor state, history, revisions, or portable document JSON", () => {
    ...
    if (!before.document) {
      throw new Error("Expected a loaded editor document");
    }
