...
const StudioWorkspaceStoreContext = createContext<StudioWorkspaceStore | null>(
  null
);
...
export function StudioWorkspaceStoreProvider({
  ...
  const [store] = useState(() =>
    storage === undefined
      ? createStudioWorkspaceStore()
      : createStudioWorkspaceStore({ storage })
  );
  return (
...
export function useStudioWorkspaceSelector<Selection>(
  ...
  if (!store) {
    throw new Error(
      "useStudioWorkspaceSelector must be used inside StudioWorkspaceStoreProvider"
    );
  }
...
export function useStudioWorkspaceActions(): StudioWorkspaceActions {
  ...
  if (!store) {
    throw new Error(
      "useStudioWorkspaceActions must be used inside StudioWorkspaceStoreProvider"
    );
  }
  return store;
}
