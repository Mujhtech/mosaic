...
describe("editor store", () => {
  ...
  it("groups document edits into monotonic undo and redo revisions", () => {
    ...
    expect(
      findNode(store.getSnapshot().document as MosaicDocument, "headline")
    ).toMatchObject({
    ...
    expect(
      findNode(final.document as MosaicDocument, "headline")
    ).toMatchObject({
      value: { default: "A safer headline" },
    });
  ...
  it("commits multiple transient document updates as one undoable revision", () => {
    ...
    const beforeHeadline = findNode(
      before.document as MosaicDocument,
      "headline"
    );
    ...
    expect(
      store.updateComponentInTransaction("headline", (node) =>
        node.type === "text"
          ? {
              ...node,
              value: { ...node.value, default: "First transient value" },
            }
          : node
      )
    ).toBe(true);
    expect(
      store.updateComponentInTransaction("headline", (node) =>
        node.type === "text"
          ? {
              ...node,
              value: { ...node.value, default: "Final transient value" },
            }
          : node
      )
    ).toBe(true);
    ...
    expect(
      findNode(transient.document as MosaicDocument, "headline")
    ).toMatchObject({
    ...
    expect(committed.document?.revision).toBe(
      (before.document?.revision ?? 0) + 1
    );
  ...
  it("cancels transient edits without rolling workspace state or clearing redo", () => {
    ...
    store.updateComponent("headline", (node) =>
      node.type === "text"
        ? { ...node, value: { ...node.value, default: "Redo target" } }
        : node
    );
    store.undo();
  ...
  it("leaves history and revisions unchanged when a transaction has no semantic change", () => {
    ...
    expect(store.updateDocumentInTransaction((document) => document)).toBe(
      false
    );
    expect(store.beginDocumentTransaction()).toBe(true);
    store.updateComponentInTransaction("headline", (node) =>
    ...
    store.updateComponentInTransaction("headline", (node) =>
      node.type === "text"
        ? { ...node, value: cloneValue(headline.value) }
        : node
    );
  ...
  it("reconciles selection when an inserted block is undone", () => {
    ...
    expect(
      findNode(afterUndo.document as MosaicDocument, insertedId)
    ).toBeNull();
    ...
    expect(
      findNode(afterRedo.document as MosaicDocument, insertedId)
    ).not.toBeNull();
    expect(
      findNode(
        afterRedo.document as MosaicDocument,
  ...
  it("prunes removed localization metadata and restores it through history", () => {
    ...
    expect(
      validateEditorDocument(removed).filter(
    ...
    ).toBe("Native on every platform");

    store.redo();
    expect(
      (store.getSnapshot().document as MosaicDocument).localization.locales.en
        ?.strings["paywall.feature.native"]
    ).toBeUndefined();
  });
  ...
  it("creates and preserves an authored bundled image asset when its block is removed", () => {
    ...
    expect(
      findNode(store.getSnapshot().document as MosaicDocument, imageId)
    ).toMatchObject({
      type: "image",
      assetId: "hero-image",
    });
    const source = store.getSnapshot().document?.assets[0]?.source;
    expect(source?.type === "bundled" ? source.key : null).toBe(
      "mosaic.paywall.hero-image"
    );
    ...
    expect(
      findNode(store.getSnapshot().document as MosaicDocument, reinsertedId)
    ).toMatchObject({
      type: "image",
      assetId: "hero-image",
    });
    ...
    expect(
      validateEditorDocument(
        store.getSnapshot().document as MosaicDocument
  ...
  it("allows removing the only child of a nested stack", () => {
    ...
    expect(
      findNode(store.getSnapshot().document as MosaicDocument, "nested-stack")
    ).toMatchObject({
      type: "stack",
      children: [],
    });
