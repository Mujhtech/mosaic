...
const selectTool = (snapshot: StudioWorkspaceSnapshot) =>
  snapshot.preferences.selectedTool;
const selectLeftSize = (snapshot: StudioWorkspaceSnapshot) =>
  snapshot.preferences.panels.left.size;
...
describe("Studio workspace store context", () => {
  it("isolates primitive and equality-selected consumers from unrelated updates", () => {
    ...
    function ToolConsumer() {
      toolRenders += 1;
      return (
        <span data-testid="tool">{useStudioWorkspaceSelector(selectTool)}</span>
      );
    }
    ...
    function SizeConsumer() {
      sizeRenders += 1;
      return (
        <span data-testid="left-size">
          {useStudioWorkspaceSelector(selectLeftSize)}
        </span>
      );
    }
    ...
    function CanvasIdentityConsumer() {
      ...
      const identity = useStudioWorkspaceSelector(
        selectCanvasIdentity,
        canvasIdentityEqual
      );
      return (
        <span data-testid="canvas-identity">
          {`${identity.device}:${identity.orientation}`}
        </span>
      );
    }
    ...
    expect(screen.getByTestId("canvas-identity")).toHaveTextContent(
