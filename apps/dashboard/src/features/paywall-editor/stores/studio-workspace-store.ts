...
function canvasPreferencesEqual(
  left: StudioCanvasPreferences,
  right: StudioCanvasPreferences
) {
...
export function createStudioWorkspaceStore(
  ...
  function persistPreferences(preferences: StudioWorkspacePreferencesV1) {
    ...
    snapshot = Object.freeze({
      ...
      latestPersistence: Object.freeze({
        operation: "write",
  ...
  function setIdState(
    key: "lockedIds" | "canvasHiddenIds",
    componentId: string,
    enabled: boolean
  ) {
    if (typeof componentId !== "string" || typeof enabled !== "boolean") {
      return false;
    }
    const current = snapshot.preferences.layerMetadata[key];
    const includesId = current.includes(componentId);
    if (includesId === enabled) {
      return false;
    }
    const next = enabled
      ? [...current, componentId]
  ...
  const actions: StudioWorkspaceActions = {
    commitPanelLayout: (panel, preference) => {
      ...
      if (
        current.size === preference.size &&
        current.collapsed === preference.collapsed
      ) {
        return false;
      }
    ...
    setSelectedTool: (selectedTool) => {
      if (
        ...
        snapshot.preferences.selectedTool === selectedTool
      ) {
        return false;
      }
    ...
    setCanvasPreferences: (canvas) => {
      ...
      if (
        ...
        canvasPreferencesEqual(
          snapshot.preferences.canvas,
          result.preferences.canvas
        )
      ) {
        return false;
      }
    ...
    setCanvasPreference: (key, value) => {
      if (
        !CANVAS_KEY_SET.has(key) ||
        Object.is(snapshot.preferences.canvas[key], value)
      ) {
        return false;
      }
      return commitCandidate({
    ...
    setLayerLabel: (componentId, label) => {
      if (typeof componentId !== "string" || typeof label !== "string") {
        return false;
      }
      if (snapshot.preferences.layerMetadata.labels[componentId] === label) {
        return false;
      }
      return commitCandidate({
        ...snapshot.preferences,
        layerMetadata: {
          ...
          labels: {
            ...snapshot.preferences.layerMetadata.labels,
            [componentId]: label,
          },
        },
      });
    ...
    setLayerLocked: (componentId, locked) =>
      setIdState("lockedIds", componentId, locked),
    ...
    toggleLayerCanvasHidden: (componentId) => {
      ...
      return setIdState(
        "canvasHiddenIds",
        componentId,
        !snapshot.preferences.layerMetadata.canvasHiddenIds.includes(
          componentId
        )
      );
    },
    recordRecentInsertion: (type) => {
      if (
        typeof type !== "string" ||
        snapshot.preferences.recentInsertions[0] === type
      ) {
        return false;
      }
      const recentInsertions = [
        type,
        ...snapshot.preferences.recentInsertions.filter(
          (entry) => entry !== type
        ),
