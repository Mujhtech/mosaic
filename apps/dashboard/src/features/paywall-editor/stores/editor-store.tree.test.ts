...
function expectOneCommit(
  store: EditorStore,
  operation: () => TreeOperationResult
) {
...
function expectExpandedIdsValid(store: EditorStore) {
  ...
  for (const id of snapshot.expandedTreeNodes) {
    expect(
      findStack(document, id),
      `Expected expanded ${id} to be a current Stack`
    ).not.toBeNull();
  }
}
...
describe("editor store tree commands", () => {
  ...
  it("does not mutate history, revisions, or subscriptions for rejected operations", () => {
    ...
    expectNoCommit(
      sibling,
      () => sibling.moveSelectedComponent(-1),
      "sibling-boundary"
    );
    ...
    expectNoCommit(
      indent,
      indent.indentSelectedComponent,
    ...
    expectNoCommit(
      deletion,
      deletion.deleteSelectedComponent,
      "empty-source-stack"
    );
  });
    ...
    store.updateComponent(secondPurchase.nodeId, (node) =>
      node.type === "button"
        ? {
            ...
            action: {
              type: "purchase",
              productSelectorId: secondSelector.nodeId,
            },
          }
        : node
    );
  ...
  it("duplicates and deletes recursive subtrees with stable undo and redo integrity", () => {
    ...
    const copiedSelector = duplicatedStack.children.find(
    ...
    const copiedIds = flattenDocument(duplicatedDocument)
      .filter((entry) =>
        [
          duplicated.nodeId,
          ...duplicatedStack.children.map((node) => node.id),
        ].includes(entry.node.id)
      )
