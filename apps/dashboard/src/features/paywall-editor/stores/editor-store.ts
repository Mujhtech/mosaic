...
function reconcileSelection(
  ...
  if (document.screens.some((screen) => screen.layout.id === selectedId)) {
    return selectedId;
  }
  if (findNode(document, selectedId)) {
    return selectedId;
  }
...
function reconcileLocale(document: MosaicDocument, locale: string) {
  return document.localization.locales[locale]
    ? locale
...
export function createEditorStore(
  ...
  function commitDocument(
    ...
    if (!current || documentsEqual(current, synchronized)) {
      return false;
    }

    const undoStack = [...state.undoStack, cloneValue(current)].slice(
    ...
    emit({
      ...
      hoveredComponentId: findNode(revisedDocument, state.hoveredComponentId)
        ...
        : null,
      productLayerPreview: findNode(
        revisedDocument,
        state.productLayerPreview?.nodeId ?? null
      )
  ...
  function unavailableOperation(
    ...
    if (!state.document) {
      return rejectTreeOperation(operation, "document-unavailable");
    }
    if (documentTransaction) {
      return rejectTreeOperation(operation, "transaction-active");
    }
  ...
  function applyTreeOperation(
    ...
    if (
      !commitDocument(result.document, {
        selectedComponentId: result.selectionId,
      })
    ) {
      return rejectTreeOperation(result.operation, "no-op", {
  ...
  function insertComponentAt(
    ...
    if (!state.document || documentTransaction) {
      return unavailableOperation("insert");
    }
    return applyTreeOperation(
      insertBlockAtLocation(state.document, type, location, configuration)
    );
  }
  ...
  function moveSelectedComponent(direction: -1 | 1): TreeOperationResult {
    ...
    const target =
      direction === -1 ? boundaries?.previousSibling : boundaries?.nextSibling;
    if (!target) {
      return rejectTreeOperation("move", "sibling-boundary", {
        nodeId: selectedId,
      });
    }
  ...
  function indentSelectedComponent(): TreeOperationResult {
    ...
    if (!boundaries?.canIndentIntoPrevious || previous?.type !== "stack") {
      return rejectTreeOperation("move", "indent-target-unavailable", {
        nodeId: selectedId,
      });
    }
    return moveComponent(selectedId, {
  ...
  function outdentSelectedComponent(): TreeOperationResult {
    ...
    if (
      ...
      document.screens.some(
        (screen) => parent.parent.id === screen.layout.content.id
      )
    ) {
      return rejectTreeOperation("move", "outdent-boundary", {
        nodeId: selectedId,
      });
    }
    return moveComponent(selectedId, {
  ...
  function updateDocumentInActiveTransaction(
    ...
    if (!(documentTransaction && state.document)) {
      return false;
    }
    const synchronized = synchronizeProtocolMetadata(
      updater(cloneValue(state.document))
    );
    const nextDocument = {
      ...cloneValue(synchronized),
  ...
  function updateComponentInActiveTransaction(
    ...
    if (
      !(documentTransaction && state.document && findNode(state.document, id))
    ) {
      return false;
    }
    const synchronized = synchronizeProtocolMetadata(
  ...
  return {
    ...
    replaceDocument: (document) => {
      ...
      const selectedComponentId =
        initialLayout(cloned).content.children[0]?.id ?? null;
      emit({
        ...state,
        document: cloned,
        editableDocumentId: createEditableDocumentId(),
    ...
    importDocument: (document) => {
      ...
      emit({
        ...
        hoveredComponentId: null,
        productLayerPreview: null,
        undoStack: [...state.undoStack, cloneValue(state.document)].slice(
          -EDITOR_HISTORY_LIMIT
        ),
        redoStack: [],
        dirty: true,
    ...
    updateComponent: (id, updater) => {
      if (!(state.document && findNode(state.document, id))) {
        return;
      }
      if (documentTransaction) {
    ...
    commitDocumentTransaction: () => {
      ...
      emit({
        ...
        undoStack: [
          ...transaction.undoStack,
          cloneValue(transaction.document),
    ...
    selectComponent: (id) => {
      ...
      const selectedComponentId =
        id &&
        (document.screens.some((screen) => id === screen.layout.id) ||
          findNode(document, id))
          ? id
          : null;
      emit({
        ...state,
    ...
    undo: () => {
      ...
      emit({
        ...
        redoStack: [...state.redoStack, cloneValue(current)].slice(
          -EDITOR_HISTORY_LIMIT
        ),
        dirty: true,
        localRevisionSequence: state.localRevisionSequence + 1,
    ...
    redo: () => {
      ...
      emit({
        ...
        hoveredComponentId: findNode(restored, state.hoveredComponentId)
          ...
          : null,
        currentLocale: reconcileLocale(restored, state.currentLocale),
        undoStack: [...state.undoStack, cloneValue(current)].slice(
          -EDITOR_HISTORY_LIMIT
        ),
    ...
    setLocalRevisionSequence: (sequence) => {
      if (
        ...
        sequence <= state.localRevisionSequence
      ) {
        return;
      }
    ...
    markSaved: (revision) => {
      if (documentTransaction || state.document?.revision !== revision) {
        return;
      }
