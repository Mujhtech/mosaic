...
const LEGACY_CANVAS_KEYS = CANVAS_KEYS.filter(
  (key) => key !== "countdownPreviewAt"
);
...
const LEGACY_DEVICE_MIGRATIONS: Readonly<Record<string, StudioCanvasDevice>> =
  Object.freeze({
...
function isPanelPreference(
  ...
  if (!(isRecord(value) && hasExactKeys(value, PANEL_PREFERENCE_KEYS))) {
    return false;
  }
  const bounds = STUDIO_WORKSPACE_PANEL_SIZE_BOUNDS[panel];
  return (
    isBoundedNumber(value.size, bounds.min, bounds.max) &&
    typeof value.collapsed === "boolean"
  );
}

function isCanvasPreferences(value: unknown): value is StudioCanvasPreferences {
  if (!(isRecord(value) && hasExactKeys(value, CANVAS_KEYS))) {
    return false;
  }
  return (
...
function isFramePositions(
  ...
  return (
    ...
    entries.every(
      ([screenId, position]) =>
        isComponentId(screenId) && isFramePosition(position)
    )
  );
}
...
function isLabelMap(value: unknown): value is Readonly<Record<string, string>> {
  ...
  return (
    ...
    entries.every(
      ([componentId, label]) =>
        isComponentId(componentId) && isLayerLabel(label)
    )
  );
}
...
function isUniqueComponentIdList(value: unknown): value is readonly string[] {
  if (
    !Array.isArray(value) ||
    value.length > STUDIO_LAYER_METADATA_MAX_ENTRIES
  ) {
    return false;
  }
...
function isLayerMetadata(value: unknown): value is StudioLayerMetadata {
  if (!(isRecord(value) && hasExactKeys(value, LAYER_METADATA_KEYS))) {
    return false;
  }
  return (
...
function isRecentInsertions(
  value: unknown
): value is readonly StudioRecentInsertableType[] {
  if (!Array.isArray(value) || value.length > STUDIO_RECENT_INSERTIONS_MAX) {
    return false;
  }
  return (
    value.every(
      (entry) => typeof entry === "string" && RECENT_TYPE_SET.has(entry)
...
function freezePreferences(preferences: StudioWorkspacePreferencesV1) {
  return Object.freeze({
    ...
    framePositions: Object.freeze(
      Object.fromEntries(
        Object.entries(preferences.framePositions).map(
          ([screenId, position]) => [screenId, Object.freeze({ ...position })]
        )
      )
    ),
    layerMetadata: Object.freeze({
      ...
      canvasHiddenIds: Object.freeze([
        ...preferences.layerMetadata.canvasHiddenIds,
      ]),
    }),
...
function migrateLegacyRecentInsertions(value: unknown): unknown {
  if (!(isRecord(value) && Array.isArray(value.recentInsertions))) {
    return value;
  }
  return {
    ...value,
...
function migrateLegacyCountdownPreviewAt(value: unknown): unknown {
  ...
  if (!hasExactKeys(value.canvas, LEGACY_CANVAS_KEYS)) {
    return value;
  }
  return {
    ...value,
...
function migrateLegacyFramePositions(value: unknown): unknown {
  if (!(isRecord(value) && hasExactKeys(value, LEGACY_ROOT_KEYS))) {
    return value;
  }
...
function parsePreferences(input: unknown): StudioWorkspacePreferencesV1 | null {
  ...
  if (value.schemaVersion !== STUDIO_WORKSPACE_SCHEMA_VERSION) {
    return null;
  }
  if (!(isRecord(value.panels) && hasExactKeys(value.panels, PANEL_KEYS))) {
    return null;
  }
  if (!isPanelPreference(value.panels.left, "left")) {
    return null;
  }
  if (!isPanelPreference(value.panels.properties, "properties")) {
    return null;
  }
  if (!isPanelPreference(value.panels.diagnostics, "diagnostics")) {
    return null;
  }
  if (
    typeof value.selectedTool !== "string" ||
    !TOOL_SET.has(value.selectedTool)
  ) {
    return null;
  }
  if (!isCanvasPreferences(value.canvas)) {
    return null;
  }
  if (!isFramePositions(value.framePositions)) {
    return null;
  }
  if (!isLayerMetadata(value.layerMetadata)) {
    return null;
  }
  if (!isRecentInsertions(value.recentInsertions)) {
    return null;
  }
  return freezePreferences(value as unknown as StudioWorkspacePreferencesV1);
}

export function parseStudioWorkspacePreferences(
  value: unknown
): StudioWorkspaceParseResult {
  try {
...
export function readStudioWorkspacePreferences(
  ...
  if (!storage) {
    return defaultReadResult("missing");
  }
  try {
    ...
    if (stored.length > STUDIO_WORKSPACE_MAX_CHARACTERS) {
      return defaultReadResult("invalid");
    }
    const result = parseStudioWorkspacePreferences(
      JSON.parse(stored) as unknown
    );
    if (
      result.status === "valid" &&
      viewportHeight !== null &&
      result.preferences.panels.diagnostics.size >
        diagnosticsViewportMaximum(viewportHeight)
    ) {
      return defaultReadResult("invalid");
    }
...
export function writeStudioWorkspacePreferences(
  ...
  try {
    ...
    if (serialized.length > STUDIO_WORKSPACE_MAX_CHARACTERS) {
      return { status: "invalid" };
    }
