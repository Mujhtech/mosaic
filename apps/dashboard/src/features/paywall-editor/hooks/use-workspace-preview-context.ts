...
export function reconcilePreviewLocale(
  document: MosaicDocument,
  requestedLocale: string
) {
  if (document.localization.locales[requestedLocale]) {
...
export function resolveRestoredPreviewContext({
  ...
  const preferred =
    workspaceSource === "persisted" ? workspaceCanvas : localProjectPreview;
  return {
...
export function useWorkspacePreviewContext() {
  ...
  useEffect(() => {
    ...
    if (locale !== canvas.locale) {
      workspace.setCanvasPreference("locale", locale);
    }
    if (locale !== currentLocale) {
      editor.setLocale(locale);
    }
    if (canvas.textScale !== textScale) {
      editor.setTextScale(canvas.textScale);
    }
  }, [
    canvas.locale,
    canvas.textScale,
