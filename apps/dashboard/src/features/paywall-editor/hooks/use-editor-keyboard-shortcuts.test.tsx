...
function ShortcutHarness({
  ...
  useEffect(() => {
    ...
    actions.updateComponent("headline", (node) =>
      node.type === "text"
        ? { ...node, value: { ...node.value, default: EDITED_HEADLINE } }
        : node
    );
  }, [actions, document]);
  ...
  return (
    <div>
      ...
      <textarea
        ...
        defaultValue="Native textarea history"
      />
      ...
      <button onClick={actions.undo} type="button">
        Prepare redo
      </button>
...
describe("editor keyboard shortcuts", () => {
  ...
  it("still dispatches document commands from a non-editable surface", async () => {
    ...
    expect(
      shortcut(surface, { ctrlKey: true, key: "z" }).defaultPrevented
    ).toBe(true);
    expect(screen.getByTestId("headline")).toHaveTextContent(ORIGINAL_HEADLINE);

    expect(
      shortcut(surface, { ctrlKey: true, key: "y" }).defaultPrevented
    ).toBe(true);
    expect(screen.getByTestId("headline")).toHaveTextContent(EDITED_HEADLINE);

    expect(
      shortcut(surface, { ctrlKey: true, key: "ArrowDown" }).defaultPrevented
    ).toBe(true);
    expect(screen.getByTestId("component-order").textContent).not.toBe(
      initialOrder
    );

    expect(
      shortcut(surface, { altKey: true, shiftKey: true, key: "d" })
        .defaultPrevented
    ).toBe(true);
  ...
  it("dispatches every conflict-safe Studio shortcut through one registry", async () => {
    const handlers = {
      onFitCanvas:
        vi.fn<NonNullable<EditorKeyboardShortcutHandlers["onFitCanvas"]>>(),
      onOpenCommandPalette:
        vi.fn<
          NonNullable<EditorKeyboardShortcutHandlers["onOpenCommandPalette"]>
        >(),
      onOpenTool:
        vi.fn<NonNullable<EditorKeyboardShortcutHandlers["onOpenTool"]>>(),
      onResetZoom:
        vi.fn<NonNullable<EditorKeyboardShortcutHandlers["onResetZoom"]>>(),
      onToggleAppearance:
        vi.fn<
          NonNullable<EditorKeyboardShortcutHandlers["onToggleAppearance"]>
        >(),
      onTogglePanel:
        vi.fn<NonNullable<EditorKeyboardShortcutHandlers["onTogglePanel"]>>(),
    };
  ...
  it("does not open commands or start tool chords from editable and search controls", async () => {
    ...
    for (const target of editableTargets()) {
      expect(
        shortcut(target, { ctrlKey: true, shiftKey: true, key: "k" })
          .defaultPrevented
      ).toBe(false);
