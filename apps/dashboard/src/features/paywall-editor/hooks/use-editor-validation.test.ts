...
describe("editor validation merge", () => {
  ...
  it("maps canonical localized defaults and keys to their exact inspector addresses", () => {
    ...
    if (mismatchHeadline?.type !== "text") {
      throw new Error("Missing headline");
    }
    mismatchHeadline.value.default =
      "A default that does not match the catalog";
    ...
    if (invalidKeyHeadline?.type !== "text") {
      throw new Error("Missing headline");
    }
  ...
  it("keeps only diagnostics from the matching text-accessibility branch", () => {
    ...
    (
      text as unknown as { accessibility: { role: "text"; level: number } }
    ).accessibility = {
      role: "text",
      level: 2,
    };
