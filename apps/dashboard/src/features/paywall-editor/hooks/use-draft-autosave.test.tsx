        ...
        onClick={() => {
          updateComponentInTransaction("headline", (node) =>
            node.type === "text"
              ? {
                  ...node,
                  value: { ...node.value, default: "Transient headline" },
                }
              : node
          );
          updateComponentInTransaction("headline", (node) =>
            node.type === "text"
              ? {
                  ...node,
                  value: { ...node.value, default: "Committed headline" },
                }
              : node
          );
        }}
...
describe("draft autosave", () => {
  ...
  it("does not autosave preview-only context and includes its latest values in a later mock save", async () => {
    ...
    const repricedProducts: readonly MockProductDefinition[] =
      DEFAULT_MOCK_PRODUCTS.map((product) =>
    ...
    rerender(
      <EditorStoreProvider>
        <AutosaveHarness
          mockProducts={repricedProducts}
          mockPurchaseState="purchaseFailure"
        />
      </EditorStoreProvider>
    );
    await flushAutosave();
            ...
            products: [
              {
                ...
                localizedPrice: "Test $7.99",
              },
              {
                productReferenceId: "yearly-plan",
                localizedPrice: "Test $59.99",
              },
            ],
          },
        },
  ...
  it("retries the current fingerprint after a failed storage write", async () => {
    ...
    await flushAutosave();
    expect(screen.getByTestId("retry-autosave-status")).toHaveTextContent(
      "failed"
    );
  ...
  it("flushes the pending local project synchronously before Back", () => {
    ...
    expect(screen.getByTestId("flush-autosave-status")).toHaveTextContent(
      "saving"
    );
  ...
  it("keeps a failed synchronous Back flush pending for the exact retry", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Retry flushed autosave" })
    );
    expect(screen.getByTestId("flush-autosave-status")).toHaveTextContent(
      "saved"
    );
  ...
  it("does not autosave transient document states and writes once after commit", async () => {
    ...
    await flushAutosave();
    expect(screen.getByTestId("transaction-autosave-status")).toHaveTextContent(
      "saved"
    );
    storageSpy.mockClear();

    fireEvent.click(
      screen.getByRole("button", { name: "Begin document transaction" })
    );
    fireEvent.click(
      screen.getByRole("button", { name: "Update headline in transaction" })
    );
    await flushAutosave();
    expect(storageSpy).not.toHaveBeenCalled();

    fireEvent.click(
      screen.getByRole("button", { name: "Commit document transaction" })
    );
    await flushAutosave();
