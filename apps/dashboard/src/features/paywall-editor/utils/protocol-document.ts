...
const CAPABILITY_BY_TYPE: Partial<
  Record<
    ProtocolNode["type"] | "scrollContainer",
...
function objectUsesColor(value: unknown): boolean {
  if (Array.isArray(value)) {
    return value.some(objectUsesColor);
  }
...
function objectUsesStyleType(
  value: unknown,
  types: ReadonlySet<string>
): boolean {
  if (Array.isArray(value)) {
    return value.some((entry) => objectUsesStyleType(entry, types));
  }
  if (!value || typeof value !== "object") {
    return false;
  }
...
function localizedTextUsesProductTemplate(
  ...
  ].some((text) => PRODUCT_TOKEN.test(text));
}
...
export function expectedCapabilities(document: MosaicDocument) {
  ...
  if (
    objectUsesStyleType(document, new Set(["linearGradient", "radialGradient"]))
  ) {
  ...
  for (const node of nodes) {
    ...
    if ("typography" in node) {
      capabilities.add("style.typography");
    }
    if (
    ...
    if ("outerInsets" in node && node.outerInsets) {
      capabilities.add("layout.outerInsets");
    }
    if (
      "appearance" in node &&
      node.appearance &&
      "clipContent" in node.appearance
    ) {
  ...
  return new Set(
    CAPABILITY_ORDER.filter((capability) => capabilities.has(capability))
  );
}
...
export function synchronizeProtocolMetadata(
  ...
  next.compatibility.requiredCapabilities = [...expectedCapabilities(next)].map(
    (name) => ({
      name,
      version: "0.2",
    })
