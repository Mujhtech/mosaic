...
export function updateGradientStopPosition(
  ...
  if (!(current && Number.isFinite(requestedPosition))) {
    return [...stops];
  }
  ...
  const position =
    minimum <= maximum
      ? Math.min(maximum, Math.max(minimum, requestedPosition))
      : current.position;

  return stops.map((stop, currentIndex) =>
    currentIndex === index ? { ...stop, position } : stop
  );
}

export function tokenReferenceType(category: DesignCategory) {
  if (category === "colors") {
    return "colorToken" as const;
  }
...
function referencedTokenId(
  ...
  return record.type === tokenReferenceType(category) &&
...
export function isSafeTokenReplacement(
  ...
  while (currentId) {
    if (currentId === sourceId || visited.has(currentId)) {
      return false;
    }
...
export function appendBackgroundAsset(
  ...
  return {
    assetId,
    document: { ...document, assets: [...document.assets, asset] },
  };
}
