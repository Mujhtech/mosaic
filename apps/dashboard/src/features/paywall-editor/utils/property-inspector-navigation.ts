...
function addressForTarget(
  ...
  if (segments.length === 1 && segments[0] === "accessibility") {
    ...
    if (target.type === "image" || target.type === "icon") {
      return "accessibility.hidden";
    }
    return "accessibility.label";
  }
  if (segments.length === 1 && segments[0] === "padding") {
    return "padding.top";
  }
  if (segments.length === 1 && segments[0] === "outerInsets") {
    return "outerInsets.top";
  }
  if (segments.length === 1 && segments[0] === "sizing") {
    return "sizing.width";
  }
  if (segments.length === 1 && segments[0] === "typography") {
    return "typography.style";
  }
  if (segments.length === 1 && segments[0] === "appearance") {
    return "appearance.opacity";
  }
  if (segments.length === 1 && segments[0] === "visibility") {
  ...
  if (addressSegments.at(-1) === "default") {
    addressSegments.pop();
  }
  return addressSegments.join(".") || fallback;
}

function inspectorTargetForPath(
  ...
  const targets: Array<{ path: string; target: InspectorTarget }> = [
    ...document.screens.flatMap((screen, index) => [
      ...
      {
        path: `/screens/${index}/layout/content`,
        target: screen.layout.content,
      },
    ]),
    ...flattenDocument(document).map((entry) => ({
...
export function getInspectorFieldId(
  componentId: string,
...
export function validationPropertyAddress(
  issue: Pick<ValidationIssue, "property">
) {
  const property = issue.property ?? "";
  if (property === "accessibility") {
    return "accessibility.label";
  }
  if (property === "productSelectorId") {
    return "action.productSelectorId";
  }
  if (property === "outerInsets") {
    return "outerInsets.top";
  }
  if (property === "sizing") {
    return "sizing.width";
  ...
  if (property === "styles") {
    return "styles.default.background";
  }
  if (FEATURE_ITEM.test(property)) {
...
export function resolveInspectorValidationIssue(
  ...
  if (matched) {
    ...
    return {
      ...
      property: addressForTarget(
        matched.target,
        pointerSegments(relativePath),
        fallback
      ),
    };
  }
  ...
  const existingTarget =
    document.screens.find((screen) => issue.componentId === screen.layout.id)
      ?.layout ?? findNode(document, issue.componentId ?? null);
  if (!existingTarget) {
    return issue;
  }
  return {
