...
export const ZERO_INSETS = Object.freeze({
  top: 0,
  start: 0,
  bottom: 0,
  end: 0,
...
export function containerCollections(
  ...
  if (container.type === "productSelector") {
    return [["cards", container.cards]];
  }
...
export function collectionChildren(
  ...
  return (
    containerCollections(container).find(
...
export function containerCanAccept(
  ...
  if (parent.type === "productSelector") {
    return (
      collection === "cards" &&
      node.type === "productCard" &&
      parent.cards.length < 20
    );
  }
  if (
    collection === "cards" ||
    (collection === "inProgressChildren" && parent.type !== "button")
  ) {
    return false;
  }
  if (parent.type === "button") {
    return isPassiveProductNode(node);
  }
...
> = {
  ...
  "transaction-active": {
    ...
    recovery:
      "Commit or cancel the active edit before changing the component tree.",
  },
  ...
  "non-stack-parent": {
    ...
    recovery:
      "Insert inside a Stack, or place the component before or after a leaf.",
  },
  "invalid-index": {
    message: "The requested child position is outside the target Stack.",
    recovery:
      "Choose an index from zero through the Stack's current child count.",
  },
  ...
  "duplicate-id": {
    message:
      "The component subtree contains an ID already used by the paywall.",
    recovery:
      "Duplicate through Studio or assign fresh component IDs before insertion.",
  },
  "root-immutable": {
    message:
      "The root content Stack and Carousel page roots cannot be moved or deleted.",
  ...
  "indent-target-unavailable": {
    message:
      "The previous sibling is not a Stack that can contain this component.",
    recovery:
      "Place a Stack immediately before the component, then indent again.",
  },
  "outdent-boundary": {
...
export function findNode(
  ...
  const root = document.screens.find(
  ...
  return (
    flattenDocument(document).find((entry) => entry.node.id === id)?.node ??
    null
  );
}
...
export function findNodeEntry(
  ...
  return (
    flattenDocument(document).find((entry) => entry.node.id === id) ?? null
  );
}

export function screenContainingNode(
  document: MosaicDocument,
  id: string | null
): Screen | null {
  if (!id) {
  ...
  const direct = document.screens.find(
    (screen) =>
      screen.id === id ||
      screen.layout.id === id ||
      screen.layout.content.id === id
  );
  if (direct) {
    return direct;
  }
  const entry = findNodeEntry(document, id);
  const screenIndex = entry?.documentPath.match(SCREEN_INDEX)?.[1];
  return screenIndex === undefined
    ? null
    : (document.screens[Number(screenIndex)] ?? null);
}

export function visitForParent(
  container: ContainerNode,
  id: string
): ParentEntry | null {
  ...
  for (const [collection, children] of collections) {
    ...
    if (index >= 0) {
      return { parent: container, index, collection };
    }
...
export function findParent(
  ...
  if (
    !id ||
    document.screens.some((screen) => id === screen.layout.content.id)
  ) {
    return null;
  }
...
export function findAncestorNodes(
  document: MosaicDocument,
  id: string | null
): ProtocolNode[] {
  if (
  ...
  function visit(
    container: ContainerNode,
    ancestors: ProtocolNode[]
  ): ProtocolNode[] | null {
    const children = containerCollections(container).flatMap(
...
export function findAncestorStacks(
  document: MosaicDocument,
  id: string | null
): StackComponent[] {
...
export function findAncestorStackIds(
  document: MosaicDocument,
  id: string | null
): string[] {
...
export function isRootStack(document: MosaicDocument, stackId: string) {
  return document.screens.some(
    (screen) => stackId === screen.layout.content.id
  );
}

export function parentEntryChildren(
  entry: ParentEntry
...
export function reconcileExpandedTreeNodes(
  ...
  return new Set(
    [
      ...document.screens.map((screen) => screen.layout.content.id),
      ...expandedIds,
    ].filter((id) => validContainerIds.has(id))
  );
}
...
export function updateNode(
  ...
  const screens = document.screens.map((screen) => {
    ...
    if (content.type !== "stack" || content === screen.layout.content) {
      return screen;
    }
    changed = true;
