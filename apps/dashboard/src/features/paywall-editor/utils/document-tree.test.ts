...
function template(id: "focused" | "benefits" = "focused") {
  ...
  if (!match) {
    throw new Error(`Missing test template ${id}`);
  }
...
function productCardStack(
  id: string,
  children: ProductCardStack["children"]
): ProductCardStack {
  return {
    type: "stack",
    id,
...
function accepted(
  result: TreeOperationResult
): asserts result is TreeOperationAccepted {
...
function editorErrors(document: MosaicDocument) {
  return validateEditorDocument(document).filter(
...
function allIdentifiers(document: MosaicDocument) {
  return [
    ...
    node.type === "featureList"
      ? [node.id, ...node.items.map((item) => item.id)]
      : [node.id]
  );
}
...
describe("document tree transforms", () => {
  it("adds a valid localized screen with collision-safe identifiers", () => {
    ...
    const added = result.document.screens.find(
      (screen) => screen.id === result.screenId
    );
    ...
    expect(screenContainingNode(result.document, result.selectionId)?.id).toBe(
      result.screenId
    );
    ...
    expect(
      validatePaywallDocument(synchronizeProtocolMetadata(result.document)).ok
    ).toBe(true);
  });
  ...
  it("creates an empty Stack and inserts blocks at root and nested locations", () => {
    ...
    expect(
      findStack(nestedInsert.document, rootInsert.nodeId)?.children
    ).toHaveLength(1);
  ...
  it("requires an explicit valid Countdown deadline and accepts an expired deadline unchanged", () => {
    ...
    expect(() => createBlock(document, "countdown")).toThrow(
      /explicit valid UTC deadline/i
    );
    ...
    expect(
      validatePaywallDocument(synchronizeProtocolMetadata(inserted.document)).ok
    ).toBe(true);
  });
  ...
  it("allows multiple selectors and keeps an unbound purchase action validation-visible", () => {
    ...
    expect(editorErrors(orphan.document)).toEqual([
      expect.objectContaining({
        ...
        componentId: orphan.nodeId,
      }),
    ]);

    const laterSelector = insertBlockAtLocation(
      orphan.document,
      "productSelector",
      {
        parentId: required(
  ...
  it("rejects invalid insertion parents, indexes, and all identifier collisions", () => {
    ...
    const emptyStack = insertExistingNodeAtLocation(
      document,
      stack("empty-stack", []),
      {
        parentId: required(document.screens[0], "document.screens[0]").layout
      ...
      insertExistingNodeAtLocation(
        document,
        {
          ...
          items: [
            {
              ...
              text: {
                default: "Collision",
                localizationKey: "paywall.collision",
              },
            },
          ],
          typography: {
          ...
          accessibility: {
            label: {
              default: "Collision",
              localizationKey: "paywall.collision.label",
            },
          },
        },
        {
  ...
  it("moves before, after, and inside while normalizing same-parent indexes", () => {
    const document = template();
    const before = moveNode(document, "headline", {
      placement: "before",
      targetId: "close",
    });
    accepted(before);
    ...
    accepted(inside);
    expect(
      findStack(inside.document, "stack-a")?.children.map((node) => node.id)
    ).toEqual(["headline", "purchase", "stack-b"]);
    expect(
      flattenDocument(inside.document).filter(
        (entry) => entry.node.id === "purchase"
      )
    ).toHaveLength(1);
  ...
  it("outdents across an ancestor without duplicating the source or emptying its old Stack", () => {
    ...
    expect(
      findStack(result.document, "stack-b")?.children.map((node) => node.id)
    ).toEqual(["restore"]);
    expect(
      findStack(result.document, "stack-a")?.children.map((node) => node.id)
    ).toEqual(["headline", "stack-b", "subtitle"]);
    expect(
      flattenDocument(result.document).filter(
        (entry) => entry.node.id === "subtitle"
      )
    ).toHaveLength(1);
  ...
  it("rejects cycles, invalid targets, and no-ops while allowing an empty nested source", () => {
    ...
    accepted(emptying);
    expect(findStack(emptying.document, "sole-stack")?.children).toHaveLength(
      0
    );
  });
  ...
  it("duplicates a recursive commerce subtree with fresh IDs, keys, and repaired references", () => {
    ...
    if (!copy) {
      throw new Error("Missing duplicated Stack");
    }
    const copiedSelector = copy.children.find(
    ...
    const copiedIds = allIdentifiers(result.document).filter(
      (id) => !beforeIds.has(id)
    );
  ...
  it("duplicates a Product Card with a unique atomic product binding", () => {
    ...
    expect(
      new Set(selector.cards.map((card) => card.productReferenceId)).size
    ).toBe(3);
    const copy = selector.cards.find((card) => card.id === result.selectionId);
    expect(copy?.productReferenceId).toBe("product-3");
    expect(
      result.document.products.find((product) => product.id === "product-3")
    ).toMatchObject({
  ...
  it("allocates collision-safe provider product IDs for added and duplicated Product Cards", () => {
    ...
    if (!appended) {
      throw new Error("Product Card was not appended");
    }
  ...
  it("rejects a move that would exceed four nested Stacks inside a Product Card", () => {
    ...
    card.children = [
      productCardStack("card-stack-1", [
        productCardStack("card-stack-2", [
          productCardStack("card-stack-3", [deepest]),
        ]),
      ]),
    ];
  ...
  it("copies every locale value when duplicated fields intentionally share one source key", () => {
    ...
    for (const [locale, value] of Object.entries(sharedValues)) {
      ...
      if (!catalog) {
        throw new Error(`Missing ${locale} catalog`);
      }
  ...
  it("returns deterministic deletion recovery and permits an empty nested Stack", () => {
    ...
    expect(
      findStack(deleted.document, "stack-b")?.children.map((node) => node.id)
    ).toEqual(["restore"]);
  ...
  it("includes Carousel containers when revealing and protecting nested page content", () => {
    ...
    if (carousel?.type !== "carousel") {
      throw new Error("Missing Carousel");
    }
    const pageStack = carousel.pages[0]?.content;
    if (!pageStack) {
      throw new Error("Missing Carousel page Stack");
    }
