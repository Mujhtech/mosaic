...
export function compatibilityWarnings(
  ...
  return clients.flatMap((client) => {
    ...
    if (!client.supportedSchemaVersions.includes(document.schemaVersion)) {
      warnings.push(
        `${client.displayName} does not report support for this paywall format.`
      );
    }
    ...
    const missingDocumentCapabilities =
      document.compatibility.requiredCapabilities.filter(
        (required) =>
          !supportsCapability(client.supportedCapabilities, required)
      );
