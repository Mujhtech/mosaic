...
function withoutMilliseconds(value: Date) {
  return value.toISOString().replace(MILLISECONDS, "Z");
}

export function isValidCountdownInstant(value: unknown): value is string {
  if (typeof value !== "string" || !UTC_COUNTDOWN_INSTANT_PATTERN.test(value)) {
    return false;
  }
  const milliseconds = Date.parse(value);
  return (
    Number.isFinite(milliseconds) &&
    withoutMilliseconds(new Date(milliseconds)) === value
  );
}

export function countdownInstantFromLocalInput(value: string) {
  if (!LOCAL_COUNTDOWN_INSTANT_PATTERN.test(value)) {
    return null;
  }
  const withoutFraction = value.replace(ZERO_FRACTION, "");
  const withSeconds =
    withoutFraction.length === 16 ? `${withoutFraction}:00` : withoutFraction;
...
export function advanceCountdownInstant(value: string, milliseconds: number) {
  if (!(isValidCountdownInstant(value) && Number.isFinite(milliseconds))) {
    return null;
  }
  try {
    const next = withoutMilliseconds(
      new Date(Date.parse(value) + milliseconds)
    );
