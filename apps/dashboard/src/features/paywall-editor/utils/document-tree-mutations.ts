...
export function resolveInsertionParent(
  ...
  if (!parent) {
    return rejectTreeOperation(operation, "unknown-parent", {
      nodeId,
  ...
  if (
    ...
    return rejectTreeOperation(operation, "invalid-index", {
      nodeId,
      targetId: location.parentId,
    });
  }
...
export function insertBlockAtLocation(
  ...
  if (
    type === "countdown" &&
    !isValidCountdownInstant(configuration.countdownEndsAt)
  ) {
...
export function resolveMoveLocation(
  ...
  if (target.targetId === nodeId) {
    return rejectTreeOperation("move", "self-target", {
      nodeId,
  ...
  if (!targetNode) {
    return rejectTreeOperation("move", "unknown-target", {
      nodeId,
      targetId: target.targetId,
    });
  }
  if (target.placement === "inside") {
    ...
    if (!Number.isInteger(index) || index < 0 || index > children.length) {
      return rejectTreeOperation("move", "invalid-index", {
        nodeId,
        targetId: target.targetId,
      });
    }
  ...
  return {
    ...
    ...(targetEntry.collection === "children"
      ? {}
...
export function moveNode(
  ...
  if (!sourceEntry) {
    return rejectTreeOperation("move", "unknown-node", { nodeId });
  }
  if (!sourceEntry.parentId) {
    return rejectTreeOperation("move", "root-immutable", { nodeId });
  }
  ...
  const sourceSubtreeIds = new Set(
    subtreeNodes(sourceEntry.node).map((node) => node.id)
  );
  ...
  const destinationCollection =
    location.collection ?? defaultCollection(destinationParent);
  const sameParent =
  ...
  if (sameParent) {
    const normalizedIndex =
      location.index > sourceEntry.index ? location.index - 1 : location.index;
    if (normalizedIndex === sourceEntry.index) {
      return rejectTreeOperation("move", "no-op", {
        nodeId,
        targetId: target.targetId,
      });
    }
  ...
  const remainingSourceChildren = sourceChildren.filter(
    (node) => node.id !== nodeId
  );
  ...
  const updatedDestinationParent = findContainer(
    withoutSource,
...
export function duplicateSubtree(
  ...
  function duplicateText(text: LocalizedText): LocalizedText {
    const localizationKey = allocateLocalizationKey(
      keys,
      `${text.localizationKey}.copy`
    );
    localizationKeyCopies.push({
      sourceKey: text.localizationKey,
      duplicateKey: localizationKey,
    });
  ...
  function duplicateControl(
    ...
    return {
      ...
      ...(accessibility.hint
        ? { hint: duplicateText(accessibility.hint) }
        : {}),
    };
  }
    ...
    switch (node.type) {
      ...
      case "productSelector": {
        const cards = node.cards.map(
          (card) =>
            duplicateTreeNode(card) as Extract<
              ProtocolNode,
              { type: "productCard" }
            >
        );
...
export function duplicateNode(
  ...
  const sourceEntry = findNodeEntry(document, nodeId);
  if (!sourceEntry) {
    return rejectTreeOperation("duplicate", "unknown-node", { nodeId });
  }
  if (!sourceEntry.parentId) {
    return rejectTreeOperation("duplicate", "root-immutable", { nodeId });
  }
  const destination = location ?? {
    parentId: sourceEntry.parentId,
    index: sourceEntry.index + 1,
    ...(sourceEntry.collection === "children"
      ? {}
  ...
  if (
    ...
    const usedReferences = new Set(
      sourceParent.parent.cards.map((card) => card.productReferenceId)
    );
  ...
  const inserted = insertExistingNodeAtLocation(
    insertionDocument,
    duplicatedNode,
    destination
  );
  if (inserted.status === "rejected") {
    return { ...inserted, operation: "duplicate", nodeId };
  }
  ...
  for (const [locale, catalog] of Object.entries(localization.locales)) {
    ...
    for (const {
      sourceKey,
      duplicateKey,
    } of duplicated.localizationKeyCopies) {
      catalog.strings[duplicateKey] =
        sourceCatalog?.strings[sourceKey] ??
        catalog.strings[duplicateKey] ??
        "";
    }
  }
...
export function deleteNode(
  ...
  const sourceEntry = findNodeEntry(document, nodeId);
  if (!sourceEntry) {
    return rejectTreeOperation("delete", "unknown-node", { nodeId });
  }
  if (!sourceEntry.parentId) {
    return rejectTreeOperation("delete", "root-immutable", { nodeId });
  }
  const parentEntry = findParent(document, nodeId);
  const parent = parentEntry?.parent;
  if (!(parentEntry && parent)) {
    return rejectTreeOperation("delete", "unknown-parent", { nodeId });
  }
  ...
  const fallbackId =
    siblings[sourceEntry.index + 1]?.id ??
    siblings[sourceEntry.index - 1]?.id ??
    parent.id;
  const children = siblings.filter((node) => node.id !== nodeId);
  let nextDocument = replaceContainerChildren(
    document,
    parent.id,
    children,
    parentEntry.collection
  );
  if (
    parent.type === "productSelector" &&
...
export function resolveLocalizedText(
  ...
  return (
    ...
    document.localization.locales[document.localization.fallbackLocale]
      ?.strings[text.localizationKey] ??
    text.default
  );
}
