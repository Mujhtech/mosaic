...
export function protocolColorLabel(
  document: MosaicDocument,
  color: ProtocolColor
) {
  if (!isColorTokenReference(color)) {
    return color;
  }
  return (
    document.designSystem.colors.find((token) => token.id === color.id)?.name ??
    color.id
  );
}

export function resolvedProtocolColor(
  document: MosaicDocument,
  color: ProtocolColor | undefined
) {
  if (!color) {
    return;
  }
  ...
  if (!resolved.startsWith("#")) {
    return SEMANTIC_COLORS[resolved];
  }
...
function remoteAssetUrl(
  ...
  const asset = document.assets.find(
    (candidate) =>
      candidate.id === assetId && (!type || candidate.type === type)
  );
...
export function resolvedBackground(
  ...
  switch (resolved.type) {
    case "color":
      return {
        style: { background: resolvedProtocolColor(document, resolved.value) },
      };
    case "linearGradient": {
    ...
    case "video": {
      ...
      return {
        mediaType: "video",
        style: {
          background: resolvedProtocolColor(document, resolved.fallbackColor),
        },
        video: src
          ? { src, poster, contentMode: resolved.contentMode }
          : undefined,
        diagnostic: src
          ? undefined
...
export function resolvedShadow(
  document: MosaicDocument,
  shadow: ProtocolShadow | undefined
) {
  if (!shadow) {
    return;
  }
