...
export function productCardBoundsAreValid(document: MosaicDocument) {
  return flattenDocument(document)
    ...
    .filter(
      (node): node is Extract<ProtocolNode, { type: "productCard" }> =>
        node.type === "productCard"
    )
    .every((card) => {
...
export function subtreeIsStructurallyValid(node: ProtocolNode): boolean {
  if (node.type === "stack") {
    return node.children.every(subtreeIsStructurallyValid);
  }
  if (node.type === "button") {
    return (
...
export function localizationKeySet(document: MosaicDocument) {
  return new Set(
    Object.values(document.localization.locales).flatMap((catalog) =>
...
export function createUniqueProductReference(
  ...
  const providerProductIds = new Set(
...
export function controlLocalizedEntries(
...
export function referencedLocalizedEntries(
  document: MosaicDocument
): LocalizedText[] {
  return [
    ...document.assets.flatMap((asset) =>
      asset.type === "image" ? [asset.fallback.value] : []
    ),
...
export function ensureLocalizationCatalogs(
  document: MosaicDocument
): MosaicDocument {
  ...
  return {
    ...
    localization: {
      ...
      locales: Object.fromEntries(
        Object.entries(document.localization.locales).map(
          ([locale, catalog]) => [
            locale,
            {
              ...catalog,
              strings: Object.fromEntries([
                ...
                ...entries
                  .filter(
                    (entry) =>
                      catalog.strings[entry.localizationKey] === undefined
                  )
...
export function ensureNodeDependencies(
  ...
  for (const candidate of nodes) {
    if (
      ...
      !assets.some((asset) => asset.id === candidate.assetId)
    ) {
    ...
    if (
      ...
      candidate.cards.every(
        (card) =>
          card.productReferenceId === "monthly-plan" ||
          card.productReferenceId === "yearly-plan"
      )
    ) {
      products.push(
        {
