...
export function restorePanelPreference(
  ...
  if (panelHandle.getSize().inPixels === 0 && !panelHandle.isCollapsed()) {
    return false;
  }
...
export function restoreWorkspacePanelPreferences({
  ...
  const diagnosticsReady =
    ...
    (diagnosticsPanelHandle.getSize().inPixels > 0 ||
      diagnosticsPanelHandle.isCollapsed());
  const propertiesReady =
    !includeProperties ||
    (propertiesPanelHandle !== null &&
      (propertiesPanelHandle.getSize().inPixels > 0 ||
        propertiesPanelHandle.isCollapsed()));
  if (!(leftReady && diagnosticsReady && propertiesReady)) {
    return false;
  }
  const leftApplied = restorePanelPreference(leftPanelHandle, preferences.left);
  const diagnosticsApplied = restorePanelPreference(
    diagnosticsPanelHandle,
...
export function commitCompletedPanelLayout({
  ...
  if (!(meta.isUserInteraction && panelHandle)) {
    return false;
  }
...
export function commitUpstreamDoubleClickResult(
  input: Omit<CompletedPanelLayoutInput, "meta">
) {
  return commitCompletedPanelLayout({
    ...input,
