...
describe("Experiment authoring invariants", () => {
  it("keeps an unavailable trusted-source metric visible in the model but unselectable", () => {
    ...
    expect(canSelectMetric({ ...metric, availability: "available" })).toBe(true)
  })

  it("rejects allocation that does not cover all 10,000 buckets", () => {
    expect(validateAllocation([variant("control", 5_000), variant("treatment", 4_999)])).toMatch(
      /10,000 basis points/,
    )
    expect(
      validateAllocation([variant("control", 5_000), variant("treatment", 5_000)]),
    ).toBeUndefined()
  })
  ...
  it("requires stable Experiment roots and holdout to cover all buckets exactly once", () => {
    ...
    expect(
      validateMutualExclusionAllocation(
        [
          { allocationBasisPoints: 5_000, experimentId: "experiment-a" },
          { allocationBasisPoints: 5_000, experimentId: "experiment-a" },
        ],
        0,
      ),
