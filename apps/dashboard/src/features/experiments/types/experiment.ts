...
export function allocationTotal(variants: readonly ExperimentVariantDraft[]) {
  return variants.reduce(
    (total, variant) => total + variant.allocationBasisPoints,
    0
  );
}
...
export function validateMutualExclusionAllocation(
  ...
  if (members.length < 2) {
    return "Select at least two Experiments.";
  }
  if (
    new Set(members.map((member) => member.experimentId)).size !==
    members.length
  ) {
  ...
  if (
    ...
    members.some((member) => member.allocationBasisPoints <= 0)
  ) {
    return "Every group member needs a positive allocation and holdout cannot be negative.";
  }
  if (
    holdoutBasisPoints +
      members.reduce((sum, member) => sum + member.allocationBasisPoints, 0) !==
    10_000
  ) {
