...
const scopeKey = (scope: ExperimentScope) => [scope.projectId, scope.environmentId] as const

export const experimentKeys = {
  all: ["experiments"] as const,
  detail: (scope: ExperimentScope, experimentId: string) =>
    [...experimentKeys.all, ...scopeKey(scope), "detail", experimentId] as const,
  history: (scope: ExperimentScope, experimentId: string) =>
    [...experimentKeys.all, ...scopeKey(scope), "history", experimentId] as const,
  list: (scope: ExperimentScope) => [...experimentKeys.all, ...scopeKey(scope), "list"] as const,
  ...
  results: (scope: ExperimentScope, experimentId: string) =>
    [...experimentKeys.all, ...scopeKey(scope), "results", experimentId] as const,
}

export function experimentsQueryOptions(scope: ExperimentScope, adapter: ExperimentAdapter) {
  return queryOptions({
...
export function experimentGroupsQueryOptions(scope: ExperimentScope, adapter: ExperimentAdapter) {
  return queryOptions({
...
export function experimentBuilderResourcesQueryOptions(
  ...
  return queryOptions({
    queryKey: [...experimentKeys.all, ...scopeKey(scope), "builder-resources", adapter],
    queryFn: async () => {
