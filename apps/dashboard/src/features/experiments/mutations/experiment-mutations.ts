...
async function invalidateExperiment(
  ...
  if (experimentId) {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: experimentKeys.detail(scope, experimentId) }),
      queryClient.invalidateQueries({ queryKey: experimentKeys.history(scope, experimentId) }),
      queryClient.invalidateQueries({ queryKey: experimentKeys.results(scope, experimentId) }),
    ])
  }
}
...
export function createExperimentMutationOptions(
  ...
  return mutationOptions({
    mutationFn: (input: { hypothesis?: string; name: string; placementId: string }) =>
      adapter.create(scope, input),
    onSuccess: (experiment) => invalidateExperiment(queryClient, scope, experiment.id),
  })
}
...
export function saveExperimentDraftMutationOptions(
  ...
  return mutationOptions({
    mutationFn: (input: { document: ExperimentDraftDocument; expectedRevision: number }) =>
      adapter.saveDraft(scope, experimentId, input.document, input.expectedRevision),
    onSuccess: async () => {
...
export function validateExperimentMutationOptions(
  ...
  return mutationOptions({ mutationFn: () => adapter.validate(scope, experimentId) })
}
...
export function emergencyStopMutationOptions(
  ...
  return mutationOptions({
    mutationFn: (reason: string) => adapter.emergencyStop(scope, experimentId, reason),
    onSuccess: () => invalidateExperiment(queryClient, scope, experimentId),
  })
}
...
export function createQaOverrideMutationOptions(
  ...
  return mutationOptions({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: experimentKeys.qa(scope, experimentId) }),
  })
}
...
export function deleteQaOverrideMutationOptions(
  ...
  return mutationOptions({
    mutationFn: (overrideId: string) => adapter.deleteQaOverride(scope, experimentId, overrideId),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: experimentKeys.qa(scope, experimentId) }),
  })
}
