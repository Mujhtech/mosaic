...
describe("generated Experiment adapter", () => {
  it("creates a subsequent mutual-exclusion Version without mutating the group root", async () => {
      ...
      return response(
        {
          ...
          id: "group-version-2",
          members: [
            { allocationBasisPoints: 4_500, experimentId: "experiment-a" },
            { allocationBasisPoints: 4_500, experimentId: "experiment-b" },
          ],
          versionNumber: 2,
        },
        201,
    ...
    const version = await adapter.createMutualExclusionGroupVersion(scope, "group", {
      assignmentKeyPolicy: "identified_user",
      holdoutBasisPoints: 1_000,
      members: [
        { allocationBasisPoints: 4_500, experimentId: "experiment-a" },
  ...
  it("sends the strong Draft precondition and exact 10,000-bucket allocation", async () => {
    ...
    const fetchImplementation = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (request.method === "GET") return response(experiment(7))
      const body = (await request.clone().json()) as { document: unknown }
      return response({ ...experiment(8).currentDraft, document: body.document })
    }) as typeof fetch
    const adapter = createGeneratedExperimentAdapter(
      createGeneratedDashboardClient(fetchImplementation),
    )
    ...
    expect(body.document.variants.reduce((sum, item) => sum + item.allocationBasisPoints, 0)).toBe(
      10_000,
    )
  })
})
