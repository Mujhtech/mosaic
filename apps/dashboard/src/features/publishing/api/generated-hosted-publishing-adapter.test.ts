...
describe("generated hosted publishing adapter", () => {
  ...
  it("copies a local document into the hosted Paywall identity without mutating the local source", async () => {
    ...
    const fetchImplementation = vi.fn(async (inputRequest: RequestInfo | URL) => {
    ...
    const body = (await request?.clone().json()) as { document: { id: string } }
  ...
  it("reuses its idempotency key after an uncertain save and sends the strong Draft precondition", async () => {
    ...
    await expect(adapter.saveDraft(input)).rejects.toBeInstanceOf(HostedDraftOfflineError)
    await expect(adapter.saveDraft(input)).resolves.toMatchObject({ revision: 5 })
  ...
  it("maps safe server conflict details into the editor conflict error", async () => {
    ...
    const error = await adapter.saveDraft(input).catch((caught: unknown) => caught)
  ...
  it("discovers an active Environment Draft and treats no active Draft as an empty result", async () => {
    const fetchImplementation = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            error: { code: "not_found", message: "No active Draft exists.", requestId: "req_02" },
          }),
