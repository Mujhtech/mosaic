...
export const MOCK_PRODUCT_ACKNOWLEDGEMENT_CODE =
  "product.mock_metadata_acknowledgement";
...
export class HostedPublishingContractPendingError extends Error {
  constructor() {
    super(
      "Hosted publishing is unavailable because the required REST operations are missing."
    );
...
export class HostedDraftConflictError extends Error {
  ...
  constructor(latestRevision: number, serverUpdatedAt?: string) {
    super(
      `Draft revision ${latestRevision} is newer than this editor session.`
    );
...
const pending = () =>
  Promise.reject(new HostedPublishingContractPendingError());

export const CONTRACT_PENDING_HOSTED_PUBLISHING_ADAPTER: HostedPublishingAdapter =
  Object.freeze({
