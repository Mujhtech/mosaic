...
function source(condition: LeafCondition): MosaicPlacementDecisionV1Source {
  if (condition.source === "user_attribute" || condition.source === "entitlement_state") {
    return { kind: condition.source, key: condition.referenceKey ?? "" }
  }
  if (condition.source === "product_availability" || condition.source === "product_readiness") {
  ...
  if (condition.source === "provider_capability") {
    return {
      capability: (condition.referenceKey ?? "product_loading") as
        "product_loading" | "purchase" | "restore" | "entitlement_lookup",
...
function operand(
  ...
  if (condition.operator === "exists" || condition.operator === "does_not_exist") return undefined
  ...
  if (attributeType === "timestamp") return { type: "timestamp", value: String(value) }
  if (attributeType === "semantic_version" || condition.source.endsWith("version")) {
    return { type: "semantic_version", value: String(value) }
  }
  if (attributeType === "string_list" || Array.isArray(value)) {
    return { type: "string_list", value: Array.isArray(value) ? [...value] : [String(value)] }
  }
...
function condition(
  ...
  if (node.kind === "not") {
    ...
    if (!child) throw new Error("Not requires one condition.")
    return { child: condition(child, attributes), type: "not" }
  }
  return { children: node.children.map((child) => condition(child, attributes)), type: node.kind }
}

export function toContractOutcome(outcome: DecisionOutcome): MosaicPlacementDecisionV1Outcome {
  switch (outcome.type) {
    case "paywall":
      return {
...
export function toContractRuleSet(
  ...
  const attributes = new Map(input.attributes.map((attribute) => [attribute.key, attribute]))
  return {
