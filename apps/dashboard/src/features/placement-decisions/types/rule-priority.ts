...
export function moveRule(
  ...
  if (!selected || !destination) return ordered
  ordered[index] = destination
  ordered[target] = selected
  return ordered.map((rule, position) => ({ ...rule, priority: position * 10 }))
}

export function duplicateRule(rules: readonly PlacementRule[], ruleId: string, id: string) {
  const ordered = [...rules].sort((left, right) => left.priority - right.priority)
  const index = ordered.findIndex((rule) => rule.id === ruleId)
  if (index < 0 || ordered.length >= 100) return ordered
