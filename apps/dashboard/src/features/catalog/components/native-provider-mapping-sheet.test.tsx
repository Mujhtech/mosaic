...
describe("NativeProviderMappingSheet", () => {
  it("requires a Google base plan and an explicit offer ID when a specific offer is selected", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Add Google Play Billing mapping" }))
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save configured mapping" }))

    expect(await screen.findByText("Enter the exact base plan ID.")).toBeVisible()
    expect(screen.getByText("Enter the exact offer ID.")).toBeVisible()
    expect(onSubmit).not.toHaveBeenCalled()

    fireEvent.change(screen.getByLabelText("Base plan ID"), { target: { value: "monthly" } })
    fireEvent.change(screen.getByLabelText("Offer ID"), { target: { value: "intro" } })
    fireEvent.click(screen.getByRole("button", { name: "Save configured mapping" }))
  ...
  it("rejects StoreKit for an Android Application without invoking the mutation", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Add StoreKit mapping" }))

    expect(screen.getByRole("alert")).toHaveTextContent(
      "StoreKit is not compatible with this ANDROID Application",
    )
    expect(screen.getByRole("button", { name: "Save configured mapping" })).toBeDisabled()
    expect(onSubmit).not.toHaveBeenCalled()
  })
})
