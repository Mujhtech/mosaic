...
function useNativeProviderMappingForm({
  ...
  return useForm({
    defaultValues: {
      ...
      offerSelection: (initialValue?.googleOfferId
        ? "specific"
        : "none") as GoogleOfferSelection,
    ...
    >
      ...
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader className="border-b p-5">
          ...
          <SheetDescription>
            Map this stable Mosaic Product to one exact native store Product.
            Saving configuration does not prove store availability or purchase
            success.
          </SheetDescription>
        </SheetHeader>
        <form
        ...
        >
          <div className="space-y-5 p-5">
            ...
            {platformCompatible ? null : (
              <p
                ...
                role="alert"
              >
                {providerLabel} is not compatible with this{" "}
                {application.platform.toUpperCase()} Application. Choose the
                platform’s built-in provider instead.
              </p>
            )}
          ...
          <SheetFooter className="border-t p-5">
            ...
            <p className="text-muted-foreground text-xs">
              Provider-owned localized metadata is read-only and appears only
              after an accepted observation. Published Paywall documents
              continue to store the Mosaic Product ID.
            </p>
          </SheetFooter>
        </form>
      </SheetContent>
