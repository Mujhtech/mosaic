...
describe("provider recovery actions", () => {
  it.each(
    PROVIDER_RECOVERY_ACTIONS
  )("maps %s to typed copy and one exact Product recovery destination", (action) => {
  ...
  it("keeps native observation recovery distinct from server synchronization", () => {
    ...
    expect(
      providerRecoveryDescriptor("rerunNativeProviderTest").message
    ).toMatch(/test-client observation/i);
    expect(
      providerRecoveryDescriptor("runNativeProviderTest").message
    ).not.toMatch(/synchroniz/i);
  });
});
