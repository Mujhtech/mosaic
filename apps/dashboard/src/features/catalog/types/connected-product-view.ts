...
export function providerMappingView(
  ...
  const application = applications.find(
    (item) => item.id === mapping.applicationId
  );
  const environment = environments.find(
    (item) => item.id === mapping.environmentId
  );
  ...
  return {
    ...
    connectionLabel:
      connection?.name ?? mapping.connectionId ?? "No connection assigned",
    connectionLastSuccessfulSyncAt: connection?.lastSuccessfulSyncAt,
    environmentLabel:
      environment?.name ?? mapping.environmentId ?? "No Environment assigned",
