...
export function nativeProviderForPlatform(
...
export function validateNativeProviderMapping(
  input: NativeProviderMappingInput
) {
  ...
  if (
    ...
    errors.googleBasePlanId =
      "Enter the exact base plan ID for this subscription.";
  }

  if (
    input.provider === "google_play" &&
    input.productType === "one_time_non_consumable"
  ) {
    ...
    if (input.googleOfferId?.trim()) {
      errors.googleOfferId =
        "One-time Products cannot use a subscription offer.";
    }
  }
