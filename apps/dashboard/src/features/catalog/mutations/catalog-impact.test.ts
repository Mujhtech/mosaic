...
describe("Catalog impact invalidation", () => {
  ...
  it("invalidates the source plus previous and next replacement targets", async () => {
    const queryClient = new QueryClient()
    const impactedProductIds = replacementImpactProductIds("source", "old_target", "new_target")

    for (const productId of [...impactedProductIds, "unrelated"]) {
      queryClient.setQueryData(catalogKeys.productUsage(productId), { productId })
    }
    ...
    for (const productId of impactedProductIds) {
      expect(queryClient.getQueryState(catalogKeys.productUsage(productId))?.isInvalidated).toBe(
        true,
      )
    }
    expect(queryClient.getQueryState(catalogKeys.productUsage("unrelated"))?.isInvalidated).toBe(
      false,
    )
  })
})
