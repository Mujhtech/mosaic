        ...
        {entries.map((entry) => (
          <EntitlementRow
            ...
            sources={sources.filter(
              (source) =>
                source.sourceId && entry.sourceIds?.includes(source.sourceId)
            )}
          />
        ))}
      </ul>
    ...
    <li className="rounded border p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <StatusPill
            label={accessStateLabel(entry.state)}
            tone={accessStateTone(entry.state)}
          />
      ...
      <p className="mt-2 text-sm leading-6">
        {accessStateExplanation(
          ...
          entry.uncertaintyReason
            ? { reason: entry.uncertaintyReason }
            : undefined
        )}
      </p>
      ...
      entry.sourceCount !== sources.length ? (
        <p className="mt-2 text-muted-foreground text-xs leading-5">
          The snapshot reports {entry.sourceCount} contributing source(s) but
          names {sources.length}. Mosaic renders only the sources the snapshot
          attributes to this entry; the difference is a data problem worth
          reporting rather than a display limit.
        </p>
      ) : null}
      ...
      >
        {open
          ? "Hide contributing sources"
          : `Show all ${sources.length} contributing source(s)`}
      </button>
        ...
        <ul className="mt-3 space-y-2" id={contentId}>
          {sources.length === 0 ? (
            <li className="text-muted-foreground text-sm leading-6">
              The snapshot names no source for this entry. For an inactive entry
              that is the expected shape: nothing is granting it.
            </li>
          ) : (
            sources.map((source) => (
...
function SourceRow({
  ...
  return (
    <li className="rounded border bg-muted/30 p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {source.isTestSource ? (
          <StatusPill label="Test source" tone="attention" />
        ) : null}
      </div>
      ...
      <p className="mt-2 text-muted-foreground text-xs">
        ...
        {sourceEndStatement({
          ...(source.sourceEnd ? { end: source.sourceEnd } : {}),
        })}
      </p>

      <div className="mt-2 flex flex-wrap gap-3 font-semibold text-xs">
        {productHref ? (
