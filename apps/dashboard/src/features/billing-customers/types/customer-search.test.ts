...
describe("customer lookup identifier gating", () => {
  ...
  it.each(["email", "name", "display_name", "receipt", ""])(
    ...
    (identifierType) => {
      expect(
        validateCustomerSearch({ identifierType, identifierValue: "someone@example.com" }),
  ...
  it("requires a non-empty value within the contract's bound", () => {
    expect(
      validateCustomerSearch({ identifierType: "application_user_id", identifierValue: "   " }),
    ).toBe("empty")
    expect(
      validateCustomerSearch({
    ...
    expect(
      validateCustomerSearch({ identifierType: "billing_customer_id", identifierValue: "cus_01" }),
    ).toBeUndefined()
  })
  ...
  it("warns that an application user ID is not an email address", () => {
    ...
    expect(identifierTypeHelp("application_user_id")).toContain("Not an email address")
  })
