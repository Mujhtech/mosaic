...
describe("entitlement access vocabulary", () => {
  ...
  it("names the reason inside the undetermined sentence", () => {
    const sentence = accessStateExplanation("unknown", { reason: "identity_unresolved" })
...
describe("entitlement source and explanation copy", () => {
  it("renders a permanent source as having no finite end rather than as expired", () => {
    ...
    expect(sourceEndStatement({})).toBe("No finite end — this source does not expire")
    expect(sourceEndStatement({ end: "2026-01-01T00:00:00Z" })).toContain("Ends 2026-01-01")
  })
  ...
  it("keeps the cancellation explanation about renewal intent", () => {
    expect(explanationSentence("subscription_cancelled_access_until_period_end")).toContain(
      "Access continues until the validated period end",
    )
  })
})
