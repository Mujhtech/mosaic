...
export type CustomerIdentifierType =
  BillingCustomerLookupRequest["identifierType"];
...
const IDENTIFIER_HELP: Record<CustomerIdentifierType, string> = {
  ...
  billing_customer_id:
    "The Mosaic Billing Customer ID, as it appears on any billing surface.",
  installation_id:
...
const ISSUE_MESSAGES: Record<SearchIssue, string> = {
  ...
  too_long:
    "That identifier is longer than any identifier Mosaic issues or accepts.",
  unsupported_type:
    "Choose which kind of identifier this is before looking it up.",
};
...
export function validateCustomerSearch(input: {
  ...
  if (
    !customerIdentifierTypes.includes(
      input.identifierType as CustomerIdentifierType
    )
  ) {
