...
export function HostedResourceBoundary({
  children,
  state,
}: HostedResourceBoundaryProps) {
  switch (state.kind) {
    case "decision_required":
      return <HostedAccessBanner />;
    case "loading":
      return (
        <LoadingState description={state.description} title={state.title} />
      );
    case "empty":
      return (
        <EmptyState
    ...
    case "permission":
      return (
        ...
        >
          ...
          <h2 className="mt-2 font-semibold text-lg" id="permission-title">
  ...
  return (
    <div className="space-y-3">
      <ErrorState
        ...
        retryLabel="Retry"
        title={
          degraded ? "Mosaic API unreachable" : "Cloud workspace unavailable"
        }
      />
      <ApiErrorDetails details={state.details} />
      <div className="flex flex-wrap items-center gap-2">
        {state.recovery ? (
          <ApiErrorRecoveryAction recovery={state.recovery} />
        ) : null}
...
export function ApiErrorDetails({
  ...
  return (
    <dl className="space-y-1 rounded border border-border bg-muted/30 p-3 text-sm">
      {details.map((entry) => (
        <div
        ...
        >
          ...
          <dd className="break-all font-mono text-xs">{entry.value}</dd>
        </div>
      ))}
    </dl>
  );
 ...
 */
export function ApiErrorRecoveryAction({
  recovery,
}: {
  recovery: ApiErrorRecovery;
}) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <span aria-live="polite" className="text-muted-foreground text-xs">
        {failed
          ? "Copying failed. Select the request ID above to copy it manually."
          : ""}
      </span>
    </div>
  );
