...
export function StoreConnectionsPage({
  organizationId,
  projectId,
}: StoreConnectionsPageProps) {
  ...
  const create = useMutation(
    createStoreCredentialMutationOptions(projectId, queryClient)
  );
  const updateSettings = useMutation(
    updateBillingSettingsMutationOptions(projectId, queryClient)
  );
  ...
  const activeCredentialCount = items.filter(
  ...
  const state = resolveHostedQueryState({
    ...
    loadingDescription:
      "Loading Store Server Credential metadata for this Project.",
    onRetry: () => {
      credentials.refetch();
    ...
    >
      ...
      <BillingEnablementPanel
        ...
        onChange={(nextEnabled) =>
          updateSettings.mutate({ billingEnabled: nextEnabled })
        }
      />
