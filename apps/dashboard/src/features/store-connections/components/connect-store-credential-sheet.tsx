...
export function ConnectStoreCredentialSheet({
  ...
  const storeEnvironment = useStore(
    form.store,
  ...
  return (
    ...
    >
      <SheetTrigger render={<Button type="button" />}>
        Add Store Server Credential
      </SheetTrigger>
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader className="border-b p-5">
          <SheetTitle>Add Store Server Credential</SheetTitle>
          <SheetDescription>
            Mosaic uses this key only to ask the store whether a transaction is
            authentic. It reads; it never changes anything in your store
            account.
          </SheetDescription>
        </SheetHeader>
        <form
        ...
        >
          ...
          <SheetFooter className="border-t p-5">
            ...
            <p className="text-muted-foreground text-xs leading-5">
              For the App Store, the notification endpoint address is shown
              exactly once on the next screen. Copy it before leaving the page.
            </p>
          </SheetFooter>
        </form>
      </SheetContent>
