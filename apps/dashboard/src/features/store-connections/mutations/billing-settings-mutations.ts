 ...
 */
export function updateBillingSettingsMutationOptions(projectId: string, queryClient: QueryClient) {
  return mutationOptions({
    ...
    onSettled: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: billingSettingsKeys.scope(projectId) }),
        // Health reports the same flag, and every billing surface reads it to
        // decide between "not set up" and "no data yet".
        queryClient.invalidateQueries({ queryKey: billingHealthKeys.scope(projectId) }),
      ])
    },
  })
}
