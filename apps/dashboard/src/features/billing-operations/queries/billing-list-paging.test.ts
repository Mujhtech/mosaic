...
const { quarantineRecordsQueryOptions } = await import(
  "@/features/billing-operations/queries/quarantine-queries"
);
const { reconciliationRunsQueryOptions } = await import(
  "@/features/billing-operations/queries/reconciliation-queries"
);
...
describe("billing list paging", () => {
  ...
  it("returns the quarantine cursor instead of swallowing it", async () => {
    listBillingQuarantine.mockResolvedValue({
      data: {
        data: { items: [{ id: "quar_1" }], nextCursor: "cursor_page_2" },
      },
    });
  ...
  it("forwards a quarantine cursor and keeps each page in its own cache entry", async () => {
    ...
    expect(options.queryKey).not.toEqual(
      quarantineRecordsQueryOptions("proj_1", "env_1", { status: "open" })
        .queryKey
    );
  });
  ...
  it("returns the reconciliation cursor and forwards it on the next page", async () => {
    ...
    expect(
      listReconciliationRuns.mock.calls[0]?.[0].query.cursor
    ).toBeUndefined();

    const second = reconciliationRunsQueryOptions(
      "proj_1",
      "env_1",
      "cursor_older"
    );
  ...
  it("returns the timeline cursor and forwards it on the next page", async () => {
    ...
    expect(firstPage.nextCursor).toBe("cursor_older");
    expect(
      listBillingSubscriptionTimeline.mock.calls[0]?.[0].query.cursor
    ).toBeUndefined();

    const second = subscriptionTimelineQueryOptions(
      "proj_1",
      "env_1",
      "sub_1",
      "cursor_older"
  ...
  it("pages the customer subscription list rather than returning a bare array", async () => {
    listBillingCustomerSubscriptions.mockResolvedValue({
      data: {
        data: {
          items: [{ subscriptionInstanceId: "sub_1" }],
          nextCursor: "cursor_older",
        },
      },
    });
    ...
    expect(
      listBillingCustomerSubscriptions.mock.calls[0]?.[0].query.cursor
    ).toBe("cursor_older");
    expect(options.queryKey).not.toEqual(
