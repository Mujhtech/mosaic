...
const TERMINAL_STATUSES: readonly ReconciliationRun["status"][] = [
  "completed",
  "failed",
  "partial",
];

export function reconciliationRunIsTerminal(
  run: Pick<ReconciliationRun, "status"> | undefined
) {
  ...
  return queryOptions({
    ...
    refetchInterval: (query) =>
      (query.state.data?.items ?? []).some(
        (run) => !reconciliationRunIsTerminal(run)
      )
        ? 5000
        : false,
  });
...
export function reconciliationRunQueryOptions(
  ...
  return queryOptions({
    ...
    refetchInterval: (query) =>
      query.state.data && !reconciliationRunIsTerminal(query.state.data)
        ? 5000
        : false,
  });
}
