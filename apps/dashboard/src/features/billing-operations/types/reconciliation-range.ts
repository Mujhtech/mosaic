...
const ISSUE_MESSAGES: Record<ReconciliationRangeIssue, string> = {
  end_in_future:
    "The window must end now or in the past. Stores have no history to reconcile yet.",
...
export function describeReconciliationRangeIssue(
  issue: ReconciliationRangeIssue
) {
...
export function validateReconciliationRange(
  ...
  if (start >= end) {
    issues.push("start_not_before_end");
  }
  if (end - start > MAX_RECONCILIATION_WINDOW_DAYS * DAY_MS) {
    issues.push("window_too_long");
  }
  if (end > (input.now?.getTime() ?? Date.now()) + FUTURE_SKEW_MS) {
    issues.push("end_in_future");
  }
...
export function defaultReconciliationWindow(now = new Date()) {
  ...
  return {
    windowEnd: toLocalInputValue(end),
    windowStart: toLocalInputValue(start),
  };
}
