...
const ASSERTION_LANGUAGE =
  /mark|accept|force|ignore|override|approve|declare|trust/i;
  ...
  it("offers nothing on a closed record, so a closure cannot be reversed by assertion", () => {
    for (const reasonCode of REASON_CODES) {
      expect(
        quarantineRecoveryActions({
          reasonCode,
          status: "closed_after_success",
        })
      ).toEqual([]);
      expect(
        quarantineRecoveryActions({ reasonCode, status: "closed_superseded" })
      ).toEqual([]);
    }
  });
