...
describe("reconciliation window", () => {
  it("accepts a bounded past window", () => {
    expect(
      validateReconciliationRange({ now: NOW, windowEnd: days(0), windowStart: days(7) }),
    ).toEqual([])
  })

  it("rejects an inverted window", () => {
    expect(
      validateReconciliationRange({ now: NOW, windowEnd: days(7), windowStart: days(0) }),
    ).toContain("start_not_before_end")
  })

  it("rejects a zero-length window", () => {
    expect(
      validateReconciliationRange({ now: NOW, windowEnd: days(3), windowStart: days(3) }),
    ).toContain("start_not_before_end")
  })
  ...
  it("rejects a window that ends in the future", () => {
    expect(
      validateReconciliationRange({ now: NOW, windowEnd: days(-2), windowStart: days(3) }),
    ).toContain("end_in_future")
  })

  it("rejects missing and unparseable bounds without throwing", () => {
    expect(validateReconciliationRange({ now: NOW, windowEnd: days(0), windowStart: "" })).toEqual([
      "start_required",
    ])
    expect(
      validateReconciliationRange({ now: NOW, windowEnd: "tomorrow", windowStart: "yesterday" }),
