...
function ObserverHarness() {
  ...
  return (
    <>
      <button
        onClick={() => mutation.mutate("mosaic_secret_first", { onSuccess: reveal })}
        type="button"
      >
        Reveal first
      </button>
      <button
        onClick={() => mutation.mutate("mosaic_secret_second", { onSuccess: reveal })}
        type="button"
      >
        Replace secret
      </button>
      ...
      <output aria-label="revealed secret">{revealed?.secret ?? "empty"}</output>
      <output aria-label="observer secret">{mutation.data?.secret ?? "empty"}</output>
    </>
  )
}
...
describe("API-key mutation secret hygiene", () => {
  it("removes raw secret mutation data immediately after transferring it to reveal state", async () => {
    ...
    const mutation = queryClient.getMutationCache().build(queryClient, {
      ...
      mutationKey: apiKeySecretMutationKey("environment_one", "rotate"),
      onSuccess: (result) => transferApiKeySecret(queryClient, result, reveal, () => undefined),
    })
  ...
  it("sanitizes the active useMutation observer on transfer, replacement, and dismissal", async () => {
    ...
    expect(await screen.findByLabelText("revealed secret")).toHaveTextContent("mosaic_secret_first")
    ...
    await waitFor(() =>
      expect(screen.getByLabelText("revealed secret")).toHaveTextContent("mosaic_secret_second"),
    )
