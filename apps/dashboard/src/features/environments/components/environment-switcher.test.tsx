...
const PATHS = [
  "/orgs/$organizationId",
  // Addresses that name a Project but no Environment. The switcher defends this
  // case rather than assuming every Project surface spells the Environment out.
  "/orgs/$organizationId/projects/$projectId",
  "/orgs/$organizationId/projects/$projectId/billing/connections/$credentialId",
...
describe("EnvironmentSwitcher", () => {
  it("switches Environment without leaving the surface", async () => {
    ...
    renderSwitcher(
      client,
      "/orgs/org_01/projects/prj_01/env/dev/monetization/paywalls"
    );

    fireEvent.click(
      await screen.findByRole("button", {
        name: /Current environment: Development, development/,
      })
    );

    expect(
      await screen.findByRole("menuitem", { name: /Production/ })
    ).toHaveAttribute(
      "href",
      "/orgs/org_01/projects/prj_01/env/prod/monetization/paywalls"
    );
  });
  ...
  it("preserves the deeper surface when switching", async () => {
    ...
    renderSwitcher(
      client,
      "/orgs/org_01/projects/prj_01/env/dev/analytics/funnel"
    );
    fireEvent.click(
    ...
    expect(
      ...
      "href",
      "/orgs/org_01/projects/prj_01/env/prod/analytics/funnel"
    );
  });
  ...
  it("marks the Environment already in scope", async () => {
    ...
    renderSwitcher(
      client,
      "/orgs/org_01/projects/prj_01/env/prod/billing/health"
    );
    fireEvent.click(
      await screen.findByRole("button", {
        name: /Current environment: Production/,
      })
    );
    ...
    expect(
      screen.getByRole("menuitem", { name: /Development/ })
  ...
  it("carries the current search across an in-place switch", async () => {
    ...
    renderSwitcher(
      client,
      "/orgs/org_01/projects/prj_01/env/dev/analytics/funnel?window=28d"
    );
    fireEvent.click(
      await screen.findByRole("button", { name: /Current environment/ })
    );
    ...
    expect(
      ...
      "href",
      "/orgs/org_01/projects/prj_01/env/prod/analytics/funnel?window=28d"
    );
  });
  ...
  it("still answers the question on a surface that carries no Environment", async () => {
    ...
    renderSwitcher(client, "/orgs/org_01/projects/prj_01");

    // The Project's first Environment stands in until a choice is made.
  ...
  it("carries a choice made off-path onto the next Environment surface", async () => {
    ...
    const off = renderSwitcher(client, "/orgs/org_01/projects/prj_01");
    fireEvent.click(
      await screen.findByRole("button", { name: /Current environment/ })
    );
    fireEvent.click(
  ...
  it("lets the address outrank the remembered choice", async () => {
    ...
    renderSwitcher(
      client,
      "/orgs/org_01/projects/prj_01/env/dev/billing/health"
    );
    expect(
      await screen.findByRole("button", {
        name: /Current environment: Development/,
