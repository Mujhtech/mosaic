...
function environment(id: string, key: string, mode: Environment["mode"]): Environment {
...
describe("environment aliases", () => {
  ...
  it("falls back to the key it cannot shorten", () => {
    expect(environmentAlias(environment("env_04", "canary", "staging"))).toBe("canary")
  })
  ...
  it("still accepts an id, so links minted before the change keep working", () => {
    expect(environmentForAlias(all, "env_03")).toBe(production)
  })
