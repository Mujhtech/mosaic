...
export function useActiveEnvironment() {
  ...
  const rememberedId = React.useSyncExternalStore(
    subscribeToActiveEnvironment,
  ...
  const fromMemory = items.find((environment) => environment.id === rememberedId)
  ...
  React.useEffect(() => {
    if (fromPath) rememberEnvironmentId(projectId, fromPath.id)
  }, [projectId, fromPath])
  ...
  const pathFor = React.useCallback(
    (environmentId: string) => {
      ...
      const next = items.find((environment) => environment.id === environmentId)
      if (!next) return null
  ...
  const select = React.useCallback(
    (environmentId: string) => {
      ...
      if (target) void navigate({ search: true, to: target })
    },
