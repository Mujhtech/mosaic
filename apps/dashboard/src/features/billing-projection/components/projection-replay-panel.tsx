...
interface ProjectionReplayPanelProps {
  ...
  canManage: boolean
  membersHref: string
  onReplay: (request: CreateProjectionReplayRequest) => Promise<ProjectionReplayResult | undefined>
}
...
export function ProjectionReplayPanel({
  ...
  const [result, setResult] = useState<ProjectionReplayResult | undefined>(undefined)
    ...
    try {
      ...
      setError(
        describeReplayRefusal(cause) ??
          (cause instanceof Error ? cause.message : "Mosaic could not run this replay."),
      )
    } finally {
      setBusy(false)
    }
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        <Field>
          <FieldLabel htmlFor="replay-subscription">Subscription Instance</FieldLabel>
          <Input
            id="replay-subscription"
            onChange={(event) => {
              const value = event.currentTarget.value || undefined
              setRequest((current) => ({ ...current, subscriptionInstanceId: value }))
            }}
        ...
        <Field>
          ...
          <Input
            ...
            onChange={(event) => {
              const value = event.currentTarget.value || undefined
              setRequest((current) => ({ ...current, billingCustomerId: value }))
            }}
            value={request.billingCustomerId ?? ""}
          />
        </Field>
        <Field>
          <FieldLabel htmlFor="replay-window-start">Fact window start</FieldLabel>
          <Input
        ...
        <Field>
          ...
          />
          <FieldDescription>
            The window bounds facts, not lineage creation: a scope is in scope when it holds a fact
            whose effective or recorded time falls inside it.
          </FieldDescription>
        </Field>
        <Field>
          <FieldLabel htmlFor="replay-rule-version">Projection rule version</FieldLabel>
          <Input
            ...
            onChange={(event) => {
              ...
              setRequest((current) => ({ ...current, projectionRuleVersion: value }))
            }}
            type="number"
          ...
          <FieldDescription>
            ...
            {activeRuleVersion === undefined ? "" : `(${activeRuleVersion})`}. A version this build
            does not derive under is refused rather than approximated.
          </FieldDescription>
        </Field>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <Button disabled={busy || Boolean(scopeIssue)} onClick={() => void run()} type="button">
          {busy ? "Replaying…" : "Run replay"}
        </Button>
        <p className="text-muted-foreground text-xs leading-5">{NO_AUTO_PROMOTION_NOTE}</p>
      </div>
      ...
      {result ? (
        <div className="mt-4 rounded border p-4">
          <p className="text-sm leading-6 font-semibold">{replaySummary(result)}</p>
          <dl className="mt-2">
            <DefinitionRow
          ...
          <ul className="mt-3 space-y-3">
            {(result.outcomes ?? []).map((outcome) => (
              <li className="rounded border p-3" key={outcome.projectionScopeKey}>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-mono text-xs break-all">{outcome.projectionScopeKey}</span>
                  <StatusPill
