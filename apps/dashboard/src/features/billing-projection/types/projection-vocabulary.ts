...
export function replayComparisonExplanation(value: string | undefined) {
  if (!value) {
    return "Mosaic did not report a comparison for this scope.";
  }
  return (
...
export function validateReplayScope(
  ...
  if (!(hasInstance || hasCustomer || hasStart || hasEnd)) {
    return "unbounded";
  }
...
export function replaySummary(result: ProjectionReplayResult | undefined) {
  ...
  if (replayed === 0) {
    return "No scope in this Environment matched the bounds you gave.";
  }
  if (changed === 0) {
