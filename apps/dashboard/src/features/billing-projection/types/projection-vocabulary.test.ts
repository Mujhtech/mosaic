...
describe("projection replay scope", () => {
  ...
  it("accepts a single named scope", () => {
    expect(
      validateReplayScope({ subscriptionInstanceId: "sub_01" })
    ).toBeUndefined();
    expect(
      validateReplayScope({ billingCustomerId: "cus_01" })
    ).toBeUndefined();
  });

  it("rejects a half-specified or inverted fact window", () => {
    expect(validateReplayScope({ windowStart: "2026-07-01T00:00:00Z" })).toBe(
      "window_incomplete"
    );
    expect(
      validateReplayScope({
...
describe("projection replay outcome copy", () => {
  it("explains the unimplemented rule version rather than showing a bare 422", () => {
    ...
    expect(message).toContain(
      "does not derive under the requested projection rule version"
    );
    expect(message).toContain(
      "indistinguishable from a genuine determinism result"
    );
  ...
  it("leaves other failures to the ordinary error path", () => {
    expect(
      describeReplayRefusal({ code: "not_found", status: 404 })
    ).toBeUndefined();
  });

  it("states that an identical replay wrote nothing and a changed one preserved the old snapshot", () => {
    expect(replayComparisonExplanation("unchanged")).toContain(
      "Nothing was written"
    );
