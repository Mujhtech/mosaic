...
function resolveRequestUrl(path: string, baseUrl: URL) {
  if (
    ABSOLUTE_SCHEME.test(path) ||
    path.startsWith("/") ||
    path.includes("\\")
  ) {
    throw new TypeError(
      "API request paths must be relative to the configured base URL."
    );
  }

  const pathEnd = path.search(QUERY_OR_FRAGMENT);
  ...
  if (decodedPath.replaceAll("\\", "/").split("/").includes("..")) {
    throw new TypeError(
      "API request paths must remain inside the configured base URL."
    );
  }
    ...
    async request<T>(path: string, options: ApiRequestOptions = {}) {
      ...
      try {
        ...
        if (isAbortError(error, signal)) {
          // biome-ignore lint/style/useErrorCause: ApiRequestAbortedError chains the cause through its constructor
          throw new ApiRequestAbortedError(correlationId, error);
        }

        // biome-ignore lint/style/useErrorCause: ApiNetworkError chains the cause through its constructor
      ...
      const responseCorrelationId =
        responseHeaderCorrelationId ?? correlationId;
