...
describe("createApiClient", () => {
  it("sends auth and request context and unwraps the response envelope", async () => {
    ...
    expect(String(requestUrl)).toBe(
      "https://api.example.test/api/v1/dashboard/health"
    );
  ...
  it("distinguishes cancellation from a network failure", async () => {
    ...
    abortedFetch.mockRejectedValueOnce(
    ...
    await expect(abortedClient.request("projects")).rejects.toBeInstanceOf(
      ApiRequestAbortedError
    );
    ...
    await expect(failedClient.request("projects")).rejects.toBeInstanceOf(
      ApiNetworkError
    );
  });
});
