...
function normalizeGeneratedError(
  ...
  const fallbackRequestId = request?.headers.get("X-Request-ID") ?? createRequestId()
  const envelope = isRecord(error) && isRecord(error.error) ? error.error : undefined
  const requestId =
    response?.headers.get("X-Request-ID") ??
    (typeof envelope?.requestId === "string" ? envelope.requestId : fallbackRequestId)

  if (!response) {
    if (request?.signal.aborted) return new ApiRequestAbortedError(requestId, error)
