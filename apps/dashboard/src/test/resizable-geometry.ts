...
function groupWidth(group: HTMLElement) {
  if (group.dataset.geometryWidth) {
    return Number(group.dataset.geometryWidth);
  }
  return group.id === "studio-horizontal-group" ||
...
function groupHeight(group: HTMLElement) {
  if (group.dataset.geometryHeight) {
    return Number(group.dataset.geometryHeight);
  }
  if (group.id === "studio-root-group") {
    return 852;
  }
  if (group.id === "studio-horizontal-group") {
    return 632;
  }
  return 800;
}
...
function panelAxisSize(element: HTMLElement, axis: "horizontal" | "vertical") {
  ...
  if (Number.isFinite(numeric) && numeric > 0) {
    if (basis.endsWith("%")) {
      return (total * numeric) / 100;
    }
    if (basis.endsWith("vh")) {
      return (window.innerHeight * numeric) / 100;
    }
    if (basis.endsWith("vw")) {
  ...
  if (flexGrow === 1) {
    const occupied = Array.from(group.children).reduce((sum, sibling) => {
      ...
      if (!Number.isFinite(siblingNumeric) || siblingNumeric <= 0) {
        return sum;
      }
      if (siblingBasis.endsWith("%")) {
        return sum + (total * siblingNumeric) / 100;
      }
      if (siblingBasis.endsWith("vh")) {
        return sum + (window.innerHeight * siblingNumeric) / 100;
      }
...
function elementAxisOffset(
  ...
  if (parent.hasAttribute("data-group") && groupOrientation(parent) !== axis) {
    return 0;
  }
  let offset = 0;
  for (const sibling of Array.from(parent.children)) {
    ...
    offset +=
      axis === "horizontal" ? sibling.offsetWidth : sibling.offsetHeight;
  }
  return offset;
}
