...
async function connect(
  ...
  sessionId,
  receivedMessages,
  protocols = PREVIEW_SUBPROTOCOL
) {
  const target = sessionId
    ? `${url}?role=studio&sessionId=${encodeURIComponent(sessionId)}`
    : url;
  ...
  assert.equal(
    socket.protocol,
    Array.isArray(protocols) && protocols.includes(PREVIEW_SUBPROTOCOL)
      ? PREVIEW_SUBPROTOCOL
      : protocols
  );
  return socket;
...
test("negotiates Local Preview 0.4 and relays a canonical Protocol 0.4 draft", async () => {
  ...
  const connected = fixture.find(
    (message) => message.type === "previewClientConnected"
  );
  const capability = fixture.find(
  ...
  assert.ok(
    studioMessages.every((message) => message.previewProtocolVersion === "0.4")
  );

  studio.send(JSON.stringify(draft));
  await waitForMessages(clientMessages, 1);
  assert.equal(clientMessages[0].type, "draftUpdated");
  assert.equal(clientMessages[0].payload.document.schemaVersion, "0.4");
...
test("relays canonical messages only within the same local preview session", async () => {
  ...
  const otherMessages = [];
  otherStudio.on("message", (payload) =>
    otherMessages.push(payload.toString())
  );
  ...
  nativeClient.send(
    JSON.stringify(
      heartbeat(connected.sessionId, 1, connected.payload.client.clientId)
    )
  );
...
test("notifies Studio when a connected native client closes", async () => {
  ...
  assert.equal(
    JSON.parse(connectedPayload.toString()).type,
  ...
  assert.equal(
    disconnected.payload.clientId,
    connected.payload.client.clientId
  );
...
test("rejects messages with unknown fields and requires the exact subprotocol", async () => {
  ...
  invalid.send(
    JSON.stringify({ ...heartbeat("session_local_01"), unexpected: true })
  );
...
test("keeps the replacement socket active when a stable client identity reconnects", async () => {
  ...
  replacement.send(
    JSON.stringify({ ...connected, messageId: "msg_replacement_connected" })
  );
  replacement.send(
    JSON.stringify({ ...capability, messageId: "msg_replacement_capability" })
  );
  await waitForMessages(studioMessages, 4);
  await originalClosed;
  await new Promise((resolve) => setTimeout(resolve, 20));
  assert.equal(
    studioMessages.filter(
      (message) => message.type === "previewClientDisconnected"
    ).length,
    0
  );
  ...
  replacement.send(
    JSON.stringify(
      heartbeat(connected.sessionId, 9, connected.payload.client.clientId)
    )
  );
