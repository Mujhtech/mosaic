...
export const PREVIEW_SUBPROTOCOLS = Object.freeze(
  localPreviewVersionPreference.map(
...
const VERSION_BY_SUBPROTOCOL = new Map(
  Object.entries(localPreviewWebSocketProtocols).map(([version, protocol]) => [
    protocol,
    version,
  ])
);
...
function rejectionReason(code) {
  if (code === "preview.incompatibleSchemaVersion") {
    return "unsupportedSchemaVersion";
  }
  if (code === "preview.unsupportedCapability") {
    return "unsupportedCapability";
  }
  if (code === "preview.documentTooLarge") {
...
export function createPreviewRelay({
  ...
  if (host !== "127.0.0.1" && host !== "::1" && host !== "localhost") {
    throw new Error(
      "The Phase 2 preview relay may only bind to a loopback interface."
    );
  }
  ...
  const server = new WebSocketServer({
    ...
    handleProtocols(protocols) {
      return (
        PREVIEW_SUBPROTOCOLS.find((protocol) => protocols.has(protocol)) ??
        false
      );
    },
    verifyClient(info, done) {
      ...
      if (
        !PREVIEW_SUBPROTOCOLS.some((protocol) =>
  ...
  function replayClients(socket, sessionId) {
    ...
    for (const client of cache.values()) {
      if (client.connected) {
        sendCanonical(socket, meta, client.connected);
      }
      if (client.capability) {
        sendCanonical(socket, meta, client.capability);
      }
    }
  }
  ...
  function disconnectClient(socket, reason) {
    const meta = metadata.get(socket);
    if (!(meta?.clientId && meta.sessionId) || meta.disconnected) {
      return;
    }
    meta.disconnected = true;
    const cache = cachedClients.get(meta.sessionId);
    if (cache?.get(meta.clientId)?.socket !== socket) {
      return;
    }
    cache?.delete(meta.clientId);
    if (cache?.size === 0) {
      cachedClients.delete(meta.sessionId);
    }
  ...
  server.on("connection", (socket, request) => {
    ...
    const meta = {
      ...
      sessionId:
        isStudio && querySession?.startsWith("session_") ? querySession : null,
      clientId: null,
    ...
    if (
      !protocolVersion ||
      (requestedRole &&
        requestedRole !== "studio" &&
        requestedRole !== "client") ||
      (isStudio && !meta.sessionId)
    ) {
      socket.close(
        1008,
        "A Studio connection requires a valid role and sessionId"
      );
      return;
    }
    if (meta.role === "studio" && meta.sessionId) {
      replayClients(socket, meta.sessionId);
    }
