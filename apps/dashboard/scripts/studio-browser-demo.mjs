...
const debuggingEndpoint =
  process.env.MOSAIC_CHROME_DEBUG_URL ?? "http://127.0.0.1:9223";
const startedAt = Date.now();
const finalHeadline = "Edit once, preview natively everywhere";
const artifactsDirectory = await mkdtemp(
  join(tmpdir(), "mosaic-phase2c-browser-demo-")
);
const validationScreenshot = join(
  artifactsDirectory,
  "studio-validation-error.png"
);
const finalScreenshot = join(artifactsDirectory, "studio-final.png");

const targets = await fetch(`${debuggingEndpoint}/json/list`).then((response) =>
  response.json()
);
const target = targets.find(
  (entry) => entry.type === "page" && entry.url.includes("/studio")
);
if (!target?.webSocketDebuggerUrl) {
  throw new Error(
    `No /studio Chrome target is available at ${debuggingEndpoint}.`
  );
}
...
await waitFor("editor shell", async () =>
  (await bodyText()).includes("Paywall order")
);
...
await waitFor(
  ...
  async () =>
    (await elementChecked("#property-monthly-card-initialProductCardId")) !==
    null
);
...
await waitFor(
  ...
  async () =>
    (await elementChecked("#property-yearly-card-initialProductCardId")) ===
    false
);
...
await waitFor("Sheet destination", async () =>
  (await bodyText()).includes("Sheet · screen-2")
);
...
for (const clientLabel of [
  ...
  if (!finalBody.includes(clientLabel)) {
    throw new Error(
      `Studio did not display the connected client: ${clientLabel}.`
    );
  }
}
...
if (!validation.ok) {
  throw new Error("The browser-exported document is not Protocol 0.2 valid.");
}

const initialScreen = exported.screens.find(
  (screen) => screen.id === exported.initialScreenId
);
...
if (subtitleIndex < 0 || headlineIndex !== subtitleIndex + 1) {
  throw new Error(
    "The exported document does not preserve the browser reorder."
  );
}
if (
  ...
  throw new Error(
    "The exported Product Selector does not select the authored yearly Product Card."
  );
}
if (sheet?.presentation?.type !== "sheet" || !sheetNavigation) {
  throw new Error(
    "The exported document does not preserve the new Sheet and its navigation edge."
  );
}
...
async function clickRoleContaining(role, text) {
  ...
  if (!clicked) {
    throw new Error(`Could not find ${role} containing: ${text}.`);
  }
}
...
async function clickCanvasComponent(componentId) {
  ...
  if (!clicked) {
    throw new Error(`Could not select canvas component: ${componentId}.`);
  }
}
...
async function moveSelectedLayerDown() {
  ...
  if (!moved) {
    throw new Error("Could not move the selected layer down.");
  }
}
...
async function setElementValue(selector, value) {
  ...
  if (!changed) {
    throw new Error(`Could not set element value: ${selector}.`);
  }
...
async function waitForExport() {
  ...
  while (Date.now() < exportDeadline) {
    ...
    if (files.length === 1) {
      return join(artifactsDirectory, files[0]);
    }
  ...
  return {
    ...
    async evaluate(expression) {
      ...
      if (response.exceptionDetails) {
        throw new Error(
          response.exceptionDetails.text ?? "Chrome evaluation failed."
        );
      }
