...
const endpoint =
  process.env.MOSAIC_PREVIEW_ENDPOINT ?? "ws://127.0.0.1:4317/preview";
const sessionId = process.env.MOSAIC_PREVIEW_SESSION_ID ?? "session_local_01";
const revisionSequence = Number.parseInt(
  process.env.MOSAIC_DEMO_SEQUENCE ?? "2",
  10
);
...
const holdMilliseconds = Number.parseInt(
  process.env.MOSAIC_DEMO_HOLD_MS ?? "0",
  10
);
...
const expectedRenderers = new Set([
  "mosaic.flutter",
  "mosaic.ios",
  "mosaic.android",
]);
...
socket.on("message", (frame) => {
  ...
  if (message.type === "previewClientConnected") {
    ...
    diagnostics.push({
      ...
      codes: (message.payload.warnings ?? message.payload.errors ?? []).map(
        (entry) => entry.code
      ),
    });
  }
...
await waitUntil(() => {
  ...
  return [...expectedRenderers].every((renderer) =>
    readyRenderers.has(renderer)
  );
});
...
const draft = structuredClone(
  fixtures.find((message) => message.type === "draftUpdated")
);
...
for (const message of [commerce, draft]) {
  ...
  if (!result.ok) {
    throw new Error(`Demo message ${message.type} is not protocol-valid.`);
  }
...
await waitUntil(() => {
  const acknowledgedRenderers = new Set(
    [...acknowledgements]
      .map((clientId) => clients.get(clientId)?.renderer.id)
      .filter(Boolean)
  );
  return [...expectedRenderers].every((renderer) =>
    acknowledgedRenderers.has(renderer)
  );
});
...
while (Date.now() < holdDeadline) {
  ...
  for (const client of result.clients) {
    ...
    if (!validatePreviewMessage(heartbeat).ok) {
      throw new Error("Demo heartbeat is not valid.");
    }
