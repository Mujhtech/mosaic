...
import {
  applyV03NavigationAction,
  expectedV03DocumentCapabilities,
  loadProtocolV03Artifacts,
  orderedV03Capabilities,
  protocolV03AuthoringWarnings,
  protocolV03RuntimeDiagnostics,
  interpolateProductText,
  resolveProductBadgeStyle,
  resolveProductSelectorSelection,
  resolveProductCardStyle,
  resolveV03AxisSizing,
  resolveV03BackgroundToken,
  resolveV03MediaBackgroundFallback,
  resolveV03CountdownState,
  ...
  v03AccessibilityAnnouncement,
  v03ResolvedCatalogStrings,
  validateAccessibilityAnnouncementVectors,
  protocolV03Paths,
  readV03Json,
  reservedAccessibilityKeys,
  resolveV03RatingAnnouncement,
  v03RatingPoints,
...
function errors(input) {
  return validateProtocolV03(input);
}

function node(document, id) {
  const entry = walkV03DocumentNodes(document).find(
...
function refreshCapabilities(input) {
  input.document.compatibility.requiredCapabilities = orderedV03Capabilities(
    input.document,
    input.paywallSchema,
  );
}
...
test("Protocol 0.3 release-candidate fixtures validate without approving the version", () => {
  ...
  assert.deepEqual(validateCanonicalV03Coverage(input.document), []);
  assert.deepEqual(validateV03JsonFormatting(), []);
  // Paywall Protocol 0.3 is a release candidate, not an approved contract. The
  // cross-platform implementability gate in
  // docs/protocol/release-approval-process.md cannot have passed: the Flutter,
  // SwiftUI, and Compose renderers for tabs, timeline, award, and socialProof
  // are still being written, and Studio cannot yet author them. Asserting the
  // status here keeps that honest -- flipping the manifest without clearing the
  // checklist would fail this test rather than pass silently.
  ...
  // approved while the contract it carries is a candidate.
  assert.equal(
    loadPreviewV03Artifacts().localPreviewManifest.status,
    "releaseCandidate",
  );
});

test("the canonical fixture declares exactly every used 0.3 capability", () => {
  const input = artifacts();
  assert.deepEqual(
    new Set(
      input.document.compatibility.requiredCapabilities.map(({ name }) => name),
    ),
    expectedV03DocumentCapabilities(input.document),
  );
  assert.deepEqual(
    expectedV03DocumentCapabilities(input.document),
  ...
  const derived = expectedV03DocumentCapabilities(presentationOnly);
...
test("Stack uses gap and distribution, permits nested emptiness, and keeps a nonempty root", () => {
  ...
  const directions = new Set(
    walkV03DocumentNodes(input.document)
...
test("sizing is uniform on both axes and unbounded Fill safely resolves to Fit", () => {
  ...
  const bounded = resolveV03AxisSizing("fill", {
    axis: "height",
    bounded: true,
    componentId: "headline",
  });
  assert.deepEqual(bounded, { value: "fill", diagnostic: null });
  const unbounded = resolveV03AxisSizing("fill", {
    axis: "height",
    bounded: false,
    componentId: "headline",
  });
...
test("RC4 design tokens are category-scoped, referential, and cycle-safe", () => {
  ...
  assert.deepEqual(
    resolveV03BackgroundToken(input.document, {
...
test("RC4 gradients, decorative media fallback, and remote asset safety are deterministic", () => {
  ...
  const poster = resolveV03MediaBackgroundFallback(
    input.document,
  ...
  const color = resolveV03MediaBackgroundFallback(
    input.document,
    { type: "backgroundToken", id: "sheet-video" },
    [],
  );
...
test("Product Selector and Feature List own their current 0.3 layout fields", () => {
  ...
  assert.deepEqual(protocolV03AuthoringWarnings(input.document), []);
...
test("Product Card and Product Badge Selected are recursively partial box-style overlays", () => {
  ...
  assert.deepEqual(errors(input), []);
  assert.equal(
    protocolV03AuthoringWarnings(input.document).some(
  ...
  assert.equal(
    protocolV03AuthoringWarnings(lowContrast.document).some(
      ({ code, field }) =>
  ...
  const unreadableWarnings = protocolV03AuthoringWarnings(unreadable.document);
  assert.equal(
  ...
  for (const readable of ["surface.elevated", "#11223380"]) {
    ...
    node(skipped.document, "plans-yearly-plan-card").children[0].typography.color =
      readable;
    assert.equal(
      protocolV03AuthoringWarnings(skipped.document).some(
...
test("Product Selector owns ordered real cards with closed Product Card and Badge structure", () => {
  ...
  assert.equal(
    selector.cards.every(
      (card) =>
        walkV03DocumentNodes(input.document).find(
...
test("product templates use only name and price inside card content and fail closed", () => {
  const input = artifacts();
  assert.equal(
    expectedV03DocumentCapabilities(input.document).has(
...
test("Countdown unit ordering and controlled-clock completion are deterministic", () => {
  ...
  assert.equal(
    resolveV03CountdownState(active, "2029-01-01T00:00:00Z")
      .remainingMilliseconds > 0,
    true,
  );
  const expired = node(input.expiredCountdownDocument, "expired-offer-countdown");
  const completed = resolveV03CountdownState(
    expired,
  ...
  for (const endsAt of ["not a timestamp", "", undefined]) {
    ...
    for (const resolve of [resolveV03CountdownState, resolveBrowserCountdownState]) {
      assert.throws(
...
test("a hidden purchase target remains valid but disables purchase with a safe diagnostic", () => {
  ...
  assert.deepEqual(protocolV03RuntimeDiagnostics(document), expected);
...
test("an accepted revision resets Switch, Tabs, Carousel, and navigation runtime state", () => {
  ...
  assert.deepEqual(runtimeStateForAcceptedV03Revision(input.document), expected);
...
test("navigation history is runtime-only and root Navigate Back is a safe no-op", () => {
  ...
  const forward = applyV03NavigationAction(initial, forwardAction);
  ...
  const back = applyV03NavigationAction(forward.state, { type: "navigateBack" });
  assert.deepEqual(back.state, initial);
  assert.equal(back.diagnostic, null);

  const rootBack = applyV03NavigationAction(initial, { type: "navigateBack" });
...
test("Local Preview still lands on 0.3 for a 0.3 client and withholds from an incompatible one", () => {
  ...
  const supported = negotiateLocalPreviewVersion(["0.3"], ["0.3"]);
  assert.equal(supported.selectedVersion, "0.3");
  assert.deepEqual(
    supported,
    negotiateBrowserPreview(["0.3"], ["0.3"]),
  );
  const incompatible = negotiateLocalPreviewVersion(["0.3"], ["0.1"]);
  assert.deepEqual(
    incompatible,
    negotiateBrowserPreview(["0.3"], ["0.1"]),
  );
...
test("browser validation dispatches the sole Protocol and Local Preview 0.3 contract", () => {
  const preview = loadPreviewV03Artifacts();
  ...
  assert.equal(
    canonicalSchemasByVersion["0.3"].paywall.$id,
...
test("every invalid-fixture corpus declares a case-count floor it actually meets", () => {
  ...
  const paywallTarget = rejectionLayerTargets.find(
    (target) => target.directory === "fixtures/v0.3/invalid",
  );
  assert.ok(paywallTarget, "the paywall corpus must be registered");
  assert.ok(paywallTarget.minimumCases >= 12);
  assert.ok(
...
test("the browser and Node reference implementations derive the same capabilities", () => {
  ...
  for (const document of [
    ...
    assert.deepEqual(
      ...
      orderedV03Capabilities(document, input.paywallSchema),
    );
  }
});
...
test("runtime diagnostics refuse a document that is not Protocol 0.3", () => {
  ...
  assert.throws(
    () => paywallRuntimeDiagnostics({ schemaVersion: "0.2" }),
    TypeError,
  );
  assert.deepEqual(
    paywallRuntimeDiagnostics(artifacts().navigationOnlyDocument),
    [],
  );
});
...
test("a rating announces points, not steps, from a reserved localized template", () => {
  ...
  assert.equal(v03RatingPoints(half), "4.5");
  assert.notEqual(v03RatingPoints(half), String(half.value));
  assert.equal(v03RatingPoints({ ...half, value: 10 }), "5");
  assert.equal(v03RatingPoints({ ...half, value: 0 }), "0");
  assert.equal(v03RatingPoints({ ...half, value: 1 }), "0.5");
  assert.equal(v03RatingPoints({ symbol: "star", value: 3, maximum: 5, step: "whole" }), "3");
  ...
  assert.equal(
    resolveV03RatingAnnouncement(half, "{{ rating.maximum }} 中 {{ rating.value }}"),
    "5 中 4.5",
  );
  ...
  assert.throws(
    () => resolveV03RatingAnnouncement(half, "{{ rating.value }} stars"),
    TypeError,
  );
  assert.throws(() => resolveV03RatingAnnouncement(half, undefined), TypeError);

  assert.deepEqual(validateRatingAnnouncementVectors(), []);
  const vectors = readV03Json(protocolV03Paths.ratingAnnouncementVectors);
...
test("the exported derivation surface is the one derivation actually uses", () => {
  ...
  assert.equal(browserPaywallContractVersion, "0.3");
  ...
  for (const document of [
    ...
    assert.deepEqual(
      [...expectedDocumentCapabilities(document)],
      orderedV03Capabilities(document, input.paywallSchema).map(
    ...
    for (const name of expectedDocumentCapabilities(document)) {
      assert.ok(
  ...
  const componentTypes = new Set(
    walkV03DocumentNodes(input.document).map(({ node }) => node.type),
  );
...
test("selection-state styling is gated by the component capability that requires it", () => {
  ...
  // `style.productCardStates` is exactly co-derived with the three components
  // that require it. Pinning that keeps a legacy signal from quietly acquiring
  // a second meaning -- such as silently starting to gate Tabs.
  for (const document of [
    input.document,
    input.edgeDocument,
    input.hiddenPurchaseTargetDocument,
    ...
    const derived = new Set(expectedDocumentCapabilities(document));
    const componentPresent = [
      "component.productSelector",
      "component.productCard",
      "component.productBadge",
    ].some((name) => derived.has(name));
    assert.equal(
      derived.has("style.productCardStates"),
      componentPresent,
      `style.productCardStates is not co-derived in ${document.id}`,
    );
  }
  ...
  const tabsOnly = new Set(expectedDocumentCapabilities(tabsOnlyDocument));
...
test("announced segments are separate elements and are never joined", () => {
  ...
  const strings = v03ResolvedCatalogStrings(document.localization, "en");

  const social = v03AccessibilityAnnouncement(node(document, "rated-review"), {
    strings,
  });
  ...
  const unrated = v03AccessibilityAnnouncement(node(document, "analyst-note"), {
    strings,
  });
  assert.deepEqual(
  ...
  const award = v03AccessibilityAnnouncement(node(document, "editor-award"), {
    strings,
  });
  assert.deepEqual(
  ...
  assert.deepEqual(
    v03AccessibilityAnnouncement(node(document, "press-award"), {
      strings,
    }).elements.map((element) => element.segment),
    ["title"],
  );

  const timeline = v03AccessibilityAnnouncement(node(document, "trial-timeline"), {
    strings,
  });
  ...
  assert.throws(
    () => v03AccessibilityAnnouncement(node(document, "headline"), { strings }),
    TypeError,
  );
  assert.throws(
    () => v03AccessibilityAnnouncement(node(document, "rated-review"), {}),
    TypeError,
  );
});
...
test("a busy Button carries its state as a value and keeps its authored name", () => {
  ...
  const idle = v03AccessibilityAnnouncement(purchase, { strings, state: "idle" });
  const busy = v03AccessibilityAnnouncement(purchase, {
    strings,
    state: "inProgress",
  });
  ...
  const german = v03AccessibilityAnnouncement(purchase, {
    strings: v03ResolvedCatalogStrings(document.localization, "de"),
    state: "inProgress",
  });
  ...
  assert.throws(
    () => v03AccessibilityAnnouncement(purchase, { strings }),
    TypeError,
  );
  assert.throws(
    () => v03AccessibilityAnnouncement(purchase, { strings, state: "busy" }),
    TypeError,
  );

  assert.deepEqual(validateAccessibilityAnnouncementVectors(), []);
  const vectors = readV03Json(
    protocolV03Paths.accessibilityAnnouncementVectors,
  );
  assert.ok(ACCESSIBILITY_ANNOUNCEMENT_CASE_FLOOR >= 11);
  assert.ok(
    validateAccessibilityAnnouncementVectors({ ...vectors, cases: [] }).some(
      (error) => error.includes("floor"),
    ),
  );
  ...
  target.separator = ". ";
  assert.ok(
    validateAccessibilityAnnouncementVectors(joined).some((error) =>
  ...
  for (const entry of vectors.cases) {
    ...
    assert.deepEqual(
      ...
      v03AccessibilityAnnouncement(target, {
