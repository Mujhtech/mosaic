...
export const localeResolutionV03Root = resolve(toolsDirectory, "..");
export const localeResolutionV03Paths = Object.freeze({
  fixture: resolve(localeResolutionV03Root, "fixtures/v0.3/locale-resolution.json"),
});

export function loadLocaleResolutionV03Artifacts() {
  return { fixture: JSON.parse(readFileSync(localeResolutionV03Paths.fixture, "utf8")) };
}
 ...
 * `zh-CN` is not a candidate. See `docs/protocol/v0.3.md`.
 */
/**
...
export function localeCandidatesV03(localization, requestedLocale) {
 ...
 */
export function resolveLocaleCatalogV03(localization, requestedLocale) {
  const declared = declaredCatalogKeys(localization);
  const candidates = localeCandidatesV03(localization, requestedLocale);
 ...
 */
const LOCALE_RESOLUTION_V03_CASE_FLOOR = 13;

export function validateLocaleResolutionV03Artifacts(artifacts = loadLocaleResolutionV03Artifacts()) {
  const errors = [];
  const { localization, cases } = artifacts.fixture;
  if (!Array.isArray(cases) || cases.length < LOCALE_RESOLUTION_V03_CASE_FLOOR) {
    errors.push(`locale resolution corpus holds ${Array.isArray(cases) ? cases.length : "no array"} cases, below the floor of ${LOCALE_RESOLUTION_V03_CASE_FLOOR}; a conformance loop over a shrunken corpus passes vacuously`);
  }
  ...
  for (const testCase of Array.isArray(cases) ? cases : []) {
    const actual = resolveLocaleCatalogV03(localization, testCase.requested);
...
export function validateLocaleResolutionV03JsonFormatting() {
  return [localeResolutionV03Paths.fixture].flatMap((path) => {
    const source = readFileSync(path, "utf8");
    return source === `${JSON.stringify(JSON.parse(source), null, 2)}\n`
      ? []
      : [`${relative(localeResolutionV03Root, path)} is not canonical JSON`];
  });
}
