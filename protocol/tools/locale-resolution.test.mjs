...
test("the shared locale-resolution corpus matches the reference resolver", () => {
  assert.deepEqual(validateLocaleResolutionArtifacts(), []);
  assert.deepEqual(validateLocaleResolutionJsonFormatting(), []);
});
...
test("a shrunken locale-resolution corpus fails instead of conforming over nothing", () => {
  ...
  const artifacts = loadLocaleResolutionArtifacts();
  for (const cases of [[], artifacts.fixture.cases.slice(0, 2), undefined]) {
    ...
    assert.ok(
      validateLocaleResolutionArtifacts(shrunken).some((error) =>
...
test("a localization block that never passed validation is refused, not resolved to null", () => {
  ...
  const { localization } = loadLocaleResolutionArtifacts().fixture;
  const refusals = [
  ...
  for (const [broken, code] of refusals) {
    for (const call of [
      () => localeCandidates(broken, "pt-BR"),
      () => resolveLocaleCatalog(broken, "pt-BR"),
    ]) {
      assert.throws(
        call,
...
test("catalog lookup canonicalizes the runtime side to the authored grammar's casing", () => {
  ...
  const localization = loadLocaleResolutionArtifacts().fixture.localization;
  for (const requested of ["pt-BR", "pt_br", "PT-br", "PT_BR", "pt-BR-u-nu-latn"]) {
    assert.equal(resolveLocaleCatalog(localization, requested).resolved, "pt-BR", requested);
  }
});
...
test("a script subtag reaches the base language, not a language-region catalog", () => {
  ...
  const localization = loadLocaleResolutionArtifacts().fixture.localization;
  assert.deepEqual(localeCandidates(localization, "zh-Hans-CN"), ["zh-Hans-CN", "zh", "en-GB", "en"]);
  assert.equal(resolveLocaleCatalog(localization, "zh-Hans-CN").resolved, "zh");
});
