...
export const paywallRulesRoot = resolve(toolsDirectory, "..");

/**
 * Corpora that pin behaviour the paywall contract carries at every version.
 *
 * They live beside the current contract's fixtures rather than in a directory
 * of their own, because they name components and localization keys the current
 * canonical document declares. They are not duplicated per version: two copies
 * of one unchanged rule is exactly the drift this repository has been bitten by.
 */
export const paywallRulesPaths = Object.freeze({
  ratingAnnouncementVectors: resolve(
    paywallRulesRoot,
    "fixtures/v0.4/rating-announcement.json",
  ),
});
...
export function readPaywallJson(filePath) {
...
export function expectedDocumentCapabilities(document) {
  ...
  for (const { node } of entries) {
    ...
    // Motion is a property of a node rather than a container, so the derivation
    // reads it exactly where it is authored.
...
export function orderedCapabilities(document, paywallSchema) {
  const expected = expectedDocumentCapabilities(document);
  return paywallSchema.$defs.capabilityName.enum
    .filter((name) => expected.has(name))
    .map((name) => ({ name, version: paywallSchema.$defs.version.const }));
}
...
export function validateLocalization(errors, document, nodeEntries) {
  ...
  for (const { node, ancestors } of walkDocumentNodes(document)) {
    if (
      node.type === "text" &&
      ancestors.some((ancestor) => ancestor.type === "productCard")
    ) {
...
function entryIsVisible(entry, selectionState) {
  return [...entry.ancestors, entry.node].every((node) =>
    evaluateVisibility(node.visibility, selectionState),
  );
}
...
export function paywallAuthoringWarnings(document) {
  const warnings = [];
    ...
    for (const [state, style] of [
      ...
      for (const descendant of node.children ?? []) {
        const foreground = descendant.typography?.color
          ? resolveColorToken(document, descendant.typography.color)
          : null;
        const field = descendant.id;
        const resolvedBackground = style.background
          ? resolveBackgroundToken(document, style.background)
          : null;
        const background =
          resolvedBackground?.type === "color"
            ? resolveColorToken(document, resolvedBackground.value)
            : null;
...
export function validateCanonicalCoverage(document) {
  const errors = [];
  const entries = walkDocumentNodes(document);
  ...
  errors.push(...canonicalNewComponentCoverage(entries));
  return errors;
}
 ...
 */
function canonicalNewComponentCoverage(entries) {
  const errors = [];
...
export function validateRatingAnnouncementVectors(
  vectors = readPaywallJson(paywallRulesPaths.ratingAnnouncementVectors),
) {
  const errors = [];
  ...
  for (const entry of cases) {
    ...
    if (ratingPoints(entry.rating) !== entry.points) {
      errors.push(
        `rating-announcement case ${entry.id} records points ${entry.points} ` +
          `but the conversion yields ${ratingPoints(entry.rating)}`,
      );
    }
    if (ratingMaximumPoints(entry.rating) !== entry.maximumPoints) {
      errors.push(
    ...
    let announcement;
    try {
      announcement = resolveRatingAnnouncement(entry.rating, entry.template);
    } catch (error) {
  ...
  if (
    !cases.some(
      (entry) =>
        ratingPoints(entry.rating) !== String(entry.rating.value),
    )
  ) {
    errors.push(
...
export function validateAccessibilityAnnouncementVectors(
  ...
  const byId = new Map(
    walkDocumentNodes(document).map(({ node }) => [node.id, node]),
  );
  const seen = new Set();
  const types = new Set();
  ...
  for (const entry of cases) {
    ...
    let expected;
    try {
      expected = accessibilityAnnouncement(node, {
...
export function validatePaywallRulesJsonFormatting() {
  const errors = [];
  for (const filePath of Object.values(paywallRulesPaths)) {
    const source = readFileSync(filePath, "utf8");
    const formatted = `${JSON.stringify(JSON.parse(source), null, 2)}\n`;
    if (source === formatted) continue;
    errors.push(`${relative(paywallRulesRoot, filePath)} is not canonical JSON`);
  }
  return errors;
}
